...
$page_styles = [
    ...
    '/assets/css/merchant-customer-profile.css',
    '/assets/css/merchant-followup-tasks.css',
    '/assets/css/merchant-crm-retention-playbooks.css',
    '/assets/css/merchant-customer-agent-timeline.css',
];
$page_scripts = ['/assets/js/merchant-customer-profile-lite-controlled.js'];
...
foreach ($merchantNav as $key => $item) {
    $appSidebarNav[$key] = ['section' => $item[3] ?? '', 'label' => $item[0], 'detail' => $item[1], 'href' => $item[2], 'visible' => true, 'active' => $key === 'customer_profile'];
}
