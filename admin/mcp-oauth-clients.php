...
if (strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET')) === 'POST') {
    try {
        ...
        $created = mg_mcp_oauth_register_client($pdo, [
            ...
            'maximum_operation_class' => $_POST['maximum_operation_class'] ?? 'read',
...
$stmt = $pdo->query(
    "SELECT r.client_id,r.client_name,r.status,r.registration_type,r.redirect_uris_json,r.client_uri,
            r.last_used_at,r.created_at,c.client_type,c.maximum_operation_class
        ...
        <section class="mg-ai-panel">
          ...
          <div class="mg-admin-oauth-list">
            <?php foreach ($clients as $client): ?>
              <article><div><strong><?= mg_e((string)$client['client_name']) ?></strong><span><?= mg_e((string)$client['status']) ?></span></div><code><?= mg_e((string)$client['client_id']) ?></code><p><?= mg_e(implode("\n", mg_mcp_oauth_json_decode($client['redirect_uris_json']))) ?></p><small><?= mg_e((string)$client['registration_type']) ?> · <?= mg_e((string)$client['client_type']) ?> · <?= mg_e((string)$client['maximum_operation_class']) ?> · Last used <?= mg_e((string)($client['last_used_at'] ?? 'never')) ?></small></article>
            <?php endforeach; ?>
          </div>
        </section>
      </div>
