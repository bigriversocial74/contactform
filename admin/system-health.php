  ...
  <div class="mg-app-workspace mg-admin-workspace">
    <section class="mg-system-health-shell" data-admin-system-health data-can-security-audit="<?= $canViewSecurityAudit ? 'true' : 'false' ?>" data-sensitive-confirm-token="<?= mg_e($sensitiveActionToken) ?>">
      ...
      <div class="mg-system-health-tab-panels" data-system-health-tab-panels>
        ...
        <section class="mg-health-tab-panel" data-health-tab-panel="merchant-flow" hidden>
          <section class="mg-system-health-section mg-system-sql-diagnostics" data-merchant-flow-diagnostics>
            <header>
              <div>
                <h2>Merchant flow contract diagnostics</h2>
                <p>Runs read-only contract checks for campaign rewards, Customer Refund sends, wallet/PPPM ownership, and ledger/stamp debit dependencies.</p>
              </div>
              <div class="mg-system-sql-actions">
                <button class="mg-btn mg-btn-ghost" type="button" data-merchant-flow-refresh disabled>Run merchant flow checks</button>
              </div>
            </header>
            <div class="mg-system-sql-summary is-loading" data-merchant-flow-summary>Loading merchant flow diagnostics…</div>
            <div class="mg-system-sql-metrics" data-merchant-flow-metrics>
              <?php foreach (['Groups','Checks','Critical','Warnings','Unavailable'] as $label): ?>
                <article><span><?= mg_e($label) ?></span><strong>—</strong><small>Waiting for diagnostics</small></article>
              <?php endforeach; ?>
            </div>
            <div class="mg-system-sql-panels">
              <div>
                <h3>Contract groups</h3>
                <div class="mg-system-sql-list" data-merchant-flow-groups><p class="mg-muted">Loading contract groups…</p></div>
              </div>
              <div>
                <h3>Top findings</h3>
