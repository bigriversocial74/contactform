...
<article style="background:#fff;border:1px solid #dce7f4;border-radius:20px;padding:26px"><h2>Account linking</h2><p>The app starts a link flow, the user signs in or creates a Microgifter account, and Microgifter returns a linked account reference for future rewards.</p></article>
<article style="background:#fff;border:1px solid #dce7f4;border-radius:20px;padding:26px"><h2>Issue a reward</h2><p>The issue endpoint validates app access, program status, product membership, recipient link state, campaign limits, and idempotency before queueing issuance jobs.</p></article>
<article style="background:#fff;border:1px solid #dce7f4;border-radius:20px;padding:26px"><h2>Webhooks and status</h2><p>Apps can check reward status and receive lifecycle callbacks for accepted, queued, issued, delivered, claimed, redeemed, failed, linked, and unlinked events.</p></article>
</section>
</main>
