...
`assets/css/merchant-agent-chat-desktop.css` now uses a desktop-only no-wrap flex row for the footer composer instead of relying on grid column rules that could still wrap controls in narrower desktop widths.
