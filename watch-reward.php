...
$description = trim((string)($campaign['form_description'] ?? '')) ?: (trim((string)($campaign['description'] ?? '')) ?: 'Watch this video and unlock rewards as you reach the milestones.');
?>
<section class="mg-public-campaign mg-public-campaign-v2 mg-watch-reward-page" data-watch-video-reward data-campaign-id="<?= mg_e((string)$campaign['public_id']) ?>" data-video-provider="<?= mg_e($provider) ?>" data-video-id="<?= mg_e($videoId) ?>" data-uploaded-video-url="<?= mg_e((string)$uploadedUrl) ?>" data-uploaded-asset-id="<?= mg_e($uploadedAssetId) ?>" data-required-percent="<?= mg_e((string)$requiredPercent) ?>">
  ...
  <div class="mg-public-campaign-shell">
    <div class="mg-public-campaign-heading mg-watch-clean-heading"><h1><?= mg_e($headline) ?></h1><p><?= mg_e($description) ?></p></div>
    <aside class="mg-public-campaign-card mg-public-campaign-flow-card mg-listen-tab-card">
      ...
      <nav class="mg-listen-tabs" aria-label="Watch reward sections"><button type="button" class="is-active" data-watch-tab-trigger="join" aria-selected="true">1. Join</button><button type="button" data-watch-tab-trigger="media" aria-selected="false" aria-disabled="true">2. Watch</button><button type="button" data-watch-tab-trigger="rewards" aria-selected="false" aria-disabled="true">3. Rewards</button></nav>
      <div class="mg-listen-tab-panel is-active" data-watch-tab-panel="join">
        <?php if (!$isLoggedIn): ?><div class="mg-listen-account-gate"><strong>Account recommended for campaign participation</strong><p>Most reward campaigns should connect to a signed-in account so Inbox delivery, PPPM tracking, reward history, and campaign history stay tied to the right customer. For now, this page still supports email-based participation while the account-gated flow is finalized.</p><div><a class="mg-btn mg-btn-soft" href="/signin.php">Sign in</a><a class="mg-btn mg-btn-primary" href="/signup.php">Create account</a></div></div><?php endif; ?>
        <form class="mg-public-campaign-form" data-watch-reward-form novalidate><input type="hidden" name="campaign_id" value="<?= mg_e((string)$campaign['public_id']) ?>"><div class="mg-public-campaign-field-grid"><label>Name<input name="name" placeholder="Your name" maxlength="180" value="<?= mg_e($prefillName) ?>"></label><label>Email<input name="email" type="email" required placeholder="you@example.com" maxlength="255" value="<?= mg_e($prefillEmail) ?>"></label><label class="mg-public-campaign-field-wide">Phone <span>(optional)</span><input name="phone" maxlength="60" placeholder="Optional"></label></div><button class="mg-btn mg-btn-primary mg-public-campaign-primary-action" type="submit">Next Step <span aria-hidden="true">→</span></button></form>
        <div class="mg-public-campaign-status mg-listen-join-status" data-watch-reward-status>Enter your info to continue.</div>
        <div class="mg-public-campaign-step-grid mg-listen-form-steps" aria-label="How watch rewards work"><div class="mg-public-campaign-mini-step"><span>1</span><strong>Add your info</strong><small>Use the email you want tied to your Microgifter Inbox.</small></div><div class="mg-public-campaign-mini-step"><span>2</span><strong>Join the campaign</strong><small>Microgifter records the campaign source and reward status.</small></div><div class="mg-public-campaign-mini-step"><span>3</span><strong>Open Inbox</strong><small>Eligible rewards continue through Inbox and PPPM tracking.</small></div></div>
      </div>
      <div class="mg-listen-tab-panel" data-watch-tab-panel="media" hidden>
        <div class="mg-public-campaign-reward mg-public-campaign-reward-tab mg-watch-track-row"><div class="mg-media-art-thumb"><?php if ($mediaImageUrl): ?><img src="<?= mg_e($mediaImageUrl) ?>" alt="<?= mg_e((string)($campaign['title'] ?? 'Video reward')) ?> artwork"><?php else: ?><div class="mg-media-art-placeholder">Video</div><?php endif; ?></div><div><span>Video</span><strong><?= mg_e((string)($campaign['title'] ?? 'Video reward')) ?></strong><em><?= mg_e((string)($campaign['reward_template_title'] ?? 'Watch reward')) ?></em></div></div>
        ...
        <div class="mg-listen-notification-panel"><div class="mg-listen-panel-head"><span class="mg-eyebrow">Realtime notifications</span><strong data-watch-reward-status>Enter your info to start watching.</strong></div><ul data-watch-reward-notifications><li>Waiting for watcher session.</li></ul></div>
        <div class="mg-listen-history-panel"><div class="mg-listen-panel-head"><span class="mg-eyebrow">Watch history</span><strong>Session activity</strong></div><ul data-watch-reward-history><li>No watch activity yet.</li></ul></div>
      </div>
      <div class="mg-listen-tab-panel" data-watch-tab-panel="rewards" hidden>
        <div class="mg-listen-reward-inbox-panel"><strong>Reward and Inbox status</strong><p>Gift activity appears here when a reward is sent. Open the Microgifter Inbox to view, manage, claim, redeem, or continue PPPM tracking.</p><a class="mg-btn mg-btn-primary" href="/inbox.php">Open Microgifter Inbox</a></div>
        <div class="mg-listen-history-panel"><div class="mg-listen-panel-head"><span class="mg-eyebrow">Reward history</span><strong>Issued rewards and Inbox status</strong></div><ul data-watch-reward-issue-history><li>No rewards issued yet.</li></ul></div>
