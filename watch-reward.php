...
function mg_watch_reward_safe_url(mixed $value): ?string
{
...
function mg_watch_reward_load(string $ref): ?array
{
    ...
    $stmt = $pdo->prepare("SELECT c.*, u.display_name merchant_user_display_name, u.full_name merchant_user_full_name,
            ...
            rt.public_id reward_template_public_id, rt.title reward_template_title, rt.description reward_template_description, rt.value_amount_cents, rt.currency
        FROM campaigns c
        ...
        WHERE c.status='active' AND c.campaign_type='watch_video_reward' AND (c.public_id=? OR c.public_slug=?) LIMIT 1");
...
?>
<section class="mg-public-campaign mg-public-campaign-v2 mg-watch-reward-page" data-watch-video-reward data-campaign-id="<?= mg_e((string)$campaign['public_id']) ?>" data-video-id="<?= mg_e($videoId) ?>" data-required-percent="<?= mg_e((string)$requiredPercent) ?>">
  ...
  <div class="mg-public-campaign-shell">
    ...
    <aside class="mg-public-campaign-card mg-public-campaign-flow-card">
      ...
      <?php if ($videoId === ''): ?><div class="mg-public-campaign-result is-visible"><strong>This Watch Video Reward campaign needs a valid YouTube video before it can be viewed.</strong></div><?php else: ?>
      <form class="mg-public-campaign-form" data-watch-reward-form novalidate>
        <input type="hidden" name="campaign_id" value="<?= mg_e((string)$campaign['public_id']) ?>">
        <div class="mg-public-campaign-field-grid"><label>Name<input name="name" placeholder="Your name" maxlength="180" value="<?= mg_e($prefillName) ?>"></label><label>Email<input name="email" type="email" required placeholder="you@example.com" maxlength="255" value="<?= mg_e($prefillEmail) ?>"></label><label class="mg-public-campaign-field-wide">Phone <span>(optional)</span><input name="phone" maxlength="60" placeholder="Optional"></label></div>
        <button class="mg-btn mg-btn-primary mg-public-campaign-primary-action" type="submit">Start video rewards <span aria-hidden="true">→</span></button>
      </form>
      <div class="mg-public-campaign-reward mg-public-campaign-reward-tab"><span>Gift milestones</span><strong><?= mg_e((string)($campaign['reward_template_title'] ?? 'Video reward')) ?></strong><em><?= mg_e($requiredPercent) ?>% watch target</em><p><?= mg_e(count($milestones) ? implode(' · ', array_map(static fn($m): string => (string)($m['percent'] ?? '') . '% gift', $milestones)) : 'Watch progress unlocks the attached reward.') ?></p></div>
      <div class="mg-public-campaign-video" data-watch-video-shell hidden><div id="mg-watch-video-player"></div></div>
      <div class="mg-public-campaign-status" data-watch-reward-status>Enter your info to start watching.</div>
      <div class="mg-campaign-checklist"><span class="mg-eyebrow">Progress</span><ul data-watch-reward-milestones><?php foreach ($milestones as $m): ?><li class="is-warn" data-watch-milestone="<?= mg_e((string)($m['percent'] ?? '')) ?>"><b></b><span><?= mg_e((string)($m['percent'] ?? '')) ?>% — <?= mg_e((string)($m['label'] ?? 'Gift milestone')) ?></span></li><?php endforeach; ?></ul></div>
