...
export function boot(){
  ...
  if(!root)return;
  const state={data:null,selected:new Map(),merchantResults:[],loading:false};
  ...
  function syncMode(){const mode=reconcileForm.querySelector('input[name="execution_mode"]:checked')?.value||'dry_run';const repairing=mode==='repair';const mutable=canManage();repairModes.disabled=!mutable||!repairing;repairModes.querySelectorAll('input').forEach((input)=>{input.disabled=!mutable||!repairing});repairConfirm.hidden=!repairing;reconcileSubmit.textContent=repairing?'Execute safe repair':'Run dry reconciliation'}
  function applyAccess(){if(canManage())return;setBusy(rolloutForm,true);setBusy(reconcileForm,true);setNotice(rolloutNotice,'Read-only access: rollout controls require the manage permission.');setNotice(reconcileNotice,'Read-only access: reconciliation execution requires the manage permission.');selectedBlock.classList.add('is-disabled')}
  function renderAll(){renderStats(root,state.data);renderReadiness(root,state.data);renderRollout(root,state.data,state.selected,removeMerchant);renderPermissions(root,state.data,syncMode);renderReceipts(root,state.data);renderOperations(root,state.data);applyAccess();root.querySelector('[data-pdo-updated]').textContent=new Intl.DateTimeFormat(undefined,{hour:'numeric',minute:'2-digit',second:'2-digit'}).format(new Date())}
  ...
  async function searchMerchants(){if(!canManage())return;const query=merchantQuery.value.trim();if(!query){setNotice(rolloutNotice,'Enter a merchant name, email, or user ID.','error');return}const button=root.querySelector('[data-pdo-search-merchants]');button.disabled=true;clear(merchantResults);try{const data=await getJson(`/api/admin/public-donations-operations.php?q=${encodeURIComponent(query)}`);state.merchantResults=Array.isArray(data.merchant_search)?data.merchant_search:[];renderSearchResults()}catch(error){setNotice(rolloutNotice,error.message||'Unable to search merchants.','error')}finally{button.disabled=false}}

  featureState.addEventListener('change',()=>selectedBlock.classList.toggle('is-disabled',featureState.value!=='selected_merchants'));
  root.querySelector('[data-pdo-clear-merchants]').addEventListener('click',()=>{if(!canManage())return;state.selected.clear();renderSelected();renderSearchResults()});
