...
if(!Array.prototype.some.call(typeSelect.options,function(option){return option.value==='loyalty_quest';})){
...
card.hidden=true;
card.innerHTML='<span class="mg-eyebrow">Loyalty Quest</span><h3>Create a verified local challenge.</h3><p>Require a Microgifter account, choose the participant action, verification method, audience, location, limits, and reward.</p>'+
'<div class="mg-grid-2"><label>Quest action<select name="quest_action_type"><option value="location_visit">Visit a location</option><option value="signed_qr">Scan a signed QR code</option><option value="purchase">Make a purchase</option><option value="product_purchase">Purchase a selected product</option><option value="event_attendance">Attend an event</option><option value="referral">Complete a referral</option><option value="social_action">Complete a social action</option><option value="milestone">Complete a milestone</option><option value="multi_location">Visit multiple locations</option><option value="sequence">Complete a sequence</option><option value="invite_code">Enter an invite code</option></select></label><label>Verification<select name="quest_verification_type" data-lq-verification-select><option value="signed_qr">Signed QR</option><option value="static_qr">Static QR</option><option value="geolocation">Geolocation</option><option value="purchase_record">Purchase record</option><option value="receipt_review">Receipt / proof review</option><option value="staff_confirmation">Staff confirmation</option><option value="event_check_in">Event check-in</option><option value="microgifter_transaction">Microgifter transaction</option><option value="referral_conversion">Referral conversion</option><option value="manual_review">Manual review</option></select></label></div>'+
'<div class="mg-grid-2"><label>Audience<select name="quest_visibility" data-lq-visibility-select><option value="public">Public</option><option value="customers">Existing customers</option><option value="loyalty_members">Loyalty members</option><option value="new_customers">New customers</option><option value="invite_only">Invite only</option><option value="campaign_contacts">Campaign contacts</option><option value="geographic_radius">Geographic radius</option></select></label><label>Merchant location ID<input name="quest_location_id" maxlength="64" placeholder="Location public ID"></label></div>'+
'<div class="mg-grid-2"><label>Verification radius (meters)<input name="quest_radius_meters" type="number" min="25" max="5000" value="150"></label><label>Maximum location accuracy (meters)<input name="quest_maximum_accuracy_meters" type="number" min="25" max="1000" value="250"></label></div>'+
'<div class="mg-grid-2"><label>Required action count<input name="quest_required_count" type="number" min="1" max="100" value="1"></label><label>Repeat-action cooldown (hours)<input name="quest_cooldown_hours" type="number" min="0" max="8760" value="24"></label></div>'+
'<label>Participant instructions<textarea name="quest_instructions" maxlength="2000" placeholder="Explain exactly what the participant must do."></textarea></label><label>Eligibility and terms<textarea name="quest_eligibility" maxlength="1000" placeholder="Age, customer, event, location, or purchase requirements."></textarea></label>'+
'<div class="mg-grid-2"><label data-lq-secret-field="invite_only">Invite code<input name="quest_invite_code" maxlength="64" autocomplete="new-password" placeholder="Leave blank to preserve an existing code"><small>Stored as a one-way hash.</small></label><label data-lq-secret-field="static_qr">Static QR completion code<input name="quest_completion_code" maxlength="128" autocomplete="new-password" placeholder="Leave blank to preserve an existing code"><small>Stored as a one-way hash.</small></label></div>'+
'<div class="mg-grid-2"><label data-lq-secret-field="staff_confirmation">Staff confirmation code<input name="quest_staff_confirmation_code" maxlength="128" autocomplete="new-password" placeholder="Leave blank to preserve an existing code"><small>Stored as a one-way hash.</small></label><label data-lq-secret-field="event_check_in">Event check-in code<input name="quest_event_checkin_code" maxlength="128" autocomplete="new-password" placeholder="Leave blank to preserve an existing code"><small>Stored as a one-way hash.</small></label></div>'+
'<div class="mg-grid-2"><label>Daily verified-action limit<input name="quest_daily_limit" type="number" min="1" placeholder="Unlimited"></label><label>Reward budget limit<input name="quest_budget_limit" type="number" min="0.01" step="0.01" placeholder="Optional campaign budget"></label></div>'+
'<div class="mg-grid-2"><label class="mg-campaign-check"><input type="checkbox" name="quest_proof_required" value="1"><span>Require participant proof</span></label><label class="mg-campaign-check"><input type="checkbox" name="quest_staff_confirmation_required" value="1"><span>Require merchant staff confirmation</span></label></div>'+
'<p class="mg-muted">Signed QR codes use the campaign QR secret and expiring signed payloads. Static, staff, event, and invite codes are never returned after saving.</p>';
...
function updateSecretFields(){
  var verification=String(form.elements.quest_verification_type&&form.elements.quest_verification_type.value||'');
  var visibility=String(form.elements.quest_visibility&&form.elements.quest_visibility.value||'');
  card.querySelectorAll('[data-lq-secret-field]').forEach(function(field){var key=field.dataset.lqSecretField;field.hidden=key==='invite_only'?visibility!=='invite_only':verification!==key;});
}
function updateVisibility(){
  card.hidden=typeSelect.value!=='loyalty_quest';
  if(typeSelect.value==='loyalty_quest'){setDefaults();updateSecretFields();}
}
typeSelect.addEventListener('change',updateVisibility);
if(form.elements.quest_verification_type)form.elements.quest_verification_type.addEventListener('change',updateSecretFields);
if(form.elements.quest_visibility)form.elements.quest_visibility.addEventListener('change',updateSecretFields);
updateVisibility();
...
form.addEventListener('submit',async function(event){
  ...
  try{
    ...
    ['quest_invite_code','quest_completion_code','quest_staff_confirmation_code','quest_event_checkin_code'].forEach(function(name){if(form.elements[name])form.elements[name].value='';});
