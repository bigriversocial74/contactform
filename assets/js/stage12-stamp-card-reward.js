document.addEventListener('DOMContentLoaded', function () {
  ...
  function setField(name, next) { var item = field(name); if (item) item.value = next == null ? '' : String(next); }
  ...
  function ensureFields() {
    ...
    card.hidden = true;
    card.innerHTML = '<span class="mg-eyebrow">Stamp Card / Visit Tracker</span><h3>Track repeat visits until a reward unlocks.</h3><p>Each customer gets a unique punch card for this campaign. Set how many punches are required before the reward is issued.</p><div class="mg-grid-2"><label>Required punches<input name="stamp_required_count" type="number" min="1" max="100" inputmode="numeric" placeholder="5"></label><label>Punch label<input name="stamp_label" maxlength="40" placeholder="Visit"></label></div><div class="mg-grid-2"><label>Cooldown hours<input name="stamp_cooldown_hours" type="number" min="0" max="8760" inputmode="numeric" placeholder="0"></label><label class="mg-campaign-check"><input type="checkbox" name="stamp_card_reward_enabled" value="1" checked> <span>Issue reward when card is full</span></label></div><p class="mg-form-hint">This value is saved into campaign rules and controls the public punch-card layout and reward issuance threshold.</p>';
  ...
  function hydrateRulesFromCampaign(campaign) {
    if (!campaign || campaign.campaign_type !== 'stamp_card_reward') return;
    var rules = campaign.rules || {};
    setField('stamp_required_count', rules.required_count || rules.stamp_required_count || 5);
    setField('stamp_label', rules.stamp_label || 'Visit');
    setField('stamp_cooldown_hours', rules.cooldown_hours || rules.stamp_cooldown_hours || 0);
    var enabled = field('stamp_card_reward_enabled');
    if (enabled) enabled.checked = rules.entry_reward_enabled !== false;
  }

  var lastHydratedId = '';
  async function syncExistingRules() {
    if (!window.Microgifter || typeof Microgifter.get !== 'function') return;
    if (activeType() !== 'stamp_card_reward') return;
    var id = value('campaign_id');
    if (!id || id === lastHydratedId) return;
    lastHydratedId = id;
    try {
      ...
      hydrateRulesFromCampaign(campaign);
    } catch (error) {}
  }
  ...
  root.addEventListener('click', function (event) {
    ...
    if (event.target && event.target.getAttribute && event.target.getAttribute('data-campaign-edit-id')) window.setTimeout(syncVisibility, 250);
  });
  window.setInterval(syncExistingRules, 900);
  syncVisibility();
});