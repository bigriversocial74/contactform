...
.then(data=>{
  ...
  policy.innerHTML=policies.length?`<h2>Merchant payout policies</h2><div class="mg-cce-policy-grid">${policies.map(p=>`<section><h3>${esc(p.merchant_name||'Merchant')}</h3><div><small>${esc(p.currency)} schedule</small><strong>${esc(label(p.cadence))}${p.next_payout_date?` · ${esc(p.next_payout_date)}`:''}</strong></div><div><small>Commission hold</small><strong>${esc(p.hold_days)} days</strong></div><div><small>Minimum payout</small><strong>${money(p.minimum_payout_minor,p.currency)}</strong></div><div><small>Payment method</small><strong>${esc(p.method_label||'Merchant-approved external method')}</strong></div><p class="mg-cce-card-note">${esc(p.payment_instructions||'Every payout requires merchant approval and an externally confirmed provider reference.')}</p></section>`).join('')}</div>`:`<h2>Payout policy</h2><p class="mg-cce-card-note">The merchant has not published a payout schedule yet. Earnings and budget commitments remain visible below.</p>`;
