...
function hideField(f,n){var l=label(field(f,n));if(l&&!l.querySelector('[data-campaign-select]'))l.hidden=true;}
...
function ensureNote(f){var s=f&&f.querySelector('[data-campaign-select]');if(!s)return null;var note=f.querySelector('[data-target-reward-note]');if(!note){note=document.createElement('div');note.className='mg-form-status';note.dataset.targetRewardNote='1';var wrap=s.closest('label');(wrap||s).insertAdjacentElement(wrap?'afterend':'afterend',note);}return note;}
...
function sync(f){
 ...
 if(has){
  ...
  setValue(f,'quantity_limit',(o&&o.dataset.available)||(o&&o.dataset.quantity)||'');
  setValue(f,'claim_limit_per_user',(o&&o.dataset.limit)||'1');
 }else{
  setValue(f,'campaign_title','');
  setValue(f,'payload_type','reward');
  setValue(f,'quantity_limit','');
  setValue(f,'claim_limit_per_user','1');
 }
...
window.MicrogifterTargetDropCampaignSync=sync;
