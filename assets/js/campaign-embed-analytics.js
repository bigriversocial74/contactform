document.addEventListener('DOMContentLoaded', function () {
  ...
  function setAlert(message, tone) { var node = root.querySelector('[data-embed-analytics-alert]'); if (!node) return; node.hidden = !message; node.className = 'mg-embed-analytics-alert' + (tone ? ' is-' + tone : ''); node.innerHTML = message ? '<strong>' + esc(message) + '</strong>' : ''; }
  ...
  function apiUrl() {
    ...
    return '/api/merchant/campaign-embed-analytics.php?' + params.toString();
  }
  ...
  function renderCampaignTable(rows) {
    ...
    if (!table) return;
    rows = rows || [];
    if (!rows.length) { table.innerHTML = '<tbody><tr><td>No campaign embed activity yet.</td></tr></tbody>'; return; }
  ...
  function renderRecent(rows) {
    ...
    if (!node) return;
    rows = rows || [];
    if (!rows.length) { node.innerHTML = '<p class="mg-empty-copy">No recent embed events yet.</p>'; return; }
    node.innerHTML = rows.map(function (row) {
  ...
  async function loadAnalytics(pushState) {
    setAlert('Loading campaign embed analytics...', 'info');
    try {
      ...
      if (data.migration_ready === false) setAlert('SQL required: ' + (data.sql_required || 'database/campaign_embed_settings_v2.sql'), 'warn');
      else if (!(data.recent_events || []).length && Number((data.totals || {}).loaded || 0) === 0) setAlert('No embed events yet. Use the QA page or a live website embed to start collecting data.', 'info');
      else setAlert('', '');
      if (pushState && window.history) window.history.replaceState({}, '', '/merchant-campaign-embed-analytics.php?' + apiUrl().split('?')[1]);
    } catch (error) {
      setAlert(error.message || 'Unable to load campaign embed analytics.', 'warn');
    }
  }
