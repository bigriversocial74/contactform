...
function loadStyles(){if(document.querySelector('link[data-crm-review-checklist-css]'))return;var l=document.createElement('link');l.rel='stylesheet';l.href='/assets/css/merchant-crm-review-checklist.css';l.setAttribute('data-crm-review-checklist-css','1');document.head.appendChild(l)}
function loadAuditTrail(){if(document.querySelector('script[data-crm-launch-audit-script]'))return;var s=document.createElement('script');s.src='/assets/js/merchant-crm-launch-audit-trail.js';s.defer=true;s.setAttribute('data-crm-launch-audit-script','1');document.head.appendChild(s)}
...
function ensure(){loadStyles();loadAuditTrail();if(qs('[data-crm-approval-gate]',root))return;var el=document.createElement('div');el.className='mg-crm-review-checklist';el.setAttribute('data-crm-approval-gate','');el.innerHTML='<div><span class="mg-eyebrow">Review checklist</span><strong data-crm-approval-title>Needs review</strong><p data-crm-approval-copy>Complete the checklist before using the reviewed draft.</p></div><ul data-crm-approval-checks></ul><button class="mg-btn mg-btn-soft" type="button" data-crm-approval-ready>Mark Ready</button>';root.insertBefore(el,root.firstElementChild||null)}
...
function block(ev){if(!ev.target||!ev.target.closest('[data-crm-launch-campaign]'))return;render();if(ready())return;ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();var status=qs('[data-crm-builder-status]',root);if(status)status.textContent='Review checklist blocked this draft. Complete the checklist first.';toast('Complete the review checklist first.')}
