document.addEventListener('DOMContentLoaded',function(){
  ...
  function renderDetail(o){
    ...
    var add=o.can_add_to_wallet?'<button class="mg-btn mg-btn-primary" type="button" data-add-wallet="'+safe(o.id)+'">Add to wallet</button><button class="mg-btn mg-btn-soft" type="button" data-save-offer="'+safe(o.id)+'">Save signal</button>':'<span class="mg-muted">Wallet add unavailable</span>';
    ...
    detail.querySelectorAll('[data-add-wallet]').forEach(function(button){button.addEventListener('click',async function(){
      try{note('Adding offer to wallet...');await feedback(button.getAttribute('data-add-wallet'),'add_attempt');var r=await Microgifter.post('/api/public/wallet/add.php',{offer_id:button.getAttribute('data-add-wallet')});await feedback(button.getAttribute('data-add-wallet'),'add_success');note(r.message||'Added to wallet.');}
