(function (window, document) {
  ...
  function openEmbeddedPost() {
    ...
    if (typeof window.Microgifter.openCreateCenterPost === 'function') {
      return window.Microgifter.openCreateCenterPost() !== false;
    }
  ...
  document.addEventListener('click', function (event) {
    var postOption = event.target && event.target.closest
      ? event.target.closest('[data-create-menu-option="post"],[data-create-inline-target="post"]')
      : null;
    if (!postOption) return;
    if (!embeddedPostNodes() && !q('[data-global-post-composer]')) return;
