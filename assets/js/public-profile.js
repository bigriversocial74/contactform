...
(function (window, document) {
  ...
  function setText(selector, value) {
    var target = node(selector, root);
    if (target) target.textContent = value === undefined || value === null ? '' : String(value);
  }

  function setHidden(target, hidden) {
    if (!target) return;
    target.classList.toggle('mg-hidden', Boolean(hidden));
  }
  ...
  function renderCover(profile) {
    ...
    if (!coverUrl) return;
    var clean = coverUrl.replace(/["'\\\n\r]/g, '');
    cover.style.backgroundImage = 'url("' + clean + '")';
  }
  ...
  function renderStatus(profile) {
    ...
    if (profile.visibility === 'unlisted') {
      row.appendChild(createPill('Unlisted', 'is-unlisted'));
      ensureNoIndex();
    }
    if (profile.availability && profile.availability.is_preview) {
      row.appendChild(createPill('Owner preview', 'is-preview'));
      ensureNoIndex();
    }
  }
  ...
  function renderMeta(profile) {
    ...
    if (profile.location_label) {
      meta.appendChild(createPill(String(profile.location_label), 'is-location'));
    }
  }
  ...
  function setState(state, error) {
    ...
    if (state === 'error') {
      ...
      setText(
        '[data-profile-error-message]',
        notFound
          ? 'This profile may be private, still in draft, suspended, blocked, or using a different address.'
  ...
  async function loadProfile() {
    ...
    var path = '/api/public/profile.php?slug=' + encodeURIComponent(slug)
      + '&product_limit=1&post_limit=1&plan_limit=1'
      + (preview ? '&preview=1' : '');
