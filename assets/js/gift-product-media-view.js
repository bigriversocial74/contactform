document.addEventListener('DOMContentLoaded',function(){
  ...
  function load(wanted){var missing=wanted.filter(function(id){return id&&!cache[id];});if(!missing.length){apply();return Promise.resolve();}return fetch('/api/account/action-center-product-media.php?ids='+encodeURIComponent(missing.join(',')),{credentials:'same-origin'}).then(function(response){return response.json();}).then(function(payload){var data=payload.data||payload;Object.keys(data.items||{}).forEach(function(id){cache[id]=data.items[id]||{};});apply();}).catch(function(error){console.error(error);});}
  ...
  app.addEventListener('click',function(event){var button=event.target.closest('[data-gift-action="load"]'),row=event.target.closest('[data-gift-id]');if(!button||!row)return;pending=row.getAttribute('data-gift-id')||'';window.setTimeout(function(){load([pending]);},150);},true);
  run();
});
