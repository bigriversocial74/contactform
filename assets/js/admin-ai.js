document.addEventListener('DOMContentLoaded', function () {
  ...
  function diagnosticHtml(provider) {
    var diagnostic = provider.key_diagnostic;
    if (!diagnostic || provider.provider_key !== 'anthropic') return '';
    var warnings = Array.isArray(diagnostic.warnings) ? diagnostic.warnings : [];
    var warningHtml = warnings.length
      ? '<ul>' + warnings.map(function (warning) { return '<li>' + esc(warning) + '</li>'; }).join('') + '</ul>'
      : '<p>No local formatting warning detected. If chat still says invalid x-api-key, Anthropic is rejecting this exact loaded key or PHP is still serving an old config value.</p>';
    return '<section class="mg-ai-key-diagnostic"><h3>Server key diagnostic</h3><div class="mg-ai-key-grid"><span>Loaded</span><strong>' + (diagnostic.configured ? 'Yes' : 'No') + '</strong><span>Prefix</span><code>' + esc(diagnostic.prefix || '—') + '</code><span>Suffix</span><code>' + esc(diagnostic.suffix || '—') + '</code><span>Length</span><code>' + esc(diagnostic.length || 0) + '</code><span>Fingerprint</span><code>' + esc(diagnostic.fingerprint || '—') + '</code></div>' + warningHtml + '<p class="mg-ai-key-note">This does not expose the full secret. Compare the prefix, suffix, length, and fingerprint after changing <code>api/config.local.php</code> to confirm the server is using the new key.</p></section>';
  }
