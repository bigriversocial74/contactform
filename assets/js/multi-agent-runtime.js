document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('click',function(event){
    var prompt=event.target.closest('[data-agent-seed-prompt]');if(prompt){event.preventDefault();composer.elements.message.value=prompt.getAttribute('data-agent-seed-prompt')||'';composer.elements.message.focus();return;}
    var thread=event.target.closest('[data-runtime-thread]');if(thread){event.preventDefault();load(thread.getAttribute('data-runtime-thread'));return;}
    var fresh=event.target.closest('[data-agent-new-thread]');if(fresh){event.preventDefault();request('POST','/api/agents/runtime.php',{id:agentId,action:'new_thread'}).then(function(data){load(data.thread.id);}).catch(function(error){status.textContent=error.message;});return;}
    var draft=event.target.closest('[data-save-agent-draft]');if(draft){event.preventDefault();var payload={};try{payload=JSON.parse(draft.getAttribute('data-draft-payload')||'{}');}catch(e){}request('POST','/api/agents/runtime.php',{id:agentId,action:'save_draft',thread_id:currentThread,title:draft.getAttribute('data-draft-title')||'Agent draft',draft_type:'plan',payload:payload}).then(function(){status.textContent='Reviewable draft saved.';}).catch(function(error){status.textContent=error.message;});}
  });
