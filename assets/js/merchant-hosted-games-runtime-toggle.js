(() => {
  ...
  function managedMarkup(game) {
    const readiness = game?.readiness || {};
    const values = [
      ['Configuration', game?.distribution_program_id ? 'Distribution Program' : 'Not configured', Boolean(game?.distribution_program_id)],
    ...
    return `<div class="hgm-runtime-managed">${values.map(([label, value, ready]) => `<div><span>${escapeHtml(label)}</span><strong class="${ready ? 'is-ready' : ''}">${escapeHtml(value)}</strong></div>`).join('')}</div>
      <p class="hgm-runtime-note">Microgifter manages the game API key, webhook secret, campaign, and reward template from the selected Distribution Program. No manual environment values are required for this hosted game.</p>`;
  }

  function decorateCards() {
    root.querySelectorAll('[data-game-card]').forEach((card) => {
      const gameButton = card.querySelector('[data-game-id]');
  ...
  function syncModal() {
    ...
    const gameId = String(root.querySelector('[data-hgm-integration-form] [name="game_id"]')?.value || '');
  ...
  async function load() {
    const data = await request('/api/merchant/hosted-games.php', { headers: { Accept: 'application/json' } });
    games.clear();
  ...
  async function toggle(button) {
    ...
    try {
      await request('/api/merchant/hosted-game-runtime.php', {
        method: 'POST',
  ...
  root.addEventListener('click', (event) => {
    ...
    if (event.target.closest('[data-hgm-action="edit"]')) window.setTimeout(syncModal, 0);
  }, true);
