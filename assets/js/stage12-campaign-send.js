document.addEventListener('DOMContentLoaded',function(){
  ...
  form.addEventListener('submit',async function(event){
    ...
    catch(error){var data=error.data||{};var code=(data.errors&&data.errors.code)||data.code||'';var msg=error.message||'Unable to record campaign send.';if(code==='insufficient_stamps'||/insufficient\s+stamps?/i.test(msg)){setStatusHtml(esc(msg)+' <a href="/merchant-stamps.php#stamp-purchases">Buy Stamps</a> to continue campaign distribution.','error');}else{setStatus(msg,'error');}}
  });
  updateEstimate();
  loadCampaigns();
});
