(() => {
  ...
  function addButton(container, attribute) {
    ...
    button.textContent = 'Save Creative Asset';
  ...
  function busy(button, value, label = 'Saving…') {
    ...
    if (value) {
      ...
      button.textContent = button.dataset.originalLabel || 'Save Creative Asset';
    }
  }
  ...
  async function waitImages(canvas) {
    await Promise.all(
      Array.from(canvas.querySelectorAll('img'))
        .filter((image) => !image.hidden && image.src)
        .map((image) => {
  ...
  function canvasBlob(canvas) {
    return new Promise((resolve, reject) => {
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(new Error('The creative image could not be prepared.'))),
        'image/jpeg',
        .96
      );
    });
  ...
  function idempotency(signature) {
    if (!keys.has(signature)) {
      keys.set(
        signature,
        `design-save:${window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`}`
      );
    }
  ...
  function captions() {
    ...
    if (item) {
      return {
        short: item.caption_short || '',
        standard: item.caption_standard || '',
        extended: item.caption_extended || '',
    ...
    const description = app.querySelector('[data-social-product-description]')?.textContent?.trim()
      || 'Discover this local product, service, or experience on Microgifter.';
    const short = `Take a closer look at ${title}.`;
    const hashtags = '#ShopLocal #SupportLocal #Microgifter';
    const standard = `${short}\n\n${description}\n\n${hashtags}`;
    ...
    return {
      ...
      platforms: {
        ...
        instagram: { short: `${short}\n${hashtags}`, standard, extended: standard },
        linkedin: { short, standard, extended: standard },
      },
    };
  }

  function socialComposition(canvas) {
    const template = app.querySelector('[data-social-template].is-active')?.dataset.socialTemplate
      || canvas.dataset.socialTemplate
      || scheduleItem?.template_key
      || 'hero-offer';
    const format = app.querySelector('[data-social-format].is-active')?.dataset.socialFormat
      || canvas.dataset.socialFormat
      || scheduleItem?.post_format
      || 'square';
    const layout = app.querySelector('[data-social-layout].is-active')?.dataset.socialLayout
      || canvas.dataset.socialLayout
      || scheduleItem?.layout_key
      || 'spotlight';

    return { template, format, layout };
  }
  ...
  async function save(kind) {
    ...
    if (!canvas || canvas.hidden) {
      status(kind, print ? 'Choose a print template first.' : 'Complete the social design steps first.', 'error');
      return;
    }
    ...
    const productId = String(
      print
        ? (scheduleItem?.product_id || query.get('product') || '')
        : (productSelect?.value || scheduleItem?.product_id || query.get('product') || '')
    );

    const composition = print
      ? {
          template: app.querySelector('[data-design-template].is-active')?.dataset.designTemplate || 'support-local',
          format: canvas.classList.contains('is-tent') ? 'tent' : 'poster',
          layout: app.querySelector('[data-design-template].is-active')?.dataset.designTemplate || 'support-local',
        }
      : socialComposition(canvas);

    const title = print
      ? (app.querySelector('[data-design-template].is-active strong')?.textContent?.trim() || 'Microgifter print creative')
      : (app.querySelector('[data-social-product-title]')?.textContent?.trim() || 'Microgifter social creative');
    const scheduleId = String(scheduleItem?.public_id || query.get('schedule') || '');

    busy(button, true);
    status(kind, 'Rendering and saving your creative…');

    try {
      await library();
      await waitImages(canvas);
      const rendered = await window.html2canvas(canvas, {
        backgroundColor: print ? '#eef3f8' : null,
        scale: print ? 3 : 2.5,
        useCORS: true,
        allowTaint: false,
        logging: false,
      ...
      const filename = `microgifter-${kind}-${composition.template}-${composition.format}-${Date.now()}.jpg`;
      const signature = [
        kind,
        productId,
      ...
      data.append('creative', blob, filename);
      data.append('idempotency_key', idempotency(signature));
      data.append('asset_kind', kind);
      data.append('template_key', composition.template);
      data.append('format_key', composition.format);
      data.append('layout_key', composition.layout);
      ...
      status(kind, error.message || 'The creative could not be saved.', 'error');
    } finally {
      busy(button, false);
    }
  }
  ...
  function calendarMode() {
    ...
    app.querySelectorAll('[data-design-mode]').forEach((item) => {
  ...
  function waitProduct(select, id, timeout = 5000) {
    if (!select || !id) return Promise.resolve(false);
  ...
  async function socialMode(product, format = 'square', layout = 'spotlight', template = 'hero-offer') {
    app.querySelector('[data-design-mode="social"]')?.click();
    const select = app.querySelector('[data-social-product-select]');
    if (product && select) {
      await waitProduct(select, product);
      select.value = product;
      select.dispatchEvent(new Event('change', { bubbles: true }));
    }

    const templateButton = app.querySelector(`[data-social-template="${CSS.escape(template)}"]`)
      || app.querySelector('[data-social-template]');
    templateButton?.click();
    app.querySelector(`[data-social-format="${CSS.escape(format)}"]`)?.click();
    app.querySelector(`[data-social-layout="${CSS.escape(layout)}"]`)?.click();
    app.scrollIntoView({ block: 'start' });
  }
  ...
  function printMode(format = 'poster', layout = 'support-local') {
    ...
    setTimeout(() => {
      app.querySelector(`[data-design-object="${CSS.escape(format)}"]`)?.click();
      setTimeout(
        () => app.querySelector(`[data-design-template="${CSS.escape(layout)}"]`)?.click(),
        60
      );
    }, 40);
    app.scrollIntoView({ block: 'start' });
  ...
  document.addEventListener('design-studio:schedule-context', (event) => {
    scheduleItem = event.detail?.item || null;
    window.MicrogifterDesignStudioScheduleContext = scheduleItem;
  });

  app.querySelector('[data-social-product-select]')?.addEventListener('change', () => {
    if (scheduleItem?.product_id !== app.querySelector('[data-social-product-select]')?.value) {
      scheduleItem = null;
    }
  });
  ...
  app.querySelectorAll('[data-design-mode]').forEach((button) => {
    button.addEventListener('click', () => {
      const calendar = app.querySelector('[data-calendar-mode-button]');
      calendar?.classList.remove('is-active');
      calendar?.setAttribute('aria-selected', 'false');
    });
  });
  ...
  if (mode === 'calendar') {
    ...
    requestAnimationFrame(() => printMode(
      ...
      String(query.get('layout') || 'support-local')
    ));
  }
})();
