document.addEventListener('DOMContentLoaded', function () {
  ...
  function closeModal() { if (!modal) return; modal.setAttribute('hidden', 'hidden'); modal.setAttribute('aria-hidden', 'true'); activeCampaign = null; activeSettings = null; }
  ...
  function renderHealth(campaign) {
    ...
    health.innerHTML = '<span class="mg-eyebrow">Embed health</span><ul>' + healthItem(enabled, enabled ? 'Embed is enabled.' : 'Embed is disabled.') + healthItem(active, active ? 'Campaign is active for public submissions.' : 'Campaign is not active; public submissions require activation.') + healthItem(hasRef, hasRef ? 'Embed reference is available.' : 'Embed reference is missing.') + healthItem(hasPublicUrl, hasPublicUrl ? 'Public page fallback is available.' : 'Public page fallback is unavailable.') + healthItem(true, 'Host CSS adoption script is available.') + '</ul>';
  }

  function renderAnalytics(stats) {
    var node = modal.querySelector('[data-campaign-embed-analytics]'); if (!node) return;
    stats = stats || {};
    node.innerHTML = '<span class="mg-eyebrow">Embed analytics</span><div class="mg-campaign-embed-stat-grid"><strong>' + esc(stats.loaded || 0) + '<small>Loaded</small></strong><strong>' + esc(stats.opened || 0) + '<small>Opened</small></strong><strong>' + esc(stats.submitted || 0) + '<small>Submitted</small></strong><strong>' + esc(stats.error || 0) + '<small>Errors</small></strong></div><p>Last origin: ' + esc(stats.last_origin_host || '—') + '</p>';
  }
  ...
  async function saveSettings() {
    ...
    } catch (error) { status(error.message || 'Unable to save embed settings.'); }
  }
  ...
  async function openEmbed(campaignId) {
    ...
    var title = modal.querySelector('#mg-campaign-embed-title'), summary = modal.querySelector('[data-campaign-embed-summary]'), publicLink = modal.querySelector('[data-campaign-public-link]');
