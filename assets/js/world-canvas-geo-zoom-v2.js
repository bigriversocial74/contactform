...
(function (window, document) {
  ...
  function clearLegacyClusters() {
    qsa('.mg-world-geo-cluster-layer,.mg-world-dot-cluster-layer', map).forEach(function (layer) {
      layer.replaceChildren();
      layer.hidden = true;
    });
    qsa('[data-world-node].is-cluster-hidden', map).forEach(function (node) {
      node.classList.remove('is-cluster-hidden');
    });
  }

  function ownedViewerNode() {
    return qs('[data-world-node].is-merchant.is-owned.is-geo-locked', map) ||
      qs('[data-world-node].is-merchant.is-owned', map) ||
      qs('[data-world-node].is-avatar.is-owned', map);
  }
  ...
  function animateTo(target, duration) {
    ...
    function frame(now) {
      ...
      applyViewport();
      if (animationProgress < 1) state.anim = window.requestAnimationFrame(frame);
      else {
        state.anim = 0;
  ...
  function isBlockedDragTarget(target) {
    var node = target.closest && target.closest('[data-world-node]');
    if (node) return false;
    return !!(target.closest && target.closest('button,a,input,select,textarea,label,[data-target-drop-id],[data-world-square-zoom],[data-world-drawer]'));
  }
  ...
  function moveDrag(event) {
    ...
    if (!state.dragMoved && Math.sqrt(dx * dx + dy * dy) < dragThreshold) return;
    if (!state.dragMoved) {
      state.dragMoved = true;
      map.classList.add('is-dragging');
      root.dataset.worldZoomInteraction = 'active';
      root.dataset.worldZoomInteractionReason = 'pan';
    }
    event.preventDefault();
    state.panX = state.startX + dx;
    state.panY = state.startY + dy;
    applyViewport();
  }
  ...
  function endDrag(event) {
    ...
    if (state.dragMoved) state.suppressNodeClickUntil = Date.now() + 350;
  ...
  map.addEventListener('click', function (event) {
    if (Date.now() <= state.suppressNodeClickUntil && event.target.closest('[data-world-node]')) {
      event.preventDefault();
