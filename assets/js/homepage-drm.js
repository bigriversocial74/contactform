(() => {
  ...
  const boot = () => {
    ...
    if (heroSearch) {
      ...
      const label = (item) => item.has_published_storefront || item.published_products > 0 ? 'Merchant' : 'User';
      ...
      const render = (items, query) => {
        ...
        if (!items.length) {
          results.innerHTML = '<div class="mg-hero-search-empty"><strong>No direct matches yet.</strong><a href="/discover.php?q=' + safeQuery + '">Search the network →</a></div>';
          return;
        }
        results.innerHTML = items.slice(0, 6).map((item) => {
          ...
          const avatar = avatarUrl ? '<img src="' + esc(avatarUrl) + '" alt="" loading="lazy">' : esc(initial);
          const meta = [label(item), item.location || item.headline || 'Microgifter Network'].filter(Boolean).join(' · ');
          return '<a class="mg-hero-search-result" href="' + esc(href) + '"><span class="mg-hero-search-avatar">' + avatar + '</span><span><strong>' + esc(name) + '</strong><small>' + esc(meta) + '</small></span></a>';
