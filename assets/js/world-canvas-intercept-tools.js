...
(function(window, document){
  ...
  function panel(){
    var p = document.querySelector('[data-world-intercept-panel]');
    if (p) return p;
    p = document.createElement('aside');
    p.className = 'mg-world-intercept-panel';
    p.dataset.worldInterceptPanel = '1';
  ...
  function render(){
    var p = panel();
    var body = p.querySelector('[data-intercept-body]');
    if (!body) return;
  ...
  function open(run){activeRun = run || activeRun; panel().classList.add('is-open'); loadTools().then(render);}
  function close(){panel().classList.remove('is-open');}
  async function loadTools(){
    try { var r = await window.Microgifter.get('/api/world-canvas/intercept-tools.php'); tools = ((r.data || r || {}).tools || []); } catch (e) { tools = []; }
  }
  async function attempt(){
    var p = panel(), msg = p.querySelector('[data-intercept-status]'), selected = p.querySelector('input[name="intercept_tool"]:checked');
    if (!activeRun || !activeRun.id) { if (msg) msg.textContent = 'No active run selected.'; return; }
    if (!selected) { if (msg) msg.textContent = 'Select a tool first.'; return; }
    try {
      if (msg) msg.textContent = 'Attempting intercept…';
      var c = csrf();
      var r = await window.Microgifter.post('/api/world-canvas/intercept-tools.php', {action:'attempt', run_id:activeRun.id, user_tool_id:selected.value, csrf_token:c, _csrf:c, csrf:c});
      var result = (r.data || r || {}).result || {};
      if (msg) msg.textContent = result.status === 'success' ? 'Intercept successful.' : 'Intercept missed.';
      document.dispatchEvent(new CustomEvent('mg:world-intercept-result', {detail:{result:result, run:activeRun}}));
    } catch (e) { if (msg) msg.textContent = e.message || 'Intercept failed.'; }
  }
  ...
  document.addEventListener('mg:world-delivery-run-created', function(event){
    var run = event.detail && event.detail.delivery_run ? event.detail.delivery_run : null;
    if (run && run.intercept_ready) activeRun = run;
  });
