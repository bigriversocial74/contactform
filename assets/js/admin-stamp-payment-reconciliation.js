document.addEventListener('DOMContentLoaded',function(){
  ...
  function actionButtons(p){
    ...
    if(state==='paid_uncredited')buttons.push('<button class="mg-btn mg-btn-soft" type="button" data-stamp-recovery="flag_paid_uncredited" data-purchase="'+esc(p.id)+'">Flag paid/uncredited</button>');
    ...
    if(p.status!=='credited'&&state!=='failed_payment')buttons.push('<button class="mg-btn mg-btn-soft" type="button" data-stamp-action="mark_failed" data-purchase="'+esc(p.id)+'">Mark failed</button>');
    if(p.status!=='credited'&&p.status!=='cancelled')buttons.push('<button class="mg-btn mg-btn-soft" type="button" data-stamp-action="mark_cancelled" data-purchase="'+esc(p.id)+'">Cancel</button>');
  ...
  if(reconList)reconList.addEventListener('click',function(event){var action=event.target.closest('[data-stamp-action]');if(action){applyAction(action);return;}var recovery=event.target.closest('[data-stamp-recovery]');if(recovery)applyRecovery(recovery);});
  loadQa();
  loadRecon();
});
