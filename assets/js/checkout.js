document.addEventListener('DOMContentLoaded',function(){
  ...
  function paymentAction(session){
    ...
    if(session.can_continue_provider&&session.checkout_url)return '<a class="mg-btn mg-btn-primary" href="'+C.esc(session.checkout_url)+'" rel="nofollow">Continue to Stripe Checkout</a>';
    ...
    if(session.provider_key!=='sandbox')return '<button class="mg-btn mg-btn-primary" type="button" disabled>Stripe Checkout unavailable</button>';
  ...
  async function load(){
    ...
    var notice=session.provider_key==='sandbox'?'Sandbox mode is active. No real card will be charged.':'Stripe hosts the payment form. Microgifter never stores raw card numbers.';
