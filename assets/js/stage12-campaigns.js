...
function setPreset(type){
  if(form&&type&&form.elements.campaign_type){form.elements.campaign_type.value=type;updateCampaignTypeUI(true);}
}
...
function matchesFilter(item,filter){
  var st=String(item.status||'draft');var type=String(item.campaign_type||'');
...
function ruleSummary(c){
  var r=c.rules||{};var mode=String(r.mode||'');
  if(c.campaign_type==='contest_giveaway'){
    if(mode==='first_x')return 'First '+count(r.winner_limit||c.quantity_limit||100)+' entries get the reward';
    if(mode==='random_draw')return 'Random drawing'+(r.draw_at?' · draw '+dateForInput(r.draw_at).replace('T',' '):'');
...
function updateCommandCenter(items){
  var totals=items.reduce(function(acc,c){if(String(c.status||'')==='active')acc.active+=1;acc.contacts+=number(c.contacts_count);acc.issued+=number(c.wallet_issued_count);acc.claimed+=number(c.wallet_claimed_count);acc.redeemed+=number(c.wallet_redeemed_count);acc.failed+=number(c.emails_failed_count);acc.delivered+=number(c.emails_delivered_count);acc.events+=number(c.events_count);if(!c.reward_template_title)acc.noTemplate+=1;return acc;},{active:0,contacts:0,issued:0,claimed:0,redeemed:0,failed:0,delivered:0,events:0,noTemplate:0});
...
function campaignCard(c,filter){
  var url=c.public_url||'';var performance=filter==='performance'?'<small>'+count(c.emails_delivered_count)+' delivered emails · '+count(c.emails_failed_count)+' failed emails · '+count(c.events_count)+' events</small>':'';var rules=ruleSummary(c);var ruleLine=rules?'<small>'+esc(rules)+'</small>':'';
