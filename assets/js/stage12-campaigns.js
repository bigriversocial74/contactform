...
var registry=Array.isArray(window.MicrogifterCampaignTypes)?window.MicrogifterCampaignTypes:[];
var registryMap={};
...
registry.forEach(function(type){if(type&&type.key&&type.default_copy){campaignDefaults[type.key]=type.default_copy;}});
...
function campaignTypeLabel(v){return registryMap[v]&&registryMap[v].label?registryMap[v].label:title(v);}
function campaignTypePath(v){return registryMap[v]&&registryMap[v].public_path?registryMap[v].public_path:({newsletter_signup:'/newsletter-signup.php',contest_giveaway:'/contest.php',qr_reward_drop:'/qr-reward.php',referral_reward:'/referral-reward.php',birthday_vip:'/birthday-vip.php',agent_offer:'/agent-offer.php'}[v]||'/campaign.php');}
...
function campaignRulePreview(){
  ...
  if(registryMap[type]&&registryMap[type].wallet_issue_mode)return campaignTypeLabel(type)+' · '+String(registryMap[type].wallet_issue_mode).replace(/_/g,' ');
...
function updateBuilderAssist(){
  ...
  var publicPath=campaignTypePath(type);
  var checklist='';
