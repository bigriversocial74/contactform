...
function installBanner(root,cfg,m,channels){if(!root)return;var old=qs('[data-module-limit-banner]',root);if(old)return;root.insertAdjacentHTML('afterbegin',banner(cfg,m,channels));}
...
function observeDynamicActions(){var app=qs(ROOT);if(!app||observer)return;observer=new MutationObserver(function(){if(cached)apply(cached);});observer.observe(app,{childList:true,subtree:true});}
async function load(){if(!qs(ROOT)||!MG.get)return;try{var res=await MG.get('/api/account/package-limits.php');cached=res.data||res;apply(cached);observeDynamicActions();}catch(e){}}
