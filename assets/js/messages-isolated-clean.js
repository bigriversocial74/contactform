document.addEventListener('DOMContentLoaded', function () {
  ...
  function csrf() { return window.Microgifter.getCsrfToken ? window.Microgifter.getCsrfToken() : ''; }
  function initials(value) {
    return String(value || 'MG').replace(/^CRM:\s*/i, '').split(/\s+/).filter(Boolean).slice(0, 2).map(function (part) { return part.charAt(0); }).join('').toUpperCase() || 'MG';
  }
  function sourceLabel(item) { return item && (item.source_label || (item.source && item.source.label)) || 'Messages'; }
  function sourceChip(item) { return '<span class="mg-message-source-chip">' + esc(sourceLabel(item)) + '</span>'; }
  function time(value) {
    if (!value) return '';
  ...
  function draftKey(id) { return 'mg:messages:draft:' + id; }
  function getDraft(id) { try { return localStorage.getItem(draftKey(id)) || ''; } catch (e) { return ''; } }
  function setDraft(id, text) { try { text ? localStorage.setItem(draftKey(id), text) : localStorage.removeItem(draftKey(id)); } catch (e) {} }
  ...
  function messagesHtml(messages) {
    ...
    return (messages || []).map(function (message) {
      ...
      lastDay = dateLabel;
      return divider + '<article class="mg-message-row ' + (mine ? 'is-mine' : 'is-theirs') + '">' + (!mine ? '<div class="mg-message-avatar">' + esc(initials(message.sender_name || 'User').slice(0, 1)) + '</div>' : '') + '<div class="mg-message-bubble ' + (mine ? 'is-mine' : 'is-theirs') + '"><strong>' + esc(message.sender_name || (mine ? 'Me' : 'User')) + '</strong><p>' + esc(message.body || '') + '</p><div class="mg-message-meta"><small>' + esc(time(message.created_at)) + '</small><span class="mg-message-status">' + esc(mine ? 'Sent' : 'Received') + '</span></div>' + (message.source && message.source.label ? '<span class="mg-message-bubble-source">' + esc(message.source.label) + '</span>' : '') + '</div></article>';
  ...
  async function openThread(id) {
    ...
    textarea.addEventListener('keydown', function (event) { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendReply(form); } });
    syncComposer(form);
  }
  ...
  if (list) list.addEventListener('click', function (event) { var row = event.target.closest('[data-thread-id]'); if (row) openThread(row.dataset.threadId).catch(console.error); });
  if (detail) detail.addEventListener('submit', function (event) { var form = event.target.closest('[data-thread-reply]'); if (form) { event.preventDefault(); sendReply(form).catch(console.error); } });
  ...
  filters.forEach(function (button) { button.addEventListener('click', function () { state.filter = button.dataset.messageFilter || 'all'; filters.forEach(function (item) { item.classList.toggle('is-active', item === button); }); renderThreads(); }); });
