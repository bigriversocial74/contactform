(() => {
  ...
  const form = root?.querySelector('[data-hgm-integration-form]');
  if (!root || !form) return;

  const statusNode = root.querySelector('[data-hgm-integration-status]');
  ...
  async function configureProgram() {
    ...
    if (!gameId) {
      setStatus('Save the game identity first.', 'error');
      return;
    }
    if (!programId) {
    ...
    setStatus('Applying the connected campaign and reward inventory…');
    try {
      const response = await fetch('/api/merchant/hosted-game-integration.php', {
        method: 'POST',
