...
function mg_merchant_payment_methods_payload(PDO $pdo, int $workspaceId): array
{
    ...
    return ['payment_methods' => ['cash' => ['enabled' => !empty($methods['cash']['enabled']), 'mode' => 'test', 'label' => 'Pay with cash', 'description' => 'Manual cash collection for local testing. No Stripe charge is created.']]];
}

if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET') mg_ok(mg_merchant_payment_methods_payload($pdo, (int)$workspace['id']));
...
try {
    ...
    $state['payment_methods']['cash'] = ['enabled' => $cashEnabled, 'mode' => 'test', 'updated_by_user_id' => (int)$user['id'], 'updated_at' => gmdate('c')];
    $pdo->prepare('UPDATE merchant_payment_readiness SET state_json=?,updated_at=NOW() WHERE id=?')->execute([json_encode($state, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), (int)$row['id']]);
    $pdo->commit();
