...
try{
    ...
    if($draft=$existing->fetch()){
        ...
        $sameRequest=(int)$draft['cart_id']===(int)$cart['id']
            ...
            && (int)$draft['subtotal_cents']===(int)$payload['totals']['subtotal_cents']
            && (int)$draft['discount_cents']===0
            && (int)$draft['tax_cents']===0
            && (int)$draft['platform_fee_cents']===0
            && (int)$draft['total_cents']===(int)$payload['totals']['total_cents']
        ...
        $pdo->commit();
        mg_ok(['checkout_draft_id'=>$draft['public_id'],'status'=>$draft['status'],'expires_at'=>$draft['expires_at'],'reused'=>true]);
    }

    $draftId=mg_public_uuid();$expires=date('Y-m-d H:i:s',time()+1800);
    $pdo->prepare("INSERT INTO checkout_drafts (public_id,cart_id,buyer_user_id,merchant_user_id,currency,subtotal_cents,discount_cents,tax_cents,platform_fee_cents,total_cents,items_json,status,idempotency_key,expires_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,'open',?,?,NOW(),NOW())")
        ->execute([$draftId,(int)$cart['id'],(int)$user['id'],$merchantIds[0],$payload['totals']['currency'],$payload['totals']['subtotal_cents'],0,0,0,$payload['totals']['total_cents'],$itemsJson,$idempotency,$expires]);
    $pdo->commit();
    mg_audit('commerce.checkout_draft_created','checkout_draft',['checkout_draft_id'=>$draftId,'cart_id'=>$cart['public_id']],(int)$user['id']);
    mg_ok(['checkout_draft_id'=>$draftId,'status'=>'open','expires_at'=>$expires,'totals'=>$payload['totals'],'items'=>$payload['items']],'Checkout draft created.',201);
}catch(Throwable $e){
