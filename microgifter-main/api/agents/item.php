...
if ($method === 'PATCH' || $method === 'POST') {
    ...
    try {
        ...
        if (($agent['lifecycle_status'] ?? '') !== 'active') mg_fail('Archived agents must be restored before editing.', 409);
        ...
        $pdo->prepare('UPDATE agents SET name = ?, category = ?, config_json = ?, version_no = version_no + 1, updated_at = NOW() WHERE id = ? AND user_id = ?')->execute([$name, $category, $configJson, (int) $agent['id'], (int) $user['id']]);
...
if ($method === 'DELETE') {
    ...
    try {
        $pdo->beginTransaction();
        $agent = mg_agent_require_owned((int) $user['id'], $id, true);
        $pdo->prepare("UPDATE agents SET lifecycle_status='deleted', runtime_status='paused', deleted_at=NOW(), archived_at=COALESCE(archived_at,NOW()), paused_at=NOW(), version_no=version_no+1, updated_at=NOW() WHERE id=? AND user_id=?")->execute([(int) $agent['id'], (int) $user['id']]);
