...
require __DIR__ . '/signed-quest-enforcement.php';

$config = lqr_config();
$quests = lqr_quests();
$state = lqr_load_state();
$playableQuests = lqr_visible_quests($quests, $state);
...
try {
    ...
    if (is_array($quest) && in_array($action, ['complete_quest','issue_reward'], true)) {
    ...
    if ($action === 'identify') {
        ...
        $signed = lqr_enforce_signed_quest_code($config, $state, $questId, $quest, (string)($_POST['qr_payload'] ?? ''));
        $message = lqr_action_complete_quest($state, $config, $user, $questId, $quest);
        $state['last_scan'] = ['quest_id'=>$questId,'qr_payload'=>(string)($_POST['qr_payload'] ?? ''),'signed_code_required'=>(bool)$signed['required'],'signed_code_verified'=>(bool)$signed['verified'],'signed_code_payload'=>$signed['payload'],'geo_lat'=>(string)($_POST['geo_lat'] ?? ''),'geo_lng'=>(string)($_POST['geo_lng'] ?? ''),'geo_accuracy'=>(string)($_POST['geo_accuracy'] ?? ''),'geo_captured_at'=>(string)($_POST['geo_captured_at'] ?? ''),'captured_at'=>gmdate('c')];
...
$userHistory = lqr_user_quest_history($user, $quests);
?>
...
<div class="lq-shell"><header class="lq-topbar"><div class="lq-brand"><span class="lq-logo">LQ</span><span>Local Quest</span></div><div class="lq-top-actions"><a class="lq-icon-btn" href="wallet.php">◉<span class="lq-badge"><?= number_format($rewardCount) ?></span></a><a class="lq-upgrade" href="developer-starter.php">Developer Starter</a><a class="lq-upgrade" href="wallet.php">Reward Wallet</a><span class="lq-user-pill"><span class="lq-avatar"><?= lqr_h(strtoupper(substr((string)$user['display_name'],0,1))) ?></span><?= lqr_h((string)$user['display_name']) ?></span></div></header>
...
<div class="lq-kpis"><div class="lq-kpi"><span>Available quests</span><strong><?= number_format(count($playableQuests)) ?></strong></div><div class="lq-kpi"><span>Completed</span><strong><?= number_format($completedCount) ?></strong></div><div class="lq-kpi"><span>Rewards</span><strong><?= number_format($rewardCount) ?></strong></div><div class="lq-kpi"><span>Claims</span><strong><?= number_format($claimCount) ?></strong></div></div>
<div class="lq-layout"><section class="lq-card"><div class="lq-card-head"><div><h2>Quest tasks</h2><p>Scan a QR code or paste a venue/prize code, capture geolocation, then complete the task.</p></div><span class="lq-pill <?= $isLinked ? 'green' : 'amber' ?>"><?= $isLinked ? 'Microgifter linked' : 'Link required' ?></span></div><div class="lq-stack"><?php if (!$playableQuests): ?><div class="lq-item"><div><h3>No playable quests right now</h3><div class="lq-meta">The admin can activate, schedule, or raise caps from Quest Controls.</div></div></div><?php endif; ?><?php foreach ($playableQuests as $questId => $quest): ?><?php $completed=!empty($user['completed_quests'][$questId]); $reward=is_array($user['rewards'][$questId]??null)?$user['rewards'][$questId]:[]; [$canIssue,$issueReason]=lqr_can_issue_reward($user,(string)$questId,$quest,$config); $controls=lqr_quest_controls($quest); $requiresSigned=!empty($controls['requires_signed_code']); ?><article class="lq-item"><div><h3><?= lqr_h((string)$quest['title']) ?></h3><div class="lq-meta"><?= lqr_h((string)$quest['description']) ?><br><strong><?= lqr_h((string)$quest['reward_label']) ?></strong> · <?= lqr_h((string)$quest['difficulty']) ?><br>Sponsor <?= lqr_h((string)($controls['sponsor'] ?: ($quest['merchant'] ?? ''))) ?><br>Program <code><?= lqr_h(lqr_quest_program_id($quest,$config) ?: 'not configured') ?></code> · Template <code><?= lqr_h(lqr_quest_template_id($quest,$config) ?: 'not configured') ?></code></div><div style="margin-top:10px"><span class="lq-pill <?= $completed ? 'green' : 'amber' ?>"><?= $completed ? 'Completed' : 'Open' ?></span><?php if(!empty($controls['featured'])): ?> <span class="lq-pill pink">Featured</span><?php endif; ?><?php if($requiresSigned): ?> <span class="lq-pill amber">Signed QR required</span><?php endif; ?><?php if($reward): ?> <span class="lq-pill pink">Reward <?= lqr_h((string)$reward['status']) ?></span><?php endif; ?></div></div><form method="post" data-lq-geo-box data-lq-qr-box style="min-width:280px"><input type="hidden" name="quest_id" value="<?= lqr_h((string)$questId) ?>"><input type="hidden" name="geo_lat"><input type="hidden" name="geo_lng"><input type="hidden" name="geo_accuracy"><input type="hidden" name="geo_captured_at"><input type="hidden" name="qr_payload"><input class="lq-input" data-lq-qr-manual placeholder="<?= $requiresSigned ? 'Signed venue QR required' : 'Venue QR / task code' ?>"><div class="lq-actions"><button class="lq-btn soft" data-lq-start-qr>Scan QR</button><button class="lq-btn soft" data-lq-capture-geo>Geo</button></div><video class="lq-qr-video" playsinline></video><div class="lq-qr-result" data-lq-qr-result></div><div class="lq-qr-result" data-lq-geo-result></div><div class="lq-actions"><button class="lq-btn green" name="action" value="complete_quest">Complete</button><button class="lq-btn amber" name="action" value="issue_reward" <?= $canIssue ? '' : 'disabled' ?>>Issue reward</button><button class="lq-btn dark" name="action" value="check_status" <?= $reward ? '' : 'disabled' ?>>Status</button></div><small><?= lqr_h($reward ? 'Reward: '.(string)$reward['reward_id'] : $issueReason) ?></small></form></article><?php endforeach; ?></div></section><aside class="lq-side-panel"><section class="lq-panel"><h2>Quick actions</h2><a class="lq-link-row" href="developer-starter.php">Open Developer Starter <span>→</span></a><form method="post"><button class="lq-link-row" name="action" value="list_programs">List Microgifter programs <span>→</span></button></form><?php if(!$isLinked): ?><form method="post"><button class="lq-link-row" name="action" value="start_account_link">Connect Microgifter <span>→</span></button></form><?php if(!empty($config['allow_sandbox_shortcut'])): ?><form method="post"><button class="lq-link-row" name="action" value="sandbox_link">Developer sandbox <span>→</span></button></form><?php endif; ?><?php endif; ?><a class="lq-link-row" href="wallet.php">Open wallet <span>→</span></a><a class="lq-link-row" href="webhook.php">Webhook status <span>→</span></a><a class="lq-link-row" href="admin-quest-controls.php">Quest controls <span>→</span></a></section><section class="lq-panel"><h2>Participant</h2><form method="post"><label class="lq-label">Display name</label><input class="lq-input" name="display_name" value="<?= lqr_h((string)$user['display_name']) ?>"><button class="lq-btn primary" name="action" value="identify" style="width:100%;margin-top:12px">Save profile</button></form><div class="lq-row"><span>Email</span><strong><?= lqr_h((string)$user['email']) ?></strong></div><div class="lq-row"><span>External ID</span><strong><?= lqr_h((string)$user['external_user_id']) ?></strong></div><div class="lq-row"><span>Microgifter</span><strong><?= lqr_h($isLinked ? (string)$user['linked_account_id'] : 'Not connected') ?></strong></div></section><section class="lq-panel"><h2>Your history</h2><?php if(!$userHistory): ?><p>No quest interactions yet.</p><?php endif; ?><?php foreach(array_slice($userHistory,0,6) as $h): ?><div class="lq-row"><span><?= lqr_h($h['quest_title']) ?></span><strong><?= lqr_h($h['claim_status'] ?: ($h['reward_status'] ?: 'completed')) ?></strong></div><?php endforeach; ?></section></aside></div>
