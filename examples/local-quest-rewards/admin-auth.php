...
function lqr_admin_users(array &$state, array $config): array
{
    ...
    if (empty($state['admin_users']) && !empty($settings['bootstrap_enabled'])) {
        ...
        $hash = trim((string)$settings['password_hash']);
        $plain = (string)$settings['password'];
        if ($username !== '' && ($hash !== '' || $plain !== '')) {
            if ($hash === '') $hash = password_hash($plain, PASSWORD_DEFAULT);
...
function lqr_admin_login_guard(): void
{
    $lockedUntil = (int)($_SESSION['lqr_admin_login_locked_until'] ?? 0);
    if ($lockedUntil > time()) {
        throw new RuntimeException('Too many sign-in attempts. Try again in ' . max(1, $lockedUntil - time()) . ' seconds.');
    }
}

function lqr_admin_note_login_failure(): void
{
    $attempts = (int)($_SESSION['lqr_admin_login_attempts'] ?? 0) + 1;
    $_SESSION['lqr_admin_login_attempts'] = $attempts;
    if ($attempts >= 5) {
        $_SESSION['lqr_admin_login_locked_until'] = time() + 300;
        $_SESSION['lqr_admin_login_attempts'] = 0;
    }
}

function lqr_admin_clear_login_failures(): void
{
    unset($_SESSION['lqr_admin_login_attempts'], $_SESSION['lqr_admin_login_locked_until']);
}

function lqr_admin_login(array &$state, array $config, string $login, string $password): array
{
    lqr_admin_login_guard();
    ...
    if (!$admin || ($admin['status'] ?? '') !== 'active' || !password_verify($password, (string)$admin['password_hash'])) {
        lqr_admin_note_login_failure();
        throw new RuntimeException('Invalid admin credentials.');
    }

    lqr_admin_clear_login_failures();
...
function lqr_admin_require(array &$state, array $config): array
{
    ...
    if (!lqr_admin_is_authed() || !$admin || ($admin['status'] ?? '') !== 'active') {
...
function lqr_admin_create_user(array &$state, array $config, string $username, string $email, string $password, string $displayName = 'Admin', string $role = 'admin'): array
{
    ...
    $allowedRoles = ['owner', 'admin', 'quest_manager', 'support', 'sponsor_viewer'];
    ...
    lqr_add_event($state, 'admin.user_created', 'Admin user created.', ['admin_id' => $publicId, 'username' => $username, 'role_key' => $role]);
    return $admin;
}
