...
header("Content-Security-Policy: default-src 'self' data: blob:; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; frame-src 'self'; connect-src 'self'; frame-ancestors 'self'");
?><!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<body>
<?php if (!$game || !$readiness || !$readiness['publish_ready']): ?>
<main class="hg-shell-unavailable">
  <article>
    ...
    <p><?= $game ? 'This hosted game is paused or its release, reward integration, or isolated database requires attention.' : 'The hosted game URL is not active.' ?></p>
    <a href="/">Return to Microgifter</a>
  </article>
</main>
<?php else: ?>
<div class="hg-shell" data-hosted-game-shell>
  <header class="hg-shell-bar">
    <div class="hg-shell-brand">
      ...
      <div class="hg-shell-title"><strong><?= mg_e((string)$game['name']) ?></strong><span>Hosted game · <?= mg_e((string)$game['slug']) ?></span></div>
    </div>
    <div class="hg-shell-actions">
      ...
      <button class="hg-shell-button" type="button" data-hg-fullscreen>Fullscreen</button>
    </div>
  </header>
  <main class="hg-shell-stage">
    <iframe
      ...
      src="/api/hosted-games/document.php?slug=<?= rawurlencode((string)$game['slug']) ?>"
      sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-popups allow-downloads"
    ...
    <div class="hg-shell-overlay" data-hg-shell-overlay>
      <article class="hg-shell-overlay-card">
        ...
        <p data-hg-shell-text>Checking your Microgifter session and game connection.</p>
...
<script src="/assets/js/hosted-game-shell.js?v=1.0.0" defer></script>
<?php endif; ?>
</body>
</html>
