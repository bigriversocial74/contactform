...
$activeSidebarKey = match ($currentSidebarScript) {
    'inbox.php' => 'inbox', 'feed.php' => 'feed', 'loyalty-cards.php' => 'loyalty-cards', 'lists.php' => 'lists', 'saves.php' => 'saves',
    'merchant-agent-chat.php' => 'merchant-agent', 'agent.php' => ((string) ($agent_personal_view ?? 'home')) === 'design' ? 'design' : 'agent',
    'design-studio.php' => 'design', 'design-calendar.php' => 'calendar', default => '',
};
...
<aside class="mg-app-sidebar mg-universal-sidebar mg-utility-sidebar is-text-sidebar mg-personal-chat-sidebar" data-app-sidebar data-sidebar-variant="utility" data-personal-agent-chat-sidebar data-agent-sidebar-mode="<?= mg_e($agentSidebarMode) ?>">
  ...
  <?php if ($user): ?>
    <nav class="mg-personal-chat-actions" aria-label="Customer and Agent navigation">
      ...
      <?php elseif ($isPersonalAgentMode && $hasPersonalAgentAccess): ?>
        <div class="mg-personal-chat-row mg-agent-nav-row<?= $selectedSidebarAgentId === '' && $activeSidebarKey === 'agent' ? ' is-active' : '' ?>" data-sidebar-agent-row="default">
          <a class="mg-personal-chat-open" href="/agent.php" data-sidebar-agent-id="default"><span class="mg-agent-nav-icon" aria-hidden="true">✦</span><span class="mg-agent-nav-copy"><strong>Agent</strong><small>Default chat agent</small></span></a>
        </div>
    ...
    <?php if ($isMerchantAgentMode && $hasMerchantAgentAccess): ?><div class="mg-personal-chat-history" data-merchant-agent-thread-groups aria-live="polite"><div class="mg-personal-chat-loading">Loading merchant chats…</div></div>
    <?php elseif ($hasPersonalAgentAccess): ?><div class="mg-personal-chat-history" data-personal-agent-thread-groups aria-live="polite"><div class="mg-personal-chat-loading">Loading chats…</div></div>
    <?php else: ?><div class="mg-personal-chat-history" aria-live="polite"><div class="mg-personal-chat-empty">Choose a paid customer or merchant package to unlock Agent chats.</div></div><?php endif; ?>
    <?php if (!$suppressAgentSidebarFooter): ?>
      <footer class="mg-personal-chat-sidebar-footer" data-agent-sidebar-footer-tools>
        ...
        <nav class="mg-agent-footer-mode-switch" aria-label="Agent mode" data-agent-footer-mode-switch><a class="mg-agent-footer-mode<?= $isPersonalAgentMode ? ' is-active' : '' ?><?= !$hasPersonalAgentAccess ? ' is-locked' : '' ?>" href="<?= mg_e($personalAgentHref) ?>" data-agent-mode-link="personal"<?= $isPersonalAgentMode ? ' aria-current="page"' : '' ?>><span aria-hidden="true">P</span><strong>Personal</strong></a><a class="mg-agent-footer-mode<?= $isMerchantAgentMode ? ' is-active' : '' ?><?= !$hasMerchantAgentAccess ? ' is-locked' : '' ?>" href="<?= mg_e($merchantAgentHref) ?>" data-agent-mode-link="merchant"<?= $isMerchantAgentMode ? ' aria-current="page"' : '' ?>><span aria-hidden="true">M</span><strong>Merchant</strong></a></nav>
      </footer>
    <?php endif; ?>
  <?php else: ?><div class="mg-personal-chat-empty-sidebar"><strong><?= $isMerchantAgentMode ? 'Merchant Agent' : 'Personal Agent' ?></strong><p>Sign in to create and manage private chats.</p></div><?php endif; ?>
</aside>
<?php if ($user && !$suppressAgentSidebarTools): ?>
  <div class="mg-agent-sidebar-tools-modal" id="agent-sidebar-tools-modal" data-agent-sidebar-tools-modal data-agent-tools-mode="<?= mg_e($quickActionMode) ?>" data-agent-tools-entitled="<?= $quickActionsEntitled ? 'true' : 'false' ?>" data-agent-subscriptions-url="/account-subscriptions.php?agent=<?= mg_e($quickActionMode) ?>" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="agent-sidebar-tools-title"><div class="mg-agent-sidebar-tools-backdrop" data-agent-suggestions-close></div><section class="mg-agent-sidebar-tools-dialog" role="document"><header class="mg-agent-sidebar-tools-head"><div><span><?= mg_e((string) $quickActions['title']) ?></span><h2 id="agent-sidebar-tools-title">Agent shortcuts</h2><p><?= mg_e((string) $quickActions['description']) ?></p></div><button type="button" data-agent-suggestions-close aria-label="Close Agent shortcuts">×</button></header><div class="mg-agent-sidebar-tools-tabs" role="tablist" aria-label="Agent shortcuts sections"><button type="button" role="tab" id="agent-suggestions-tab" aria-selected="true" aria-controls="agent-suggestions-panel" data-agent-tools-tab="suggestions">Suggestions</button><button type="button" role="tab" id="agent-keywords-tab" aria-selected="false" aria-controls="agent-keywords-panel" data-agent-tools-tab="keywords">Quick keywords</button></div><div class="mg-agent-sidebar-tools-body"><section id="agent-suggestions-panel" role="tabpanel" aria-labelledby="agent-suggestions-tab" data-agent-tools-panel="suggestions"><div class="mg-agent-suggestion-grid"><?php foreach ((array) $quickActions['suggestions'] as $suggestion): ?><button class="mg-agent-suggestion-card" type="button" data-agent-suggestion-prompt="<?= mg_e((string) $suggestion['prompt']) ?>" data-agent-target-mode="<?= mg_e($quickActionMode) ?>"><span class="mg-agent-suggestion-icon" aria-hidden="true"><?= mg_e((string) $suggestion['icon']) ?></span><span><strong><?= mg_e((string) $suggestion['label']) ?></strong><small><?= mg_e((string) $suggestion['detail']) ?></small></span><span class="mg-agent-suggestion-run" aria-hidden="true">→</span></button><?php endforeach; ?></div></section><section id="agent-keywords-panel" role="tabpanel" aria-labelledby="agent-keywords-tab" data-agent-tools-panel="keywords" hidden><p class="mg-agent-keywords-intro">Select a keyword to place its current command or example in the chat box. Navigation keywords open their page directly.</p><div class="mg-agent-keyword-groups"><?php foreach ((array) $quickActions['keyword_groups'] as $group): ?><section class="mg-agent-keyword-group"><h3><?= mg_e((string) $group['label']) ?></h3><div class="mg-agent-keyword-list"><?php foreach ((array) $group['items'] as $item): ?><button type="button" data-agent-keyword-prompt="<?= mg_e((string) ($item['prompt'] ?? '')) ?>" <?php if (!empty($item['href'])): ?>data-agent-keyword-href="<?= mg_e((string) $item['href']) ?>"<?php endif; ?> data-agent-target-mode="<?= mg_e($quickActionMode) ?>"><strong><?= mg_e((string) $item['keyword']) ?></strong><small><?= mg_e((string) $item['detail']) ?></small></button><?php endforeach; ?></div></section><?php endforeach; ?></div></section></div></section></div>
