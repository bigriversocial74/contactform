...
function mg_investment_save_documents_audited_v2(PDO $pdo,array $actor,array $input): array
{
    ...
    foreach(array_slice(is_array($input['items']??null)?$input['items']:[],0,100) as $item){
        ...
        $q->execute([$publicId,(int)$workspace['id']]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        if(!in_array((string)$current['status'],['approved','published'],true))throw new MgInvestmentException('Approve the exact investment document version before publishing it.',409);
        $changed=(string)$current['title']!==$title||(string)($current['external_url']??'')!==(string)$url||(string)$current['visibility']!==$visibility;
        if($changed)throw new MgInvestmentException('A changed investment document must be saved as an approved version before it can be published.',409);
    }
...
function mg_investment_governance_save_tax_document_audited_v3(PDO $pdo,array $actor,array $input): array
{
    ...
    if($status==='published'){
        ...
        $q=$pdo->prepare('SELECT td.*,v.external_url,v.external_reference,v.status AS version_status FROM investment_tax_documents td LEFT JOIN investment_tax_document_versions v ON v.tax_document_id=td.id AND v.version_number=td.current_version_number WHERE td.public_id=? AND td.round_id=? LIMIT 1');
        $q->execute([mg_investment_text($publicId,36,36,'Tax document identifier'),(int)$round['id']]);$current=$q->fetch(PDO::FETCH_ASSOC);
        if(!$current)throw new MgInvestmentException('Tax document not found.',404);
        if((string)$current['status']!=='approved'||(string)($current['version_status']??'')!=='approved')throw new MgInvestmentException('Approve the exact tax-document version before publishing it.',409);
        ...
        $newTitle=mg_investment_text($input['title']??'',220,2,'Document title');
        $newProvider=mg_investment_audit_nullable_text($input['external_provider']??'',180);
        $newType=(string)($input['document_type']??'other');
        $newYear=(int)($input['reporting_year']??date('Y'));
        $newInvestor=(int)($input['investor_user_id']??0);
        foreach([
            'external_url'=>$newUrl,'external_reference'=>$newReference,'title'=>$newTitle,'external_provider'=>$newProvider,
            'document_type'=>$newType,'reporting_year'=>$newYear,'investor_user_id'=>$newInvestor,
        ] as $field=>$value){
            if(!mg_investment_audit_same_value($current[$field]??null,$value))throw new MgInvestmentException('The published tax document must exactly match its approved version and metadata.',409);
        }
    }
