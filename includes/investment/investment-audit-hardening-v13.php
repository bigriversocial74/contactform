...
function mg_investment_audit_require_exact(array $current,array $proposed,string $message): void
{
    foreach($proposed as $field=>$value){
        if(!mg_investment_audit_same_value($current[$field]??null,$value))throw new MgInvestmentException($message,409);
    }
}

function mg_investment_governance_dashboard_audited_v2(PDO $pdo,array $filters=[]): array
{
    $data=mg_investment_governance_dashboard_audited($pdo,$filters);
    $roundPublicId=trim((string)($filters['round_id']??''));
    $where='';$params=[];
    if($roundPublicId!==''){
        $round=mg_investment_governance_round($pdo,$roundPublicId);
        $where=' WHERE (m.round_id=? OR m.round_id IS NULL)';
        $params=[(int)$round['id']];
    }
    $q=$pdo->prepare('SELECT d.*,m.public_id AS meeting_public_id,m.title AS meeting_title FROM investment_board_packet_documents d INNER JOIN investment_board_meetings m ON m.id=d.meeting_id'.$where.' ORDER BY m.starts_at DESC,d.title,d.version_number DESC');
    $q->execute($params);
    $data['packet_documents']=$q->fetchAll(PDO::FETCH_ASSOC);
    return $data;
}
...
function mg_investment_dataroom_save_document_audited_v2(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['status']??'draft')==='published'){
        ...
        mg_investment_audit_require_exact($current,[
            ...
            'investor_description'=>mg_investment_audit_nullable_long_text($input['investor_description']??'',6000),
            ...
            'download_allowed'=>mg_investment_bool($input['download_allowed']??true)?1:0,
...
function mg_investment_qa_save_audited_v2(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['status']??'draft')==='published'){
        ...
        $q=$pdo->prepare('SELECT * FROM investment_qa_entries WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        $roundId=null;if(!empty($input['round_id']))$roundId=(int)mg_investment_diligence_round($pdo,mg_investment_text($input['round_id'],36,36,'Round identifier'))['id'];
        mg_investment_audit_require_exact($current,[
            'round_id'=>$roundId,'category'=>mg_investment_text($input['category']??'general',80,2,'Category'),
...
function mg_investment_communication_save_audited_v3(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['status']??'draft')==='published'){
        ...
        $q=$pdo->prepare('SELECT * FROM investor_communications WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        $roundId=null;if(!empty($input['round_id']))$roundId=(int)mg_investment_diligence_round($pdo,mg_investment_text($input['round_id'],36,36,'Round identifier'))['id'];
        mg_investment_audit_require_exact($current,[
            'round_id'=>$roundId,'communication_type'=>(string)($input['communication_type']??'round_update'),
            'audience_type'=>(string)($input['audience_type']??'approved_investors'),'subject'=>mg_investment_text($input['subject']??'',220,2,'Subject'),
            'body'=>mg_investment_long_text($input['body']??'',30000,2,'Communication body'),'requires_legal_review'=>mg_investment_bool($input['requires_legal_review']??false)?1:0,
...
function mg_investment_governance_save_meeting_audited_v2(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['summary_status']??'draft')==='published'){
        ...
        $q=$pdo->prepare('SELECT * FROM investment_board_meetings WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        mg_investment_audit_require_exact($current,[
            'round_id'=>$round?(int)$round['id']:null,'meeting_type'=>(string)($input['meeting_type']??'regular_board'),
            'title'=>mg_investment_text($input['title']??'',220,2,'Meeting title'),'starts_at'=>mg_investment_governance_datetime($input['starts_at']??'',true,'Meeting start'),
            'ends_at'=>mg_investment_governance_datetime($input['ends_at']??''),'location'=>mg_investment_audit_nullable_text($input['location']??'',300),
            'meeting_url'=>mg_investment_url($input['meeting_url']??''),'confidentiality'=>(string)($input['confidentiality']??'board_only'),
...
{
    if((string)($input['status']??'draft')!=='published')return mg_investment_governance_save_packet_document_audited($pdo,$actor,$input);
    ...
    return mg_investment_audit_transaction($pdo,function()use($pdo,$actor,$input,$publicId,$meeting):array{
        $q=$pdo->prepare('SELECT * FROM investment_board_packet_documents WHERE public_id=? AND meeting_id=? LIMIT 1 FOR UPDATE');
        $q->execute([$publicId,(int)$meeting['id']]);$current=$q->fetch(PDO::FETCH_ASSOC);
        if(!$current)throw new MgInvestmentException('Board packet document not found.',404);
        if((string)$current['status']==='published')return mg_investment_governance_dashboard_audited_v2($pdo,$input);
        ...
        if((string)($input['confidentiality']??'board_only')!=='funded_investors')throw new MgInvestmentException('Only funded-investor packet documents may be published to the Investor Portal.',409);
        mg_investment_audit_require_exact($current,[
            'document_type'=>(string)($input['document_type']??'supporting'),'title'=>mg_investment_text($input['title']??'',220,2,'Document title'),
            'external_url'=>mg_investment_url($input['external_url']??'',true),'confidentiality'=>(string)($input['confidentiality']??'board_only'),
            'counsel_status'=>(string)($input['counsel_status']??'not_started'),
        ],'The published request must exactly match the approved board packet document. Save a corrected approved version before publishing.');
        $pdo->prepare('UPDATE investment_board_packet_documents SET status="superseded" WHERE meeting_id=? AND title=? AND status="published" AND id<>?')->execute([(int)$meeting['id'],(string)$current['title'],(int)$current['id']]);
        $pdo->prepare('UPDATE investment_board_packet_documents SET status="published",published_at=NOW() WHERE id=?')->execute([(int)$current['id']]);
        mg_audit('investment_board_packet_document_published','investment_board_meeting',['meeting_id'=>$meeting['public_id'],'document_id'=>$publicId,'version'=>(int)$current['version_number']],(int)$actor['id']);
        return mg_investment_governance_dashboard_audited_v2($pdo,$input);
    });
}
...
function mg_investment_governance_save_consent_audited_v2(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['status']??'draft')==='executed'){
        ...
        $q=$pdo->prepare('SELECT * FROM investment_written_consents WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        $round=mg_investment_governance_round($pdo,$input['round_id']??'',false);$batchId=null;
        if(!empty($input['batch_id'])){$b=$pdo->prepare('SELECT id FROM investment_closing_batches WHERE public_id=? LIMIT 1');$b->execute([mg_investment_text($input['batch_id'],36,36,'Batch identifier')]);$batchId=(int)$b->fetchColumn()?:null;}
        mg_investment_audit_require_exact($current,[
            'round_id'=>$round?(int)$round['id']:null,'closing_batch_id'=>$batchId,'consent_type'=>(string)($input['consent_type']??'board'),
            'title'=>mg_investment_text($input['title']??'',220,2,'Consent title'),'resolution_text'=>mg_investment_long_text($input['resolution_text']??'',50000,20,'Resolution text'),
            'approval_group'=>mg_investment_text($input['approval_group']??'',180,2,'Approval group'),'approval_threshold'=>mg_investment_audit_nullable_text($input['approval_threshold']??'',180),
            'response_due_at'=>mg_investment_governance_datetime($input['response_due_at']??''),'counsel_status'=>(string)($input['counsel_status']??'not_started'),
...
function mg_investment_governance_save_notice_audited_v4(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['status']??'draft')==='published'){
        ...
        $q=$pdo->prepare('SELECT * FROM investment_material_notices WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
...
{
    $status=(string)($input['status']??'draft');$visible=mg_investment_bool($input['investor_visible']??false);
    if($status==='active'||$visible){
        mg_investment_require_permission($actor,'admin.investment.governance.publish');
        $publicId=mg_investment_text($input['right_id']??'',36,36,'Right identifier');
        $q=$pdo->prepare('SELECT * FROM investment_investor_rights WHERE public_id=? LIMIT 1');$q->execute([$publicId]);$current=$q->fetch(PDO::FETCH_ASSOC);
        ...
        mg_investment_audit_require_exact($current,[
            'round_id'=>(int)$round['id'],'investor_user_id'=>(int)($input['investor_user_id']??0),'right_type'=>(string)($input['right_type']??'information'),
            'title'=>mg_investment_text($input['title']??'',220,2,'Right title'),'description'=>mg_investment_audit_nullable_long_text($input['description']??'',10000),
            'source_document_reference'=>mg_investment_text($input['source_document_reference']??'',220,2,'Source document reference'),'source_document_url'=>mg_investment_url($input['source_document_url']??''),
            'cadence'=>(string)($input['cadence']??'none'),'custom_cadence'=>mg_investment_audit_nullable_text($input['custom_cadence']??'',180),
            'starts_at'=>mg_investment_date($input['starts_at']??null),'expires_at'=>mg_investment_date($input['expires_at']??null),'counsel_status'=>(string)($input['counsel_status']??'not_started'),
...
function mg_investment_governance_save_obligation_audited_v2(PDO $pdo,array $actor,array $input): array
{
    if((string)($input['portal_publication_status']??'not_required')==='published'){
        ...
        $q=$pdo->prepare('SELECT * FROM investment_reporting_obligations WHERE public_id=? AND round_id=? LIMIT 1');$q->execute([$publicId,(int)$round['id']]);$current=$q->fetch(PDO::FETCH_ASSOC);
        if(!$current)throw new MgInvestmentException('Reporting obligation not found.',404);
        if(!in_array((string)$current['portal_publication_status'],['approved','published'],true))throw new MgInvestmentException('Approve the exact reporting obligation before publishing it.',409);
        mg_investment_audit_require_exact($current,[
            'investor_user_id'=>(int)($input['investor_user_id']??0)?:null,'obligation_type'=>(string)($input['obligation_type']??'quarterly_report'),
            'title'=>mg_investment_text($input['title']??'',220,2,'Obligation title'),'reporting_period'=>mg_investment_audit_nullable_text($input['reporting_period']??'',120),
            'due_at'=>mg_investment_governance_datetime($input['due_at']??'',true,'Obligation due date'),'recipient_scope'=>(string)($input['recipient_scope']??'funded_investors'),
            'completion_reference'=>mg_investment_audit_nullable_text($input['completion_reference']??'',220),
        ],'The published request must exactly match the approved reporting obligation. Save changes as approved before publishing.');
    }
