...
function mg_investment_engagement_calculate(PDO $pdo,int $investorUserId,?int $roundId=null): array
{
    ...
    $events=$pdo->query('SELECT event_type,COUNT(*) AS total,COUNT(DISTINCT subject_id) AS unique_subjects,MAX(created_at) AS latest FROM investment_portal_events WHERE investor_user_id='.(int)$investorUserId.$roundClause.' GROUP BY event_type')->fetchAll(PDO::FETCH_ASSOC);
