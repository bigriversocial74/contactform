...
function mg_investment_portal_data_v2(PDO $pdo,array $user): array
{
    ...
    foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row){
        ...
        if($sections['documents']){$q=$pdo->prepare('SELECT public_id,title,document_type,status,external_url,visibility FROM investment_documents WHERE workspace_id=? AND status="published" AND visibility IN ("approved_investors","selected_investors","funded_investors","public_summary") ORDER BY title');$q->execute([(int)$row['workspace_id']]);$documents=$q->fetchAll(PDO::FETCH_ASSOC);}
