...
function mg_agent_analytics_rows(PDO $pdo, int $merchantId, array $input = []): array
{
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $rows[] = [
            ...
            'bucket_date' => (strtotime((string)($row['created_at'] ?? '')) ? date('Y-m-d', strtotime((string)$row['created_at'])) : 'unknown'),
            'context' => $ctx,
        ];
    }
    return $rows;
...
function mg_agent_analytics_summary(array $rows): array
{
    ...
    $executionOutcomes = $s['execution_completed'] + $s['execution_failed'] + $s['execution_skipped'];
    $s['approval_rate'] = mg_agent_analytics_rate($s['approvals'], $decisions);
    $s['execution_completion_rate'] = mg_agent_analytics_rate($s['execution_completed'], max($s['execution_started'], $executionOutcomes));
    $s['failed_skipped_rate'] = mg_agent_analytics_rate($s['execution_failed'] + $s['execution_skipped'], $executionOutcomes);
...
function mg_agent_analytics_grouped(array $rows, string $key, string $labelKey = ''): array
{
    ...
    foreach ($rows as $row) {
        ...
        $groups[$id]['id'] = $id;
        $groups[$id]['label'] = $labelKey !== '' ? (string)($row[$labelKey] ?? $id) : ucwords(str_replace(['_', '.', '-'], ' ', $id));
    }
    foreach ($groups as $id => $group) {
        $summary = mg_agent_analytics_summary($group['events'] ?? []);
        $summary['id'] = $id;
        $summary['label'] = trim((string)($group['label'] ?? '')) ?: 'Unknown';
        $groups[$id] = $summary;
    }
...
function mg_agent_analytics_customer_breakdown(array $rows): array
{
    $groups = [];
    foreach ($rows as $row) {
        $id = mg_agent_analytics_customer_key($row) ?: 'unknown';
        $groups[$id]['events'][] = $row;
        $groups[$id]['label'] = (string)($row['customer_name'] ?: $row['customer_email'] ?: 'Unknown customer');
    }
    foreach ($groups as $id => $group) {
        $summary = mg_agent_analytics_summary($group['events'] ?? []);
        $summary['id'] = $id;
        $summary['label'] = $group['label'];
        $summary['customer_url'] = str_starts_with($id, 'email:') ? ('/merchant-customer.php?email=' . rawurlencode(substr($id, 6)) . '&tab=timeline') : '/merchant-customer.php?tab=timeline';
        $groups[$id] = $summary;
    }
    usort($groups, static fn($a, $b) => ((int)$b['events_total']) <=> ((int)$a['events_total']));
    return array_slice($groups, 0, 20);
