...
function mg_ai_merchant_find_anthropic_model(PDO $pdo, ?int $agentId = null): array
{
    if ($agentId !== null) {
        $stmt = $pdo->prepare(
            "SELECT m.*, p.provider_key, p.display_name provider_name, p.env_var_name, p.enabled provider_enabled
             FROM agent_ai_settings aas
             INNER JOIN ai_models m ON m.id = aas.model_id
             INNER JOIN ai_providers p ON p.id = aas.provider_id
             WHERE aas.agent_id = ? AND p.provider_key = 'anthropic' AND p.enabled = 1 AND m.enabled = 1
             LIMIT 1"
        );
    ...
    $stmt = $pdo->prepare(
        "SELECT m.*, p.provider_key, p.display_name provider_name, p.env_var_name, p.enabled provider_enabled
         ...
         WHERE p.provider_key = 'anthropic'
           AND p.enabled = 1
           AND m.enabled = 1
           AND m.model_key IN ('claude-sonnet-4-6','claude-3-5-sonnet-latest')
         ORDER BY (m.model_key = 'claude-sonnet-4-6') DESC, m.is_default DESC, m.sort_order ASC
         LIMIT 1"
    );
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!is_array($row)) {
        mg_fail('Claude Sonnet is not enabled in the AI model catalog.', 503);
    }
    if (!mg_ai_env_configured((string) $row['env_var_name'])) {
        mg_fail('Anthropic is not configured on the server. Set MG_ANTHROPIC_API_KEY.', 503);
    }
    return $row;
}
...
function mg_ai_merchant_provider(PDO $pdo, int $providerId): array
{
    ...
    if (!is_array($provider) || !(bool) $provider['enabled']) {
        mg_fail('AI provider is not enabled.', 503);
    }
    return $provider;
}
...
- Recommend practical actions for rewards, campaigns, CRM follow-ups, claims operations, reports, analytics, onboarding, and developer API opportunities.
- Return only valid JSON. No markdown. No prose outside JSON.

Hard safety boundaries:
- You do not execute actions.
- You do not issue rewards, redeem claims, move money, change payment state, send customer messages, alter wallet ownership, or change PPPM lifecycle state.
- You may recommend drafts, tasks, alerts, reports, or review items.
- High and critical risk recommendations must require approval.
- Every recommendation must be merchant-owned, explainable, and reversible where possible.
  ...
  "recommendations": [
    {
      ...
      "target_type": "campaign|reward_template|crm_contact|claim|report|location|developer_api|workspace|alert|none",
...
{
    return json_encode([
        'instruction' => 'Create a supervised merchant automation plan using only the allowed action keys.',
        'allowed_action_keys' => $allowedActions,
...
function mg_ai_merchant_normalize_plan_payload(array $payload): array
{
    ...
    if ($summary === '') {
        $summary = 'Claude created a supervised merchant automation plan.';
    }
    ...
    foreach ((array) ($payload['recommendations'] ?? []) as $item) {
        ...
        $items[] = [
            ...
            'title' => $title,
            'target_type' => mg_ai_merchant_clean_string($item['target_type'] ?? 'none', 120) ?: 'none',
            'target_reference' => mg_ai_merchant_clean_string($item['target_reference'] ?? '', 190) ?: null,
            'risk_level' => $risk,
            'requires_approval' => true,
            'confidence' => $confidence,
            'reason' => mg_ai_merchant_clean_string($item['reason'] ?? '', 4000),
    ...
    if ($items === []) {
        $items[] = [
            'action_key' => 'create_report_snapshot',
            'title' => 'Generate a merchant performance report',
            'target_type' => 'report',
            'target_reference' => null,
            'risk_level' => 'low',
            'requires_approval' => true,
            'confidence' => 0.5,
            'reason' => 'No specific safe action was returned, so the fallback is a reviewable report snapshot.',
            'suggested_payload' => ['scope' => 'merchant_summary'],
        ];
    }
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO ai_usage_events
             (provider_id,model_id,user_id,agent_id,request_status,request_units,input_tokens,output_tokens,metadata_json,created_at)
             VALUES (?,?,?,?,?,0,?,?,?,NOW())"
        );
        $stmt->execute([
            $providerId,
        ...
        if (function_exists('mg_security_log')) {
            mg_security_log('warning', 'ai.usage_completion_log_failed', 'Unable to log AI usage completion.', [
...
function mg_ai_merchant_public_plan(array $plan, array $items): array
{
    return [
        ...
        'merchant_goal' => (string) ($plan['merchant_goal'] ?? ''),
        'status' => (string) $plan['status'],
        'priority' => (string) $plan['priority'],
        'summary' => (string) ($plan['summary'] ?? ''),
        'model' => [
            'provider' => (string) ($plan['provider_key'] ?? 'anthropic'),
            'model_key' => (string) ($plan['model_key'] ?? ''),
        ],
        'usage' => [
            'input_tokens' => (int) ($plan['input_tokens'] ?? 0),
            'output_tokens' => (int) ($plan['output_tokens'] ?? 0),
        ],
        'items' => array_map(static function (array $item): array {
            $payload = [];
            if (!empty($item['suggested_payload_json'])) {
                $decoded = json_decode((string) $item['suggested_payload_json'], true);
...
function mg_ai_merchant_create_plan(PDO $pdo, array $user, array $input): array
{
    ...
    if (!in_array($scope, mg_ai_merchant_allowed_scopes(), true)) {
        mg_fail('Invalid merchant AI planning scope.', 422);
    }

    $agent = null;
    $agentPublicId = trim((string) ($input['agent_id'] ?? ''));
    if ($agentPublicId !== '') {
        $agent = mg_agent_require_owned($merchantId, $agentPublicId);
    }
    ...
    $request = [
        ...
        'max_tokens' => max(1024, min(6000, (int) ($input['max_tokens'] ?? 3000))),
        'temperature' => 0.2,
        'system' => $system,
        'messages' => [
            [
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO ai_merchant_plans
             (public_id,merchant_user_id,agent_id,provider_id,model_id,scope,merchant_goal,status,priority,summary,prompt_fingerprint,input_context_json,raw_response_json,input_tokens,output_tokens,created_by_user_id,created_at,updated_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())'
        );
        $stmt->execute([
            $planPublicId,
            ...
            (int) $model['id'],
            $scope,
            mg_ai_merchant_clean_string($input['merchant_goal'] ?? '', 1000) ?: null,
            'review_ready',
            $normalized['priority'],
            $normalized['summary'],
            $fingerprint,
        ...
        $itemStmt = $pdo->prepare(
            'INSERT INTO ai_merchant_plan_items
             (public_id,plan_id,sequence_no,action_key,target_type,target_reference,risk_level,requires_approval,confidence,title,reason,suggested_payload_json,status,created_at,updated_at)
        ...
        foreach ($normalized['recommendations'] as $item) {
            $sequence++;
            $payloadJson = json_encode($item['suggested_payload'] ?? [], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            $itemStmt->execute([
                mg_merchant_uuid(),
                $planId,
                $sequence,
        ...
        mg_audit('merchant.ai_plan_created', 'ai_merchant_plan', [
            'plan_id' => $planPublicId,
            'scope' => $scope,
            'item_count' => count($items),
            'model_key' => (string) $model['model_key'],
        ], $merchantId);
        mg_event('merchant.ai_plan_created', [
            'plan_id' => $planPublicId,
            'scope' => $scope,
            'item_count' => count($items),
        ], $merchantId);
        ...
        mg_ai_merchant_record_usage_event($pdo, (int) $provider['id'], (int) $model['id'], $merchantId, $agent ? (int) $agent['id'] : null, 'failed', $rawResponse, [
            'source' => 'merchant_agent_plan',
            'exception_class' => $error::class,
        ]);
        mg_security_log('error', 'merchant.ai_plan_failed', 'Merchant AI plan failed.', [
            'exception_class' => $error::class,
            'scope' => $scope,
        ], $merchantId);
