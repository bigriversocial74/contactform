...
function mg_ai_chat_allowed_links(): array
{
    return [
        ...
        '/merchant-campaign-media-performance.php',
        '/merchant-crm-segment-action-center.php',
...
function mg_ai_chat_url_allowed(string $url, array $allowedLinks): bool
{
    if ($url === '') return true;
    if (in_array($url, $allowedLinks, true)) return true;
    $path = (string)(parse_url($url, PHP_URL_PATH) ?: '');
    return $path !== '' && in_array($path, $allowedLinks, true);
}
...
function mg_ai_chat_infer_action_key(array $card): string
{
    ...
    if (str_contains($url, 'segment') || str_contains($url, 'crm') || str_contains($url, 'followup') || str_contains($text, 'follow-up') || str_contains($text, 'follow up') || str_contains($text, 'crm')) return 'create_crm_followup_task';
...
function mg_ai_chat_normalize_cards(mixed $cards): array
{
    ...
    foreach ($cards as $card) {
        ...
        $url = mg_ai_chat_clean($card['action_url'] ?? $card['url'] ?? '', 260);
        if (!mg_ai_chat_url_allowed($url, $allowedLinks)) $url = '';
...
CRM media intelligence rules:
- You are aware of Watch Video Reward and Listen Music Reward campaigns, CRM contact timelines, media progress events, milestone-issued rewards, Inbox / PPPM handoff, claimed/redeemed status, saved CRM media segments, and the Saved Segment Action Center.
- Use crm_media_campaigns and saved_crm_media_segments from the merchant_operating_snapshot when present.
- Suggest saved segments for practical behavior groups: started but did not finish, milestone reached but not claimed, claimed but not redeemed, redeemed/completed, no tracked activity, website/embed media leads, and high-intent media contacts.
- Recommend the segment action center for previewing matching contacts, checking count movement, refreshing counts, renaming/duplicating/deleting segments, exporting CSV, or launching bulk CRM actions.
- You may link to /merchant-campaign-media-performance.php, /merchant-crm.php, and /merchant-crm-segment-action-center.php, including query strings from the operating snapshot.
- Never claim that you sent a message, issued a reward, created a follow-up, or changed a segment. Say the merchant can open the workflow, preview, and approve.
...
function mg_ai_chat_record_message(PDO $pdo, int $merchantId, string $role, string $body, array $cards = [], array $meta = []): string
{
    ...
    $context = array_merge([
        ...
        'guardrail_applied' => 'Merchant agent chat is advisory. Review queue approval is required for workflow actions.',
    ], $meta);
...
function mg_ai_chat_public_state(PDO $pdo, int $merchantId): array
{
    ...
    return [
        ...
        'messages' => $messages,
        'quick_prompts' => [
            'Analyze my Watch/Listen CRM media segments and suggest the best follow-up.',
            'Which saved media segment should I message or reward next?',
            'Show me media campaign claim and redemption gaps.',
...
function mg_ai_chat_bridge_to_review(PDO $pdo, array $user, array $input): array
{
    ...
    if (!empty($card['review_item_id'])) return ['card' => $card, 'state' => mg_ai_chat_public_state($pdo, $merchantId)];
    ...
    $payload = array_merge(mg_ai_chat_json($card['review_payload'] ?? []), ['source'=>'merchant_agent_chat','source_chat_message_id'=>$messageId,'source_card_index'=>$cardIndex,'title'=>$title,'reason'=>$reason,'action_url'=>$card['action_url'] ?? '']);
    try {
        $pdo->beginTransaction();
        $planPublicId = mg_ai_chat_uuid();
        $itemPublicId = mg_ai_chat_uuid();
        $contextJson = json_encode(['source'=>'merchant_agent_chat','chat_message_id'=>$messageId,'card_index'=>$cardIndex,'card'=>$card], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        ...
        $stmt->execute([$planPublicId,$merchantId,null,(int)$model['provider_id'],(int)$model['id'],$scope==='overview'?'all':$scope,$title,'review_ready','medium',$reason!==''?$reason:$title,$fingerprint,$contextJson,json_encode(['source'=>'merchant_agent_chat_bridge'],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),0,0,$merchantId]);
        ...
        $stmt->execute([$itemPublicId,$planId,1,$actionKey,'agent_chat_card',$messageId,$risk,1,0.8,$title,$reason,json_encode($payload,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)]);
        ...
        if (function_exists('mg_audit')) mg_audit('merchant.agent_chat_sent_to_review', $merchantId, ['plan_id'=>$planPublicId,'item_id'=>$itemPublicId,'action_key'=>$actionKey]);
        if (function_exists('mg_event')) mg_event($merchantId, 'merchant.agent_chat.sent_to_review', ['plan_id'=>$planPublicId,'item_id'=>$itemPublicId,'action_key'=>$actionKey]);
        $pdo->commit();
...
function mg_ai_chat_send(PDO $pdo, array $user, array $input): array
{
        ...
        'messages' => [[
            ...
            'content' => [[
                ...
                'text' => json_encode([
                    ...
                    'agent_profile' => mg_agent_profile($pdo, $merchantId),
                    'active_thread' => $thread,
                    'bridge_instruction' => 'When useful, include review_action_key and review_payload so the merchant can send a card to the Agent Review Queue. For Watch/Listen CRM media opportunities, prefer create_crm_followup_task for follow-up workflows, create_message_draft for message drafts, recommend_reward_optimization for reward gaps, and recommend_campaign_optimization for campaign/media performance improvements.',
    ...
    try {
        ...
        $decoded = mg_ai_chat_decode_agent_payload($text);
        ...
        mg_ai_merchant_record_usage_event($pdo, (int)$provider['id'], (int)$model['id'], $merchantId, null, 'completed', $rawResponse, ['source'=>'merchant_agent_chat','scope'=>$scope,'skills'=>$skillKeys,'thread_id'=>$threadId]);
        $pdo->beginTransaction();
        $meta = ['scope'=>$scope,'thread_public_id'=>$threadId,'skills'=>$skillKeys];
        $userId = mg_ai_chat_record_message($pdo, $merchantId, 'user', $message, [], $meta);
        $assistantId = mg_ai_chat_record_message($pdo, $merchantId, 'assistant', $reply, $cards, $meta + ['blocks'=>$blocks,'model'=>(string)$model['model_key']]);
        $pdo->commit();
        return [
            'user_message' => ['id'=>$userId,'role'=>'user','body'=>$message,'cards'=>[],'blocks'=>[],'scope'=>$scope,'thread_public_id'=>$threadId,'created_at'=>date('c')],
            'assistant_message' => ['id'=>$assistantId,'role'=>'assistant','body'=>$reply,'cards'=>$cards,'blocks'=>$blocks,'scope'=>$scope,'thread_public_id'=>$threadId,'model'=>(string)$model['model_key'],'created_at'=>date('c')],
