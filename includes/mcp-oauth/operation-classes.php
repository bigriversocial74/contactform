...
function mg_mcp_oauth_maximum_operation_class(mixed $value, string $default = 'read'): string
{
    ...
    if (!in_array($class, ['read', 'draft'], true)) {
        throw new MgMcpOAuthException('Only read or reviewable draft client access is supported.', 'invalid_client_metadata', 422);
    }
    return $class;
}
...
function mg_mcp_oauth_scope_keys_for_class(PDO $pdo, mixed $value, string $maximumOperationClass, bool $requireProfile = true): array
{
    ...
    if (count($requested) > 20) throw new MgMcpOAuthException('Too many scopes were requested.', 'invalid_scope', 422);
    ...
    $stmt = $pdo->prepare(
        "SELECT scope_key,operation_class FROM mcp_scope_catalog
         WHERE scope_key IN ($placeholders) AND active=1 AND grantable=1"
    );
    ...
    foreach ($rows as $row) {
        ...
        if (!in_array($operationClass, ['read', 'draft'], true)
            || mg_mcp_oauth_operation_rank($operationClass) > mg_mcp_oauth_operation_rank($maximumOperationClass)) {
            continue;
        }
        $allowed[] = (string)$row['scope_key'];
    }
    sort($allowed);
    $expected = $requested;
    sort($expected);
...
function mg_mcp_oauth_scopes_supported_for_class(PDO $pdo, string $maximumOperationClass = 'draft'): array
{
    $maximumOperationClass = mg_mcp_oauth_maximum_operation_class($maximumOperationClass);
    $classes = $maximumOperationClass === 'draft' ? ['read', 'draft'] : ['read'];
    $placeholders = implode(',', array_fill(0, count($classes), '?'));
    $stmt = $pdo->prepare(
        "SELECT scope_key FROM mcp_scope_catalog
         WHERE active=1 AND grantable=1 AND operation_class IN ($placeholders)
         ORDER BY scope_key"
    );
...
function mg_mcp_oauth_operation_class_for_scopes(PDO $pdo, array $scopes): string
{
    ...
    $stmt = $pdo->prepare("SELECT operation_class FROM mcp_scope_catalog WHERE scope_key IN ($placeholders)");
    $stmt->execute($scopes);
    foreach ($stmt->fetchAll(PDO::FETCH_COLUMN) as $operationClass) {
        if ((string)$operationClass === 'draft') return 'draft';
    }
    return 'read';
}
