...
function mg_mcp_oauth_resolve_access_token_hash(PDO $pdo, string $tokenHash, string $resourceUri): array
{
    ...
    $stmt = $pdo->prepare(
        "SELECT t.*,c.public_id AS connection_public_id,c.token_version AS current_token_version,
                oc.status AS consent_status,r.client_id,r.status AS registration_status,
                mc.status AS mcp_client_status
         FROM mcp_oauth_tokens t
         INNER JOIN mcp_connections c ON c.id=t.connection_id
         INNER JOIN mcp_oauth_consents oc
           ON oc.connection_id=c.id AND oc.client_registration_id=t.client_registration_id
         INNER JOIN mcp_oauth_client_registrations r ON r.id=t.client_registration_id
         INNER JOIN mcp_clients mc ON mc.id=r.mcp_client_id
    ...
    if (!$token
        ...
        || (string)$token['registration_status'] !== 'active'
        || !in_array((string)$token['mcp_client_status'], ['development', 'active'], true)
