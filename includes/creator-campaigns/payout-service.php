...
function mg_creator_campaign_payout_create(PDO $pdo,array $user,string $participantPublicId,array $input): array
{
    ...
    try{
        ...
        $stmt=$pdo->prepare("SELECT r.*,e.public_id earning_public_id FROM creator_campaign_budget_reservations r INNER JOIN creator_campaign_earning_events e ON e.id=r.earning_event_id LEFT JOIN creator_campaign_payout_items i ON i.reservation_id=r.id AND i.status IN('scheduled','paid') LEFT JOIN creator_campaign_disputes d ON d.source_type='reservation' AND d.source_public_id=r.public_id AND d.status IN('open','under_review') WHERE r.campaign_id=? AND r.participant_id=? AND r.creator_user_id=? AND r.currency=? AND r.status='committed' AND i.id IS NULL AND d.id IS NULL ORDER BY r.id ASC FOR UPDATE");
        $stmt->execute([(int)$participant['campaign_id'],(int)$participant['id'],(int)$participant['creator_user_id'],$currency]);$reservations=$stmt->fetchAll(PDO::FETCH_ASSOC)?:[];
        $total=0;foreach($reservations as$r)$total+=(int)$r['amount_minor'];if($total<1)throw new DomainException('No eligible committed reservations are available.');if($total<(int)$profile['minimum_payout_minor'])throw new DomainException('Eligible committed balance is below the payout minimum.');
        ...
        $payout=['id'=>$payoutId,'public_id'=>$publicId,'amount_minor'=>$total,'currency'=>$currency];mg_creator_campaign_payout_append_event($pdo,$payout,'created',null,'draft',(int)$context['actor_user_id'],'payout-created:'.$publicId,'Payout record created from committed campaign reservations.');
        $pdo->commit();return['payout_id'=>$publicId,'status'=>'draft','amount_minor'=>$total,'currency'=>$currency,'item_count'=>count($reservations)];
    }catch(Throwable$e){if($pdo->inTransaction())$pdo->rollBack();throw$e;}
}
