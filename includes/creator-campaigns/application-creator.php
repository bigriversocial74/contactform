...
function mg_creator_campaign_application_save_creator(
    ...
    try {
        ...
        if ($submit && !empty($campaign['automatic_acceptance'])) {
            $automaticEvaluation = mg_creator_campaign_evaluate_automatic_acceptance(
                $pdo,
                $campaign,
                $creatorUserId
            );
            if (!empty($automaticEvaluation['eligible'])) {
                ...
                $participant = mg_creator_campaign_participant_upsert_pending(
                    ...
                    null,
                    $actorId,
                    'Automatically accepted by campaign eligibility rules.'
                );
                ...
                $pdo->prepare(
                    "UPDATE creator_campaign_applications
                     SET status='approved',decision_note='Automatically accepted by campaign eligibility rules.',
                         reviewed_at=NOW(),decided_at=NOW(),updated_by_user_id=?,lock_version=lock_version+1,updated_at=NOW()
                     WHERE id=?"
                )->execute([$actorId, $applicationId]);
        ...
        mg_creator_campaign_participation_event($pdo, [
            ...
            'event_type' => $finalStatus === 'approved'
                ? 'application.auto_approved'
                : ($submit ? 'application.submitted' : 'application.saved'),
            'from_status' => $fromStatus,
            'to_status' => $finalStatus,
            'reason' => $finalStatus === 'approved' ? 'Campaign automatic acceptance rules passed.' : null,
            'context' => [
                'automatic_acceptance_enabled' => !empty($campaign['automatic_acceptance']),
                'automatic_evaluation' => $automaticEvaluation === null ? null : [
                    'eligible' => (bool) $automaticEvaluation['eligible'],
                    'checks' => $automaticEvaluation['checks'],
                    'capacity' => $automaticEvaluation['capacity'],
                ],
            ],
        ]);
