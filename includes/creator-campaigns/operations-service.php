...
{
    if(!mg_creator_campaign_operations_installed($pdo))throw new RuntimeException('Creator affiliate operations schema is incomplete.');
}
...
function mg_creator_campaign_operations_policy(PDO $pdo,int $workspaceId,string $currency='USD',bool $forUpdate=false): ?array
{
    ...
    $stmt=$pdo->prepare($sql);$stmt->execute([$workspaceId,$currency]);$row=$stmt->fetch(PDO::FETCH_ASSOC);
    if(!$row)return null;
    $row['configured']=true;$row['next_payout_date']=mg_creator_campaign_operations_next_payout_date($row);
    return $row;
}

function mg_creator_campaign_operations_effective_policy(PDO $pdo,int $workspaceId,string $currency='USD'): array
{
    return mg_creator_campaign_operations_policy($pdo,$workspaceId,$currency,false)?:mg_creator_campaign_operations_default_policy($currency);
}
...
function mg_creator_campaign_operations_next_payout_date(array $policy,?DateTimeImmutable $now=null): ?string
{
    ...
    $utc=new DateTimeZone('UTC');$now=($now?:new DateTimeImmutable('now',$utc))->setTimezone($utc);
    if($cadence==='weekly'){
        $weekday=max(1,min(7,(int)($policy['payout_weekday']??5)));$days=($weekday-(int)$now->format('N')+7)%7;if($days===0)$days=7;
        return $now->modify('+'.$days.' days')->format('Y-m-d');
    }
    if($cadence==='biweekly'){
        $weekday=max(1,min(7,(int)($policy['payout_weekday']??5)));
        $anchorRaw=(string)($policy['created_at']??$policy['updated_at']??$now->format('Y-m-d H:i:s'));
        try{$anchor=new DateTimeImmutable($anchorRaw,$utc);}catch(Throwable){$anchor=$now;}
        $anchor=$anchor->setTimezone($utc)->setTime(0,0);$offset=($weekday-(int)$anchor->format('N')+7)%7;$candidate=$anchor->modify('+'.$offset.' days');
        while($candidate<=$now)$candidate=$candidate->modify('+14 days');
    ...
    if($candidate<=$now){$next=$now->modify('first day of next month');$candidate=$next->setDate((int)$next->format('Y'),(int)$next->format('m'),$day)->setTime(0,0);}
    return $candidate->format('Y-m-d');
}
...
function mg_creator_campaign_operations_policy_save(PDO $pdo,array $user,array $input): array
{
    ...
    $workspaceId=(int)$context['workspace_id'];$currency=mg_creator_campaign_operations_currency($input['currency']??'USD');$status=strtolower(trim((string)($input['status']??'active')));$cadence=strtolower(trim((string)($input['cadence']??'manual')));
    ...
    $weekday=$input['payout_weekday']??null;$monthDay=$input['payout_day_of_month']??null;$weekday=$weekday===''||$weekday===null?null:(int)$weekday;$monthDay=$monthDay===''||$monthDay===null?null:(int)$monthDay;
    ...
    if($cadence==='manual'){$weekday=null;$monthDay=null;}if(in_array($cadence,['weekly','biweekly'],true))$monthDay=null;if($cadence==='monthly')$weekday=null;
    $holdDays=(int)($input['hold_days']??7);$minimum=mg_creator_campaign_compensation_minor($input['minimum_payout_minor']??2500,'minimum_payout_minor',false);$disputeWindow=(int)($input['dispute_window_days']??30);
    if($holdDays<0||$holdDays>90)throw new InvalidArgumentException('Hold days must be between 0 and 90.');if($disputeWindow<0||$disputeWindow>120)throw new InvalidArgumentException('Dispute window must be between 0 and 120 days.');
    $method=mb_substr(trim((string)($input['method_label']??'')),0,120);$instructions=mb_substr(trim((string)($input['payment_instructions']??'')),0,2000);$actor=(int)$context['actor_user_id'];
    ...
    try{
        ...
        if($existing){$pdo->prepare('UPDATE creator_campaign_payout_policies SET status=?,cadence=?,payout_weekday=?,payout_day_of_month=?,hold_days=?,minimum_payout_minor=?,method_label=?,payment_instructions=?,dispute_window_days=?,manual_approval_required=1,updated_by_user_id=?,lock_version=lock_version+1 WHERE id=?')->execute([$status,$cadence,$weekday,$monthDay,$holdDays,$minimum,$method?:null,$instructions?:null,$disputeWindow,$actor,(int)$existing['id']]);}
        else{$pdo->prepare('INSERT INTO creator_campaign_payout_policies(public_id,workspace_id,currency,status,cadence,payout_weekday,payout_day_of_month,hold_days,minimum_payout_minor,method_label,payment_instructions,dispute_window_days,manual_approval_required,created_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,1,?,?)')->execute([mg_creator_campaign_public_id('ccpy'),$workspaceId,$currency,$status,$cadence,$weekday,$monthDay,$holdDays,$minimum,$method?:null,$instructions?:null,$disputeWindow,$actor,$actor]);}
        $policy=mg_creator_campaign_operations_policy($pdo,$workspaceId,$currency,false);$pdo->commit();return $policy?:mg_creator_campaign_operations_default_policy($currency);
...
function mg_creator_campaign_operations_campaign_readiness(PDO $pdo,int $workspaceId): array
{
    $stmt=$pdo->prepare("SELECT cc.public_id,cc.title,cc.status,cc.updated_at,
      ...
      (SELECT COUNT(*) FROM creator_campaign_budgets b WHERE b.campaign_id=cc.id AND b.status='active' AND (SELECT COALESCE(SUM(be.available_delta_minor),0) FROM creator_campaign_budget_events be WHERE be.budget_id=b.id)>0) active_budgets,
    ...
    $stmt->execute([$workspaceId]);$rows=$stmt->fetchAll(PDO::FETCH_ASSOC)?:[];
    foreach($rows as &$row){
        $checks=[
        ...
        $score=0;$missing=[];foreach($checks as $check){if($check['ok'])$score+=(int)$check['weight'];else$missing[]=$check;}$row['readiness_score']=$score;$row['ready']=$score===100;$row['checks']=$checks;$row['missing']=$missing;
    }
...
function mg_creator_campaign_operations_participants(PDO $pdo,int $workspaceId,string $currency): array
{
    $stmt=$pdo->prepare("SELECT p.public_id participant_public_id,p.status participant_status,p.creator_user_id,
      ...
      WHERE cc.workspace_id=? GROUP BY p.id,pp.id ORDER BY cc.updated_at DESC,cp.display_name ASC,p.id DESC LIMIT 500");
    $stmt->execute([$currency,$currency,$currency,$workspaceId]);return $stmt->fetchAll(PDO::FETCH_ASSOC)?:[];
}
...
{
    $candidates=[];$errors=[];$run=static function(string $name,callable $query)use(&$candidates,&$errors):void{try{$items=$query();foreach($items as $item)$candidates[]=$item;}catch(Throwable $e){$errors[]=['check'=>$name,'message'=>$e->getMessage()];}};
    ...
    $run('paid_order_state',function()use($pdo,$workspaceId):array{
        $stmt=$pdo->prepare("SELECT o.public_id order_public_id,cc.public_id campaign_public_id,cc.title campaign_title,
          ...
          FROM commerce_orders o INNER JOIN merchant_workspaces mw ON mw.merchant_user_id=o.merchant_user_id
        ...
        $stmt->execute([$workspaceId]);$out=[];foreach($stmt->fetchAll(PDO::FETCH_ASSOC)?:[] as $row){$status=(string)$row['affiliate_status'];$severity=$status==='failed'?'critical':($status==='earned_unreserved'?'high':'warning');$out[]=mg_creator_campaign_operations_candidate('paid_order_incomplete',$severity,'order',(string)$row['order_public_id'],'Paid order has an incomplete Creator affiliate lifecycle.',['status'=>$status,'campaign_title'=>$row['campaign_title'],'last_error'=>$row['last_error']],(string)$row['campaign_public_id']);}return $out;
    });
    ...
    $run('attribution_without_earning',function()use($pdo,$workspaceId):array{
        $stmt=$pdo->prepare("SELECT a.public_id attribution_public_id,cc.public_id campaign_public_id,cc.title campaign_title,te.public_id conversion_public_id
          ...
          WHERE cc.workspace_id=? AND a.status IN('attributed','overridden') AND ee.id IS NULL ORDER BY a.attributed_at DESC LIMIT 300");
    ...
    $run('earning_without_reservation',function()use($pdo,$workspaceId):array{
        $stmt=$pdo->prepare("SELECT e.public_id earning_public_id,e.amount_minor,e.currency,cc.public_id campaign_public_id,cc.title campaign_title
          FROM creator_campaign_earning_events e INNER JOIN creator_campaigns cc ON cc.id=e.campaign_id
    ...
    $run('refund_without_adjustment',function()use($pdo,$workspaceId):array{
        $stmt=$pdo->prepare("SELECT pr.public_id refund_public_id,pr.amount_cents,o.public_id order_public_id,cc.public_id campaign_public_id,cc.title campaign_title,
          ...
          FROM payment_refunds pr INNER JOIN commerce_orders o ON o.id=pr.order_id INNER JOIN merchant_workspaces mw ON mw.merchant_user_id=o.merchant_user_id
          INNER JOIN creator_campaigns cc ON cc.workspace_id=mw.id AND cc.public_id=JSON_UNQUOTE(JSON_EXTRACT(o.metadata_json,'$.creator_affiliate.campaign_id'))
          WHERE cc.workspace_id=? AND pr.status='succeeded' AND COALESCE(JSON_UNQUOTE(JSON_EXTRACT(o.metadata_json,'$.creator_affiliate.earning_event_id')),'')<>''
...
{
    $context=mg_creator_campaign_payout_merchant_context($pdo,$user,'merchant.creator_payouts.manage');mg_creator_campaign_operations_assert_installed($pdo);
    $workspaceId=(int)$context['workspace_id'];$actor=(int)$context['actor_user_id'];$scanToken=bin2hex(random_bytes(16));$detected=mg_creator_campaign_operations_detect($pdo,$workspaceId);
    mg_creator_campaign_assert_transaction_boundary($pdo);$pdo->beginTransaction();
    try{
        ...
        if($detected['errors']===[])$pdo->prepare("UPDATE creator_campaign_reconciliation_cases SET status='resolved',resolved_at=NOW(),updated_at=NOW() WHERE workspace_id=? AND status IN('open','acknowledged') AND COALESCE(scan_token,'')<>?")->execute([$workspaceId,$scanToken]);
        if($detected['errors']!==[]){
        ...
        $pdo->commit();return ['detected'=>count($detected['candidates']),'errors'=>$detected['errors'],'scan_token'=>$scanToken,'actor_user_id'=>$actor];
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
...
function mg_creator_campaign_operations_cases(PDO $pdo,int $workspaceId,array $filters=[]): array
{
    ...
    $sql='SELECT * FROM creator_campaign_reconciliation_cases WHERE workspace_id=?';$params=[$workspaceId];if($status!=='all'){$sql.=' AND status=?';$params[]=$status;}$sql.=" ORDER BY FIELD(severity,'critical','high','warning','info'),last_seen_at DESC,id DESC LIMIT 500";
    $stmt=$pdo->prepare($sql);$stmt->execute($params);$rows=$stmt->fetchAll(PDO::FETCH_ASSOC)?:[];foreach($rows as &$row){$row['detail']=mg_creator_campaign_participation_decode_json($row['detail_json']??null);unset($row['detail_json'],$row['scan_token'],$row['fingerprint']);}unset($row);return $rows;
}
...
function mg_creator_campaign_operations_case_update(PDO $pdo,array $user,string $casePublicId,array $input): array
{
    ...
    $note=mb_substr(trim((string)($input['operator_note']??'')),0,2000);$actor=(int)$context['actor_user_id'];$stmt=$pdo->prepare('SELECT id,status FROM creator_campaign_reconciliation_cases WHERE public_id=? AND workspace_id=? LIMIT 1');$stmt->execute([$casePublicId,(int)$context['workspace_id']]);$case=$stmt->fetch(PDO::FETCH_ASSOC);if(!$case)throw new RuntimeException('Reconciliation case not found.');
    $pdo->prepare("UPDATE creator_campaign_reconciliation_cases SET status=?,operator_note=?,assigned_user_id=?,acknowledged_at=CASE WHEN ?='acknowledged' THEN NOW() WHEN ?='open' THEN NULL ELSE acknowledged_at END,resolved_at=CASE WHEN ?='resolved' THEN NOW() WHEN ?='open' THEN NULL ELSE resolved_at END,updated_at=NOW() WHERE id=?")
        ->execute([$status,$note?:null,$status==='open'?null:$actor,$status,$status,$status,$status,(int)$case['id']]);
...
function mg_creator_campaign_operations_creator_policies(PDO $pdo,int $creatorUserId): array
{
    ...
    $stmt=$pdo->prepare("SELECT DISTINCT py.public_id,py.workspace_id,py.currency,py.status,py.cadence,py.payout_weekday,py.payout_day_of_month,py.hold_days,py.minimum_payout_minor,py.method_label,py.payment_instructions,py.dispute_window_days,py.created_at,py.updated_at
      FROM creator_campaign_payout_policies py INNER JOIN creator_campaigns cc ON cc.workspace_id=py.workspace_id INNER JOIN creator_campaign_participants p ON p.campaign_id=cc.id AND p.creator_user_id=? ORDER BY py.currency,py.updated_at DESC");
...
function mg_creator_campaign_operations_dashboard(PDO $pdo,array $user,array $filters=[]): array
{
    ...
    $workspaceId=(int)$context['workspace_id'];$currency=mg_creator_campaign_operations_currency($filters['currency']??'USD');$policy=mg_creator_campaign_operations_effective_policy($pdo,$workspaceId,$currency);$readiness=mg_creator_campaign_operations_campaign_readiness($pdo,$workspaceId);$participants=mg_creator_campaign_operations_participants($pdo,$workspaceId,$currency);$cases=mg_creator_campaign_operations_cases($pdo,$workspaceId,$filters);
    ...
    foreach($readiness as $campaign)if(!empty($campaign['ready']))$metrics['ready_campaigns']++;foreach($cases as $case){if(in_array($case['status'],['open','acknowledged'],true))$metrics['active_cases']++;if($case['severity']==='critical'&&in_array($case['status'],['open','acknowledged'],true))$metrics['critical_cases']++;}foreach($participants as $participant){if($participant['payout_profile_status']==='eligible')$metrics['eligible_creators']++;$metrics['committed_minor']+=(int)$participant['committed_minor'];}
