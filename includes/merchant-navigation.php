...
if (!function_exists('mg_merchant_navigation_items')) {
    function mg_merchant_navigation_items(): array
    {
        return [
            ...
            'bundles' => ['Product Bundles', 'Curated multi-product experiences', '/merchant-bundles.php', 'Products & Engagement'],
            'bundle_invitations' => ['Bundle Invitations', 'Review merchant participation terms', '/merchant-bundle-invitations.php', 'Products & Engagement'],
...
if (!function_exists('mg_merchant_navigation_active_key')) {
    function mg_merchant_navigation_active_key(string $active): string
    {
        $active = trim($active);
        $aliases = [
            'merchant'=>'overview','merchant-notifications'=>'notifications','onboarding'=>'overview','merchant-onboarding'=>'overview','intelligence'=>'overview','merchant-intelligence'=>'overview',
            'product'=>'products','merchant-product'=>'products','merchant-products'=>'products','merchant-bundles'=>'bundles','bundle'=>'bundles','merchant-bundle'=>'bundles','merchant-bundle-builder'=>'bundles','merchant-bundle-invitations'=>'bundle_invitations',
            'merchant-reward-templates'=>'reward_templates','merchant-orders'=>'orders','claim'=>'claims','merchant-claim'=>'claims','merchant-claims'=>'claims','wallet_redemptions'=>'claims','merchant-wallet-redemptions'=>'claims',
            'pppm_item'=>'pppm','merchant-pppm-item'=>'pppm','merchant-pppm'=>'pppm','merchant-crm'=>'merchant_crm','merchant-reviews'=>'reviews','customer-reviews'=>'reviews','merchant-campaigns'=>'campaigns',
            'loyalty_quests'=>'loyalty_quests','merchant-loyalty-quests'=>'loyalty_quests','quest_creative'=>'loyalty_quests','merchant-loyalty-quest-creative'=>'loyalty_quests','quest_reviews'=>'loyalty_quests','merchant-quest-reviews'=>'loyalty_quests','quest_delivery'=>'loyalty_quests','merchant-loyalty-quest-delivery'=>'loyalty_quests','quest_analytics'=>'loyalty_quests','merchant-loyalty-quest-analytics'=>'loyalty_quests',
            'campaign_embed_leads'=>'campaigns','merchant-campaign-embed-leads'=>'campaigns','campaign_embed_analytics'=>'campaigns','merchant-campaign-embed-analytics'=>'campaigns','campaign_stamps'=>'stamps','merchant-campaign-stamps'=>'stamps',
            'ads-manager'=>'campaign_ads','merchant-ad-manager'=>'campaign_ads','merchant-ad-performance'=>'campaign_ads','merchant-distribution'=>'distribution','distribution_program'=>'distribution','merchant-distribution-program'=>'distribution',
            'merchant-games'=>'hosted_games','merchant-hosted-games'=>'hosted_games','hosted-games'=>'hosted_games','merchant-agent-chat'=>'agent_chat','merchant-storefront'=>'storefront','storefront_preview'=>'storefront','merchant-storefront-preview'=>'storefront','merchant-pwa'=>'merchant_pwa',
            'store-canvas'=>'store_canvas','merchant-canvas'=>'store_canvas','world-canvas'=>'world_canvas','world_canvas'=>'world_canvas','merchant-media'=>'media','merchant-payments'=>'payments','merchant-stamps'=>'stamps','merchant-locations'=>'locations','merchant-team'=>'team','merchant-integrations'=>'integrations','merchant-developer-api'=>'developer_api','merchant-settings'=>'settings',
        ];
...
if (!function_exists('mg_merchant_navigation_sidebar')) {
    ...
    {
        $activeKey=mg_merchant_navigation_active_key($active); $sidebar=[];
        foreach(mg_merchant_navigation_items() as $key=>$item){$sidebar[$key]=['section'=>$item[3]??'','label'=>$item[0]??$key,'detail'=>$item[1]??'','href'=>$item[2]??'#','visible'=>true,'active'=>$activeKey===$key];}
        return $sidebar;
    }
}
