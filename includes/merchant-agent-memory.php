...
{
    $types = ['merchant.agent_goals.saved','merchant.agent_feedback.saved','merchant.agent_preference.saved','merchant.agent_avoid_action.saved','merchant.agent_memory.updated','merchant.ai_plan_item.executed','merchant.ai_plan_item.rejected','merchant.ai_plan_item.deferred'];
...
function mg_agent_memory_summary(PDO $pdo, int $merchantId): array
{
    $summary = [
        ...
        'rejected_patterns' => [],
        'recent_feedback' => [],
        'counts' => ['useful' => 0, 'not_useful' => 0, 'too_risky' => 0, 'already_done' => 0, 'saved_preferences' => 0, 'avoid_actions' => 0],
    ];
    $seenPrefs = [];
    $seenAvoid = [];
...
function mg_agent_memory_prompt_context(PDO $pdo, int $merchantId): array
{
    ...
    return [
        ...
        'guidance' => 'Use saved preferences and approved patterns first. Avoid action types and ideas similar to rejected or too-risky feedback. Prefer low-risk review-only cards when confidence is uncertain.',
    ];
}
