...
function mg_agent_approval_source_label(string $source): string
{
    return match ($source) {
        'merchant_agent_chat' => 'Agent Chat',
        'agent_draft_package' => 'One-Click Draft Package',
        default => 'Claude Agent Plan',
    };
}
...
function mg_agent_approval_recommendation_item(int $merchantId, array $rec, array $setting, array $usage): array
{
    ...
    return [
        ...
        'source_label' => 'CRM Playbook',
        'source_context' => 'Generated by CRM playbook scan.',
        'original_chat_url' => '',
...
function mg_agent_approval_event_item(int $merchantId, array $event): array
{
    ...
    return [
        ...
        'source_type' => 'event',
        'source_id' => $sourceId,
        'source_label' => 'Automation Event',
        'source_context' => 'Generated by an approval-gated automation event.',
        'original_chat_url' => '',
...
function mg_agent_approval_ai_plan_items(PDO $pdo, int $merchantId): array
{
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $sourceLabel = mg_agent_approval_source_label($source);
        $sourceChatId = (string)($payload['source_chat_message_id'] ?? '');
        $status = (string)$row['status'] === 'deferred' ? 'deferred' : 'pending';
        $items[] = [
            'approval_id' => mg_agent_approval_id($merchantId, 'ai_plan', (string)$row['public_id']),
            'source_type' => 'ai_plan',
            'source_id' => (string)$row['public_id'],
            'source_label' => $sourceLabel,
            'source_context' => (string)($payload['package_title'] ?? $payload['source'] ?? 'Claude merchant plan recommendation.'),
            ...
            'customer_name' => '',
            'customer_email' => '',
            'campaign_title' => $sourceLabel,
            'why' => (string)($row['reason'] ?? $payload['reason'] ?? 'Agent recommendation requires merchant review.'),
            'guardrail_applied' => 'Claude created a recommendation only. Merchant approval is required before any draft, task, report, alert, or status change is created.',
            'expected_action' => (string)($payload['recommended_next_action'] ?? $row['action_key']),
            'recommended_next_action' => (string)($payload['recommended_next_action'] ?? 'Review and execute through the Stage 19C safe adapter.'),
            'risk_level' => (string)($row['risk_level'] ?? 'medium'),
            'confidence' => $row['confidence'] !== null ? (float)$row['confidence'] : null,
            'merchant_approval_required' => true,
            'can_create_task' => (string)$row['action_key'] === 'create_crm_followup_task',
            'action_type' => (string)$row['action_key'],
            'status' => $status,
            'customer_url' => '',
            'message_url' => (string)$row['action_key'] === 'create_message_draft' ? '/merchant-agent-messages.php' : '',
            'reward_url' => str_contains((string)$row['action_key'], 'reward') ? '/merchant-reward-templates.php' : '',
            'ai_plan_id' => (string)$row['plan_public_id'],
            'ai_plan_item_id' => (string)$row['public_id'],
            '_ai_plan_item' => $row,
        ];
    }
    return $items;
...
function mg_agent_approval_queue(PDO $pdo, int $merchantId, array $input = []): array
{
    ...
    $items = [];
    foreach (mg_agent_approval_ai_plan_items($pdo, $merchantId) as $item) {
        $items[] = $item;
    }
    ...
    $publicItems = array_map(static function (array $item): array {
        unset($item['_rec'], $item['_event'], $item['_ai_plan_item']);
        return $item;
...
function mg_agent_approval_find_item(PDO $pdo, int $merchantId, string $approvalId): ?array
{
    foreach (mg_agent_approval_queue($pdo, $merchantId, ['limit' => 100])['items'] as $publicItem) {
        if ((string)$publicItem['approval_id'] === $approvalId) {
            ...
            if ((string)$publicItem['source_type'] === 'ai_plan') {
                $publicItem['_ai_plan_item'] = mg_ai_plan_item_owned($pdo, $merchantId, $sourceId, false);
            }
            return $publicItem;
        }
    }
...
function mg_agent_approval_record_decision(PDO $pdo, int $merchantId, int $actorId, array $item, string $action, array $input = []): array
{
    ...
    $context = [
        ...
        'original_chat_url' => (string)($item['original_chat_url'] ?? ''),
