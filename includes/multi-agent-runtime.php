...
function mg_multi_agent_runtime_fallback(array $agent,array $template,string $message,array $context): array
{
    if(($template['key']??'')==='birthday_occasion'){
        ...
        return ['reply'=>$reply,'cards'=>[ ['type'=>'question','title'=>'Choose a deterministic action','body'=>'Ask about a gift lifecycle, Inbox, Sent, Claimed, send, regift, claim, redemption, follow-up, or message capability.','action'=>'seed_prompt','prompt'=>'Show lifecycle status for my selected gifts.','review_payload'=>[]] ]];
    }
...
function mg_multi_agent_runtime_chat(PDO $pdo,int $userId,array $agent,array $input): array
{
    ...
    $modelKey='';$creditAfter=null;$tokens=['input'=>0,'output'=>0,'total'=>0];
    if(!$result&&$tool==='discover_products'){$result=mg_task_agent_discover_products($pdo,$userId,(int)$agent['id'],$toolInput);$responseSource='system_query';$aiReason='';}
    if(!$result&&$aiReason!==''){
        try{
            $synthesis=mg_task_agent_ai_synthesis($pdo,$userId,$agent,$template,$context,$history,$message,$aiReason,mg_personal_agent_text($input['model_id']??'',80),(string)$thread['public_id']);
            if($synthesis){$result=$synthesis['result'];$modelKey=(string)$synthesis['model_key'];$tokens=$synthesis['tokens'];$creditAfter=$synthesis['credits'];$responseSource='anthropic';}
        }catch(Throwable $error){$responseSource='safe_fallback';mg_security_log('warning','multi_agent.ai_fallback','Specialized agent used safe fallback.',['exception_type'=>$error::class,'ai_reason'=>$aiReason],$userId);}
    }
    ...
    mg_audit('multi_agent.chat_completed','agent',['agent_id'=>(string)$agent['public_id'],'thread_id'=>(string)$thread['public_id'],'response_source'=>$responseSource,'model_key'=>$modelKey?:$responseSource,'ai_reason'=>$aiReason,'tool'=>$tool,'used_ai'=>$modelKey!=='','ai_tokens_total'=>$tokens['total']],$userId);
    return ['thread'=>['id'=>(string)$thread['public_id'],'title'=>(string)$thread['title']],'user_message'=>$userMessage,'assistant_message'=>$assistant,'used_ai'=>$modelKey!=='','response_source'=>$responseSource,'ai_reason'=>$aiReason,'model_key'=>$modelKey,'ai_tokens_used'=>$tokens,'ai_credits'=>$creditAfter??mg_ai_credit_snapshot($pdo,$userId,'anthropic')];
}
