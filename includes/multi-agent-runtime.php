...
function mg_multi_agent_runtime_fallback(array $agent,array $template,string $message,array $context):array
{
    if(($template['key']??'')==='birthday_occasion'){
        ...
        return['reply'=>$reply,'cards'=>[['type'=>'question','title'=>'Choose a deterministic action','body'=>'Name a recipient, shortlisted product, or editable gift plan.','action'=>'seed_prompt','prompt'=>'Show my selected plan product.','review_payload'=>[]]]];
    }
...
function mg_multi_agent_runtime_chat(PDO $pdo,int $userId,array $agent,array $input):array
{
    ...
    if(!$result&&$aiReason!==''){
        try{
            ...
            mg_security_log('warning','multi_agent.ai_fallback','Specialized agent used safe fallback.',['exception_type'=>$error::class,'ai_reason'=>$aiReason],$userId);
        }
    }
    ...
    mg_audit('multi_agent.chat_completed','agent',[
        'agent_id'=>(string)$agent['public_id'],'thread_id'=>(string)$thread['public_id'],'response_source'=>$responseSource,
        'model_key'=>$modelKey?:$responseSource,'ai_reason'=>$aiReason,'tool'=>$tool,'used_ai'=>$modelKey!=='','ai_tokens_total'=>$tokens['total'],
    ],$userId);
    return[
        'thread'=>['id'=>(string)$thread['public_id'],'title'=>(string)$thread['title']],
        'user_message'=>$userMessage,'assistant_message'=>$assistant,'used_ai'=>$modelKey!=='','response_source'=>$responseSource,
        'ai_reason'=>$aiReason,'model_key'=>$modelKey,'ai_tokens_used'=>$tokens,'ai_credits'=>$creditAfter??mg_ai_credit_snapshot($pdo,$userId,'anthropic'),
    ];
}
