 ...
 * Account restriction is immediate; irreversible processing is policy-driven.
 */
...
function mg_privacy_hash_key(): string
{
    ...
    if ($key === '' && function_exists('mg_config_value')) {
        $key = trim((string) mg_config_value('security','privacy_hash_key',''));
        if ($key === '') $key = trim((string) mg_config_value('security','claim_code_pepper',''));
    }
    if ($key === '') {
        throw new RuntimeException('Privacy hashing is not configured. Set MG_PRIVACY_HASH_KEY before processing privacy requests.');
    }
    return $key;
}
...
{
    $cursor=$date;$added=0;
    while($added<$days){$cursor=$cursor->modify('+1 day');if((int)$cursor->format('N')<6)$added++;}
    return $cursor;
}
...
function mg_privacy_request_for_user(PDO $pdo,int $userId): ?array
{
    ...
    $stmt->execute([$userId]);
    return ($row=$stmt->fetch(PDO::FETCH_ASSOC))?$row:null;
}

function mg_privacy_request_by_id(PDO $pdo,int $requestId,bool $lock=false): ?array
{
    $stmt=$pdo->prepare('SELECT * FROM privacy_requests WHERE id=? LIMIT 1'.($lock?' FOR UPDATE':''));
...
function mg_privacy_verify_password(PDO $pdo,int $userId,string $password): void
{
    ...
    if($hash===''||$password===''||!password_verify($password,$hash))throw new RuntimeException('Password confirmation failed.');
}
...
function mg_privacy_generate_merchant_handoffs(PDO $pdo,int $requestId,int $userId,string $dueAt): int
{
    ...
    $count=0;
    foreach($stmt->fetchAll(PDO::FETCH_COLUMN) as $merchantId){$insert->execute([$requestId,(int)$merchantId,$dueAt]);$count+=$insert->rowCount();}
    return $count;
}

function mg_privacy_restrict_account(PDO $pdo,int $requestId,int $userId): array
{
    $set=['status="disabled"','privacy_state="deletion_pending"','deletion_requested_at=COALESCE(deletion_requested_at,NOW())','privacy_restricted_at=COALESCE(privacy_restricted_at,NOW())','updated_at=NOW()'];
    if(mg_privacy_column_exists($pdo,'users','auth_version'))$set[]='auth_version=auth_version+1';
    $pdo->prepare('UPDATE users SET '.implode(',',$set).' WHERE id=?')->execute([$userId]);
    ...
    if(mg_privacy_table_exists($pdo,'user_sessions')){
        ...
        $stmt->execute([$userId]);$revoked=$stmt->rowCount();
    }
    foreach(['password_reset_tokens','email_verification_tokens'] as $table){
        if(mg_privacy_table_exists($pdo,$table)&&mg_privacy_column_exists($pdo,$table,'user_id')){
            $stmt=$pdo->prepare("DELETE FROM `{$table}` WHERE user_id=?");$stmt->execute([$userId]);
    ...
    if(mg_privacy_table_exists($pdo,'public_profiles')&&mg_privacy_column_exists($pdo,'public_profiles','user_id')){
        $profileSet=[];
        if(mg_privacy_column_exists($pdo,'public_profiles','visibility'))$profileSet[]='visibility="private"';
        if(mg_privacy_column_exists($pdo,'public_profiles','status'))$profileSet[]='status="hidden"';
        if(mg_privacy_column_exists($pdo,'public_profiles','published_at'))$profileSet[]='published_at=NULL';
        if($profileSet){$stmt=$pdo->prepare('UPDATE public_profiles SET '.implode(',',$profileSet).' WHERE user_id=?');$stmt->execute([$userId]);mg_privacy_action($pdo,$requestId,'restrict_public_profiles','public_profiles','restrict','completed',$stmt->rowCount(),'Public identity is unpublished while the request is processed.');}
    }
...
function mg_privacy_create_delete_request(PDO $pdo,array $user,array $input): array
{
    ...
    $identity=$pdo->prepare('SELECT email FROM users WHERE id=? LIMIT 1 FOR UPDATE');$identity->execute([$userId]);
    $email=(string)$identity->fetchColumn();
    if($email==='')throw new RuntimeException('Account identity could not be verified.');
    $emailHash=mg_privacy_identity_hash($email);
    ...
    $stmt->execute([$publicId,$userId,$jurisdiction,$email,$emailHash,mg_privacy_dt($deadlines['acknowledgement_due_at']),mg_privacy_dt($deadlines['response_due_at']),mg_privacy_dt($finalGrace),$userId,json_encode(['requested_user_agent'=>substr((string)($_SERVER['HTTP_USER_AGENT']??''),0,255)],JSON_UNESCAPED_SLASHES)]);
...
{
    if(!mg_privacy_table_exists($pdo,$table)||!mg_privacy_column_exists($pdo,$table,'user_id')){mg_privacy_action($pdo,$requestId,$actionKey,$table,'delete','skipped',0,'Table or user key is not installed.');return 0;}
    $stmt=$pdo->prepare("DELETE FROM `{$table}` WHERE user_id=?");$stmt->execute([$userId]);$rows=$stmt->rowCount();
    mg_privacy_action($pdo,$requestId,$actionKey,$table,'delete','completed',$rows,'Data is no longer required after account closure.');return $rows;
}
...
function mg_privacy_anonymize_profile_table(PDO $pdo,int $requestId,int $userId,string $table): int
{
    ...
    $set=[];
    foreach(['avatar_url','headline','bio','phone','address_line1','address_line2','city','state','postal_code','website_url','cover_url','cover_image_url','location_label'] as $column)if(mg_privacy_column_exists($pdo,$table,$column))$set[]="`{$column}`=NULL";
    if(mg_privacy_column_exists($pdo,$table,'display_name'))$set[]='display_name="Deleted User"';
    if(mg_privacy_column_exists($pdo,$table,'visibility'))$set[]='visibility="private"';
    if($table==='public_profiles'&&mg_privacy_column_exists($pdo,$table,'status'))$set[]='status="hidden"';
    if($table==='public_profiles'&&mg_privacy_column_exists($pdo,$table,'published_at'))$set[]='published_at=NULL';
    if(!$set)return 0;
    $stmt=$pdo->prepare("UPDATE `{$table}` SET ".implode(',',$set)." WHERE user_id=?");$stmt->execute([$userId]);
    mg_privacy_action($pdo,$requestId,'anonymize_'.$table,$table,'anonymize','completed',$stmt->rowCount(),'Remove public and profile identifiers.');return $stmt->rowCount();
}
...
function mg_privacy_anonymize_merchant_crm(PDO $pdo,int $requestId,int $userId): int
{
    ...
    $stmt->execute([$token,$requestId,$userId]);$rows=$stmt->rowCount();
    if(mg_privacy_table_exists($pdo,'merchant_crm_contact_events')){$event=$pdo->prepare('UPDATE merchant_crm_contact_events SET user_id=NULL,email=NULL,phone=NULL,name=NULL,metadata_json=JSON_SET(COALESCE(metadata_json,JSON_OBJECT()),"$.privacy_erased",true) WHERE user_id=?');$event->execute([$userId]);$rows+=$event->rowCount();}
    mg_privacy_action($pdo,$requestId,'anonymize_merchant_crm','merchant_crm_contacts','anonymize','completed',$rows,'Merchant records are preserved as non-identifying business history.');return $rows;
}
...
function mg_privacy_finalize_request(PDO $pdo,int $requestId,?int $actorUserId=null,bool $force=false): array
{
    ...
    if($hold=mg_privacy_active_hold($pdo,$requestId,$userId)){
    ...
    $deleted=0;
    foreach(['user_sessions','password_reset_tokens','email_verification_tokens','user_agent_messages','user_agent_settings','user_gifting_plans','user_gifting_schedules','user_recipient_data_requests','user_gift_bundles','notification_preferences','push_subscriptions','user_devices','user_saved_searches'] as $table)$deleted+=mg_privacy_delete_user_rows($pdo,$requestId,$userId,$table,'delete_'.$table);
    $anonymized=0;
    ...
    $identity=$pdo->prepare('SELECT email FROM users WHERE id=? LIMIT 1 FOR UPDATE');$identity->execute([$userId]);$email=(string)$identity->fetchColumn();
    $identityHash=(string)$request['contact_email_hash'];if($identityHash==='')$identityHash=mg_privacy_identity_hash($email);
    $anonymousEmail='deleted+'.substr($identityHash,0,16).'.'.$userId.'@privacy.invalid';
    $passwordHash=password_hash(bin2hex(random_bytes(32)),PASSWORD_DEFAULT);
    if(!is_string($passwordHash)||$passwordHash==='')throw new RuntimeException('Unable to invalidate account credentials.');
