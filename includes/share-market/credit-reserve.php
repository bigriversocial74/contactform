...
if (!function_exists('mg_share_market_credit_reserve_json')) {
    function mg_share_market_credit_reserve_json(array $value): string
    {
...
if (!function_exists('mg_share_market_credit_reserve_hash')) {
    function mg_share_market_credit_reserve_hash(array $value): string
    {
        return hash('sha256', mg_share_market_credit_reserve_json($value));
    }
}
...
if (!function_exists('mg_share_market_credit_reserve_submit')) {
    function mg_share_market_credit_reserve_submit(PDO $pdo, array $input, array $user): array
    {
        ...
        $payload = mg_share_market_credit_reserve_validate($input);
        $userId = (int)$user['id'];
        ...
        $pdo->prepare("INSERT INTO share_market_approval_requests (public_id,request_type,action_key,event_type,target_type,target_public_id,requester_user_id,status,risk_level,required_approvals,approval_count,manifest_json,projection_json,payload_hash,execution_enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,'awaiting_first_approval','medium',1,0,?,?,?,0,NOW(),NOW())")
            ->execute([mg_share_market_sql_public_id(),'treasury','credit_reserve_request','share_market.sql.credit_reserve_requested','merchant_treasury',(string)$enrollment['public_id'],$userId,mg_share_market_credit_reserve_json($manifest),mg_share_market_credit_reserve_json(['review_status'=>'pending','execution_enabled'=>false]),$hash]);
...
if (!function_exists('mg_share_market_credit_reserve_user_snapshot')) {
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM share_market_approval_requests WHERE request_type='treasury' AND action_key='credit_reserve_request' AND requester_user_id=? ORDER BY updated_at DESC,id DESC LIMIT 20");
        $stmt->execute([$userId]);
        $requests = [];
        foreach (($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) as $row) $requests[] = mg_share_market_credit_reserve_payload($row);
        return ['reserve_requests'=>$requests,'latest_reserve_request'=>$requests[0] ?? null,'execution_enabled'=>false];
    }
}
...
if (!function_exists('mg_share_market_credit_reserve_admin_queue')) {
    ...
    {
        $stmt = $pdo->query("SELECT r.*,u.email,u.display_name,u.full_name FROM share_market_approval_requests r LEFT JOIN users u ON u.id=r.requester_user_id WHERE r.request_type='treasury' AND r.action_key='credit_reserve_request' ORDER BY r.updated_at DESC,r.id DESC LIMIT 200");
        $items = [];
        foreach (($stmt ? $stmt->fetchAll(PDO::FETCH_ASSOC) : []) ?: [] as $row) {
            $item = mg_share_market_credit_reserve_payload($row);
            $item['merchant_label'] = trim((string)($row['display_name'] ?? $row['full_name'] ?? $row['email'] ?? 'Merchant'));
            $items[] = $item;
        }
        return ['credit_reserve_requests'=>$items,'summary'=>['total'=>count($items),'pending'=>count(array_filter($items, fn($i)=>$i['status']==='awaiting_first_approval')),'execution_enabled'=>false]];
    }
}
...
if (!function_exists('mg_share_market_credit_reserve_decide')) {
    function mg_share_market_credit_reserve_decide(PDO $pdo, array $input, array $user): array
    {
        ...
        $id = filter_var($input['request_id'] ?? null, FILTER_VALIDATE_INT);
        if ($id === false || $id < 1) throw new InvalidArgumentException('Select a valid reserve request.');
        $decision = strtolower(trim((string)($input['decision'] ?? '')));
        if (!in_array($decision, ['approve','reject','changes'], true)) throw new InvalidArgumentException('Select approve, reject, or request changes.');
        ...
        try {
            $stmt = $pdo->prepare("SELECT * FROM share_market_approval_requests WHERE id=? AND request_type='treasury' AND action_key='credit_reserve_request' LIMIT 1 FOR UPDATE");
            $stmt->execute([(int)$id]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$row) throw new InvalidArgumentException('Reserve request was not found.');
            $item = mg_share_market_credit_reserve_payload($row);
            $newStatus = $decision === 'approve' ? 'approved' : ($decision === 'reject' ? 'rejected' : 'awaiting_first_approval');
            $reviewStatus = $decision === 'changes' ? 'changes_requested' : $newStatus;
            $projection = ['review_status'=>$reviewStatus,'admin_note'=>$note,'reviewed_by_user_id'=>$actorId,'reviewed_at'=>gmdate('c'),'execution_enabled'=>false];
            ...
            if ($decision === 'approve') {
                $enrollment = mg_share_market_sql_fetch_enrollment_by_user($pdo, (int)$row['requester_user_id'], true);
                if (!$enrollment) throw new RuntimeException('Enrollment no longer exists.');
                $existing = mg_share_market_fetch_treasury_by_user($pdo, (int)$row['requester_user_id']);
                if (($existing['status'] ?? 'not_created') === 'not_created') {
                    $pdo->prepare("INSERT INTO share_market_credit_treasuries (public_id,enrollment_id,participant_user_id,status,credits_allocated,credits_available,created_by_user_id,metadata_json) VALUES (?,?,?,?,?,?,?,?)")->execute([mg_share_market_sql_public_id(),(int)$enrollment['id'],(int)$row['requester_user_id'],'active',$item['credits_requested'],$item['credits_requested'],$actorId,mg_share_market_credit_reserve_json(['source'=>'credit_reserve_approval','approval_request_id'=>(int)$id,'execution_enabled'=>false])]);
                } else {
                    $pdo->prepare("UPDATE share_market_credit_treasuries SET status='active',credits_allocated=credits_allocated+?,credits_available=credits_available+?,metadata_json=? WHERE public_id=?")->execute([$item['credits_requested'],$item['credits_requested'],mg_share_market_credit_reserve_json(['source'=>'credit_reserve_approval','approval_request_id'=>(int)$id,'execution_enabled'=>false]),(string)$existing['public_id']]);
                }
            }
            mg_share_market_sql_admin_event($pdo,$actorId,'share_market.sql.credit_reserve_' . $reviewStatus,'treasury_reserve',(string)$row['public_id'],(string)$row['status'],$reviewStatus,$note);
            $pdo->commit();
            mg_share_market_notify_merchant($pdo,(int)$row['requester_user_id'],$actorId,'DAVE share credit reserve ' . str_replace('_',' ',$reviewStatus),$note,'credit_reserve_decision.' . (int)$id . '.' . $reviewStatus,['target_type'=>'credit_reserve','approval_request_id'=>(int)$id,'share_market_status'=>$reviewStatus]);
