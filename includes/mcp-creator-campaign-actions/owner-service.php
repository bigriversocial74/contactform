...
function mg_mcp_creator_campaign_action_decide(PDO $pdo,array $user,string $actionPublicId,string $decision,string $reason): array
{
    ...
    try{
        ...
        $pdo->prepare('UPDATE mcp_creator_campaign_action_approvals SET status=?,decision_reason=?,decided_at=NOW(),decided_by_user_id=?,updated_at=NOW() WHERE action_id=? AND status=\'pending\'')->execute([$target,$reason,$ownerId,(int)$row['id']]);
        if($target==='approved'){
            $pdo->prepare("UPDATE mcp_automation_actions SET status='approved',approved_at=NOW(),updated_at=NOW() WHERE id=? AND status='waiting_for_approval'")->execute([(int)$row['id']]);$pdo->prepare("UPDATE mcp_automation_runs SET status='approved',updated_at=NOW() WHERE id=? AND status='waiting_for_approval'")->execute([(int)$row['run_id']]);
        }else{
            $pdo->prepare("UPDATE mcp_automation_actions SET status='rejected',error_code='MCP_CREATOR_CAMPAIGN_ACTION_REJECTED',error_message=?,completed_at=NOW(),updated_at=NOW() WHERE id=? AND status='waiting_for_approval'")->execute([$reason,(int)$row['id']]);$pdo->prepare("UPDATE mcp_automation_runs SET status='failed',error_code='MCP_CREATOR_CAMPAIGN_ACTION_REJECTED',error_message=?,completed_at=NOW(),updated_at=NOW() WHERE id=?")->execute([$reason,(int)$row['run_id']]);
        }
