...
function mg_mcp_creator_campaign_action_row(PDO $pdo,string $publicId,int $ownerUserId,bool $forUpdate=false): array
{
    $sql="SELECT aa.*,r.public_id run_public_id,r.status run_status,r.grant_id,a.public_id automation_public_id,
                 ...
                 g.workspace_type,g.workspace_id,g.maximum_operation_class grant_maximum_operation_class,
                 g.risk_ceiling,g.currency,g.per_run_amount_limit_cents,g.per_run_quantity_limit,
                 c.id connection_db_id,c.public_id connection_public_id,c.status connection_status,
                 c.expires_at connection_expires_at,c.maximum_operation_class connection_maximum_operation_class,
                 cl.id client_db_id,cl.public_id client_public_id,cl.client_key,cl.display_name client_name,
                 cl.status client_status,cl.maximum_operation_class client_maximum_operation_class,
...
{
    $input=mg_mcp_creator_campaign_action_decode($row['sanitized_input_json']??null);$receipt=$row['receipt']??null;
    return [
        'id'=>(string)$row['public_id'],'tool'=>(string)$row['tool_name'],'status'=>(string)$row['status'],
        'risk'=>(string)$row['risk_level'],'operation_class'=>(string)$row['operation_class'],'input'=>$input,
        ...
        'idempotency_key'=>(string)$row['idempotency_key'],'duplicate'=>$duplicate,
...
{
    $stmt=$pdo->prepare("SELECT ap.id,ap.action_id,aa.run_id FROM mcp_creator_campaign_action_approvals ap INNER JOIN mcp_automation_actions aa ON aa.id=ap.action_id WHERE ap.owner_user_id=? AND ap.status='pending' AND ap.expires_at<NOW()");
    $stmt->execute([$ownerUserId]);$rows=$stmt->fetchAll(PDO::FETCH_ASSOC)?:[];$expired=0;
    foreach($rows as $row){
        $approval=$pdo->prepare("UPDATE mcp_creator_campaign_action_approvals SET status='expired',updated_at=NOW() WHERE id=? AND status='pending' AND expires_at<NOW()");$approval->execute([(int)$row['id']]);
        ...
        $pdo->prepare("UPDATE mcp_automation_runs SET status='cancelled',completed_at=NOW(),error_code='MCP_CREATOR_CAMPAIGN_ACTION_APPROVAL_EXPIRED',error_message='Owner approval expired.',updated_at=NOW() WHERE id=? AND status='waiting_for_approval'")->execute([(int)$row['run_id']]);$expired++;
    }
    return $expired;
}
...
{
    $stmt=$pdo->prepare("SELECT * FROM mcp_automations WHERE grant_id=? AND playbook_key='creator_campaign_manual_canonical_actions' LIMIT 1 FOR UPDATE");$stmt->execute([(int)$grant['id']]);$row=$stmt->fetch(PDO::FETCH_ASSOC);if($row)return $row;
    $publicId=mg_public_uuid();$pdo->prepare("INSERT INTO mcp_automations(public_id,grant_id,owner_user_id,workspace_type,workspace_id,name,playbook_key,description,status,configuration_json,timezone,current_version,created_at,updated_at) VALUES (?,?,?,?,?,'Creator Campaign canonical actions','creator_campaign_manual_canonical_actions','Owner-approved, manually executed Creator Campaign canonical actions.','active','{}','UTC',1,NOW(),NOW())")->execute([$publicId,(int)$grant['id'],(int)$grant['authorizing_user_id'],$grant['workspace_type'],$grant['workspace_id']]);$stmt->execute([(int)$grant['id']]);return $stmt->fetch(PDO::FETCH_ASSOC)?:[];
}
