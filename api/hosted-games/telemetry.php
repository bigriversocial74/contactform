...
$allowed = [
    'game_loaded','game_startup','asset_load_failed','sdk_request_failed','sdk_request_slow',
    'runtime_error','manifest_warning','api_slow','webhook_failed','database_failed','reward_failed'
];
...
if (function_exists('mg_rate_limit')) mg_rate_limit('hosted.game.telemetry',$rateKey,240,300);
...
try {
    ...
    $eventId = mg_hosted_game_observability_event($pdo,$game,null,$userId ?: null,$storedType,$event,$client);
    ...
    $durationMs = max(0,min(120000,(int)($event['duration_ms'] ?? 0)));
    ...
    if (in_array($eventType,$diagnosticTypes,true) || $isSlowStartup || $isSlowRequest) {
        ...
        $diagnosticId = mg_hosted_game_observability_diagnostic($pdo,$game,null,$userId ?: null,[
            'category'=>$eventType,
            'severity'=>$severity,
