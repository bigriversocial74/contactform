...
function mg_world_canvas_safe_detail_type(mixed $value): string
{
    ...
    return in_array($type, ['merchant', 'campaign', 'reward', 'claim', 'session'], true) ? $type : '';
}
...
function mg_world_canvas_campaign_detail(PDO $pdo, string $campaignPublicId): array
{
    ...
    return [
        ...
        'subtitle' => trim((string) ($row['merchant_name'] ?? '')) ?: 'Microgifter campaign',
        'stats' => [
            ['label' => 'Type', 'value' => (string) ($row['campaign_type'] ?? 'campaign')],
            ['label' => 'Issued', 'value' => $rewards],
            ['label' => 'Limit', 'value' => $limit],
            ['label' => 'Events', 'value' => $events],
            ['label' => 'Reward', 'value' => trim((string) ($row['reward_template_title'] ?? '')) ?: 'Reward template'],
        ],
        'actions' => [],
        'note' => 'Campaign details are shown as aggregate world activity. Merchant-only campaign controls stay inside merchant tools.',
    ];
}
...
function mg_world_canvas_signal_detail(PDO $pdo, string $type, string $id, array $viewer): array
{
    if ($type === 'reward' && mg_world_canvas_table($pdo, 'wallet_items')) {
        ...
        if ($row) {
            ...
            return [
                ...
                'subtitle' => 'Reward signal · ' . (trim((string) ($row['merchant_name'] ?? '')) ?: 'Microgifter merchant'),
                'stats' => [
                    ['label' => 'Status', 'value' => (string) ($row['status'] ?? 'issued')],
                    ['label' => 'Campaign', 'value' => trim((string) ($row['campaign_title'] ?? '')) ?: 'Not attached'],
                    ['label' => 'Issued', 'value' => (string) ($row['issued_at'] ?? '')],
                ],
                'actions' => [],
                'note' => $owned ? 'This reward is connected to your account or merchant workspace.' : 'This public world signal hides recipient details unless you own the relationship.',
            ];
        }
    }
    ...
    if (($type === 'claim' || $type === 'session') && mg_world_canvas_store_ready($pdo)) {
        ...
        if ($row) {
            return [
                'type' => $type,
                'title' => trim((string) ($row['event_label'] ?? '')) ?: ucwords(str_replace('_', ' ', (string) ($row['event_type'] ?? 'Canvas signal'))),
                'subtitle' => trim((string) ($row['merchant_name'] ?? '')) ?: 'Store Canvas event',
                'stats' => [
                    ['label' => 'Signal', 'value' => (string) ($row['event_type'] ?? 'event')],
                    ['label' => 'Source', 'value' => trim((string) ($row['source_headline'] ?? '')) ?: 'World Canvas'],
                    ['label' => 'Time', 'value' => (string) ($row['created_at'] ?? '')],
                ],
                'actions' => [],
                'note' => 'Customer identity is anonymized in World Canvas. Open Merchant Store Canvas for owned CRM context.',
            ];
        }
    }
