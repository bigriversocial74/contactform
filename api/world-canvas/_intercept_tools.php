...
function mg_world_intercept_catalog(PDO $pdo): array
{
    ...
    $rows = mg_world_canvas_rows($pdo, "SELECT public_id, tool_key, name, category, rarity, description, range_meters, speed_bonus_percent, success_bonus_percent, cooldown_seconds, uses_limit, status FROM campaign_drop_tools WHERE status='active' ORDER BY FIELD(rarity,'common','rare','epic','legendary'), category, name LIMIT 100");
    return array_map(static fn(array $r): array => [
        ...
        'range_meters' => (int)$r['range_meters'],
        'speed_bonus_percent' => (int)$r['speed_bonus_percent'],
        'success_bonus_percent' => (int)$r['success_bonus_percent'],
        'cooldown_seconds' => (int)$r['cooldown_seconds'],
        'uses_limit' => $r['uses_limit'] === null ? null : (int)$r['uses_limit'],
    ], $rows);
}
...
function mg_world_intercept_user_tools(PDO $pdo, int $userId, bool $grantStarter = true): array
{
    ...
    return array_map(static function(array $r): array {
        ...
        return [
            'id' => (string)$r['user_tool_public_id'],
            'tool_id' => (string)$r['tool_public_id'],
            'key' => (string)$r['tool_key'],
            'name' => (string)$r['name'],
            'category' => (string)$r['category'],
            'rarity' => (string)$r['rarity'],
            'description' => (string)($r['description'] ?? ''),
            'status' => (string)$r['user_tool_status'],
            'range_meters' => (int)$r['range_meters'],
            'speed_bonus_percent' => (int)$r['speed_bonus_percent'],
            'success_bonus_percent' => (int)$r['success_bonus_percent'],
            'cooldown_seconds' => (int)$r['cooldown_seconds'],
            'uses_remaining' => $r['uses_remaining'] === null ? null : (int)$r['uses_remaining'],
            'cooldown_until' => $cooldownUntil,
            'cooldown_active' => $cooldownUntil !== null && strtotime((string)$cooldownUntil) > time(),
...
function mg_world_intercept_attempt(PDO $pdo, array $user, array $input): array
{
    ...
    try {
        ...
        if ($status === 'success') {
            $pdo->prepare("UPDATE merchant_target_drop_delivery_runs SET status='intercepted', intercepted_at=NOW(), intercepted_by_user_id=?, intercept_tool_public_id=?, updated_at=NOW() WHERE id=?")->execute([$userId, (string)$userToolPublicId, (int)$run['id']]);
        }
        $pdo->commit();
    } catch (Throwable $e) {
