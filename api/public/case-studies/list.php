...
try{
 ...
 $q=trim((string)($_GET['q']??''));$category=trim((string)($_GET['category']??''));$sort=strtolower(trim((string)($_GET['sort']??'featured')));$limit=max(1,min(50,(int)($_GET['limit']??24)));$where=["f.status='published'","p.visibility='public'","p.status='active'"];$args=[];
 if($q!==''){$where[]='(COALESCE(f.title,p.display_name) LIKE ? OR COALESCE(f.subtitle,p.headline) LIKE ? OR p.display_name LIKE ?)';$like='%'.$q.'%';array_push($args,$like,$like,$like);}if($category!==''){$where[]='p.profile_type=?';$args[]=$category;}
 ...
 $sql="SELECT f.public_id case_study_id,f.status,f.display_order,f.hero_featured,f.title,f.subtitle,f.challenge_text,f.solution_text,f.outcomes_json,f.testimonial_text,f.testimonial_name,f.testimonial_role,f.published_at,p.public_id profile_id,p.slug,p.display_name,p.headline,p.profile_type,p.location_label,p.avatar_url,p.cover_url,
