...
$pdo = mg_db();
$integrityContext = mg_lqi_gate_request($pdo, (int)$user['id'], 'submit', $ref);
$pdo->beginTransaction();
try {
    $campaign = mg_lqp_campaign($pdo, $ref, true);
    mg_lqi_record_attempt($pdo, $campaign, (int)$user['id'], 'submit', $integrityContext, 'allowed');
    ...
    $integrity = mg_lqi_evaluate($pdo, $campaign, $participation, $user, $evidence, $integrityContext);
    $verificationPassed = !empty($evidence['verified']);
    $requiresReview = !$verificationPassed || $integrity['decision'] === 'review';
    if ($verificationPassed && !$requiresReview) mg_lqp_enforce_daily_limit($pdo, $campaign);

    $evidenceId = mg_lqp_uuid();
    $evidenceStatus = $requiresReview ? 'submitted' : 'verified';
    $metadata = [
        'action_type'=>(string)$evidence['action_type'],
        'verification_type'=>(string)$evidence['verification_type'],
        'verification_passed'=>$verificationPassed,
        'integrity_review_required'=>$integrity['decision']==='review',
        'user_agent'=>substr((string)($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255),
        'nonce_hash'=>$evidence['nonce_hash'],
        'signed_payload'=>$evidence['signed_payload'],
    ];
    $pdo->prepare('INSERT INTO loyalty_quest_evidence (public_id,participation_id,campaign_id,merchant_user_id,participant_user_id,evidence_type,status,code_hash,latitude,longitude,accuracy_meters,distance_meters,proof_url,proof_note,reference_id,evidence_fingerprint,ip_hash,device_hash,integrity_score,integrity_status,verified_at,metadata_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')
        ->execute([
            ...
            $integrity['evidence_fingerprint'],$integrity['ip_hash'],$integrity['device_hash'],(int)$integrity['score'],(string)$integrity['status'],
            !$requiresReview ? gmdate('Y-m-d H:i:s') : null,
            json_encode($metadata, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
        ]);
    $evidenceDbId=(int)$pdo->lastInsertId();
    $signalIds=mg_lqi_persist_signals($pdo,$campaign,$participation,$user,$evidenceDbId,$integrity);
    $pdo->prepare('UPDATE loyalty_quest_integrity_attempts SET outcome=? WHERE request_hash=? AND participant_user_id=?')
        ->execute([$integrity['decision']==='review'?'review':'allowed',$integrityContext['request_hash'],(int)$user['id']]);
    mg_lqi_security_event($campaign,$participation,$integrity,(int)$user['id']);
    ...
    if ($requiresReview) {
        ...
        mg_lqp_event($pdo, $campaign, null, (int)$contact['id'], 'quest.evidence_submitted', [
            'participation_id'=>(string)$participation['public_id'],
            'evidence_id'=>$evidenceId,
            'verification_type'=>(string)$evidence['verification_type'],
            'integrity_review_required'=>$integrity['decision']==='review',
            'integrity_score'=>(int)$integrity['score'],
        ]);
        ...
        mg_audit('participant.loyalty_quest_evidence_submitted', 'loyalty_quest_evidence', [
        ...
        $pdo->commit();
        $message=$integrity['decision']==='review'
            ? 'Completion submitted for merchant integrity review.'
            : 'Completion submitted for merchant review.';
        mg_ok([
            'participation_id'=>(string)$participation['public_id'],
    ...
    $find->execute([(int)$campaign['id'], (int)$user['id']]);
    $participation = $find->fetch(PDO::FETCH_ASSOC);
    mg_lqp_event($pdo, $campaign, null, (int)$contact['id'], 'quest.evidence_verified', ['participation_id'=>(string)$participation['public_id'],'evidence_id'=>$evidenceId,'progress_count'=>$newProgress,'required_count'=>(int)$participation['required_count'],'integrity_score'=>(int)$integrity['score']]);
    mg_audit('participant.loyalty_quest_evidence_verified', 'loyalty_quest_evidence', ['campaign_id'=>(string)$campaign['public_id'],'participation_id'=>(string)$participation['public_id'],'evidence_id'=>$evidenceId,'integrity_score'=>(int)$integrity['score']], (int)$user['id']);
