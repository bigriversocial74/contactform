...
try{
    ...
    if(!$participation){
        ...
        $metadata=['action_type'=>(string)($campaign['rules']['action_type']??''),'verification_type'=>(string)($campaign['rules']['verification_type']??''),'joined_from'=>'public_quest_page'];
        ...
        $existing->execute([(int)$campaign['id'],(int)$user['id']]);
        $participation=$existing->fetch(PDO::FETCH_ASSOC);
        mg_lqp_event($pdo,$campaign,null,(int)$contact['id'],'quest.joined',['participation_id'=>$publicId,'action_type'=>$metadata['action_type'],'verification_type'=>$metadata['verification_type']]);
    ...
    $pdo->commit();
    mg_ok(['participation'=>['id'=>(string)$participation['public_id'],'status'=>(string)$participation['status'],'progress_count'=>(int)$participation['progress_count'],'required_count'=>(int)$participation['required_count'],'completion_percent'=>(int)$participation['completion_percent']],'campaign'=>['id'=>(string)$campaign['public_id'],'title'=>(string)$campaign['title'],'action_type'=>(string)($campaign['rules']['action_type']??''),'verification_type'=>(string)($campaign['rules']['verification_type']??'')]],'Loyalty Quest started.',201);
