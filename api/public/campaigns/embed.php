...
function mg_campaign_embed_endpoint(string $type): string { return '/api/public/campaigns/embed-submit.php'; }
function mg_campaign_embed_submit_label(string $type): string { return match ($type) { 'newsletter_signup' => 'Join and claim reward', 'contest_giveaway' => 'Enter contest', 'qr_reward_drop' => 'Claim QR reward', 'referral_reward' => 'Join referral campaign', 'birthday_vip' => 'Join birthday rewards', 'agent_offer' => 'Add offer interest', default => 'Submit' }; }
function mg_campaign_embed_type_label(string $type): string { return match ($type) { 'newsletter_signup' => 'Newsletter signup', 'contest_giveaway' => 'Contest / giveaway', 'qr_reward_drop' => 'QR reward drop', 'referral_reward' => 'Referral reward', 'birthday_vip' => 'Birthday / VIP reward', 'agent_offer' => 'Agent offer', default => 'Campaign' }; }
...
{
    return match ($type) { 'newsletter_signup' => '/newsletter-signup.php', 'contest_giveaway' => '/contest.php', 'qr_reward_drop' => '/qr-reward.php', 'referral_reward' => '/referral-reward.php', 'birthday_vip' => '/birthday-vip.php', 'agent_offer' => '/agent-offer.php', default => '/campaign.php' };
}
...
try {
    ...
    mg_ok([
        'campaign' => [
            'id' => (string)$campaign['public_id'], 'slug' => $campaign['public_slug'] ?? null, 'campaign_type' => $campaignType, 'type_label' => mg_campaign_embed_type_label($campaignType), 'title' => (string)$campaign['title'], 'headline' => $headline, 'description' => $description, 'success_message' => $successMessage, 'status' => (string)$campaign['status'], 'available' => $available, 'availability_message' => $availabilityMessage, 'qr_token' => $campaignType === 'qr_reward_drop' ? ($campaign['qr_code_token'] ?? null) : null, 'rules' => json_decode((string)($campaign['rules_json'] ?? ''), true) ?: [], 'public_url' => $publicUrl, 'submit_endpoint' => $submitEndpoint, 'submit_label' => $settings['custom_button_text'] ?: mg_campaign_embed_submit_label($campaignType),
        ],
        'merchant' => [ 'name' => $merchantName, 'headline' => trim((string)($campaign['merchant_profile_headline'] ?? '')), 'location' => trim((string)($campaign['merchant_profile_location'] ?? '')), 'avatar_url' => mg_campaign_embed_safe_url($campaign['merchant_profile_avatar_url'] ?? null), 'profile_url' => $merchantSlug !== '' ? $base . '/profile.php?slug=' . rawurlencode($merchantSlug) : null ],
        'reward' => [ 'id' => $campaign['reward_template_public_id'] ?? null, 'title' => trim((string)($campaign['reward_template_title'] ?? '')) ?: 'Microgifter reward', 'description' => trim((string)($campaign['reward_template_description'] ?? '')), 'value_label' => mg_campaign_embed_value($campaign), 'currency' => $campaign['currency'] ?? 'USD', 'redemption_instructions' => trim((string)($campaign['redemption_instructions'] ?? '')), 'expires_at' => $campaign['reward_expires_at'] ?? null ],
        'embed' => [ 'version' => 3, 'adopts_host_css' => true, 'render_modes' => ['inline', 'button', 'compact'], 'request_origin' => $origin, 'cors_mode' => 'public_no_credentials', 'settings' => $settings, 'health' => [ 'active' => (string)$campaign['status'] === 'active', 'available' => $available, 'embed_enabled' => (bool)$settings['embed_enabled'], 'domain_allowed' => true, 'has_public_url' => $publicUrl !== '', 'has_submit_endpoint' => $submitEndpoint !== '', 'host_css_adoption' => true ] ],
