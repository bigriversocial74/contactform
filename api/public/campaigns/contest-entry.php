...
function mg_contest_campaign_bridge(PDO $pdo, array $campaign, array $contact, int $walletDbId, string $walletPublicId, ?int $userId, ?string $expiresAt, string $sourceType): ?array
{
    ...
    return mg_zero_reward_issue_from_wallet($pdo, [
        'merchant_user_id' => (int)$campaign['merchant_user_id'], 'recipient_user_id' => $userId, 'recipient_external_id' => (string)($contact['public_id'] ?? ''),
        ...
        'source_type' => $sourceType, 'source_reference' => $walletPublicId, 'source_line_reference' => (string)($contact['public_id'] ?? $walletPublicId), 'title' => (string)($campaign['reward_template_title'] ?? 'Contest reward'),
        ...
        'redemption_instructions' => $campaign['redemption_instructions'] ?? null, 'terms' => ['campaign_type' => 'contest_giveaway'],
    ]);
}
...
try {
    ...
    $sql = 'SELECT c.*, rt.id reward_template_db_id, rt.public_id reward_template_public_id, rt.title reward_template_title, rt.description reward_template_description, rt.redemption_instructions, rt.value_amount_cents, rt.currency, rt.expiration_rule, rt.expiration_days, rt.expires_at, rt.quantity_limit reward_template_quantity_limit, rt.issued_count reward_template_issued_count, rt.per_user_limit reward_template_per_user_limit FROM campaigns c INNER JOIN reward_templates rt ON rt.id = c.reward_template_id AND rt.status = \'active\' WHERE c.campaign_type = \'contest_giveaway\' AND c.status = \'active\' AND (c.public_id = ? OR c.public_slug = ?) LIMIT 1 FOR UPDATE';
    ...
    $meta = mg_public_campaign_metadata_with_embed(['campaign_type' => $campaignType, 'campaign_public_id' => (string)$campaign['public_id'], 'entry' => $entryContext, 'rules' => $rules, 'ip' => mg_client_ip()], $embedAttribution);
    ...
    $crm = mg_merchant_crm_record_event($pdo, ['merchant_user_id' => $merchantId, 'campaign_id' => $campaignId, 'campaign_type' => $campaignType, 'event_type' => 'contest.entered', 'source_type' => $source, 'source_public_id' => (string)($contact['public_id'] ?? ''), 'user_id' => $userId, 'email' => $email, 'phone' => $phone, 'name' => $name, 'metadata' => $meta]);
    ...
    mg_contest_campaign_event($pdo, $merchantId, $campaignId, null, $contactId, 'contest.entered', ['email' => $email, 'campaign_type' => $campaignType, 'rules' => $rules, 'embed_attribution' => $embedAttribution, 'merchant_crm' => $crm, 'merchant_notification' => $merchantNotification]);

    $walletPublicId = null; $walletStatus = null; $alreadyIssued = false; $rewardTitle = null; $expiresAt = null; $bridge = null; $stampLedger = null;
    ...
    if ($existing) {
        ...
        $stampLedger = mg_public_campaign_debit_reward_stamp($pdo, $campaign, $walletPublicId, $source, ['contact_id' => (string)($contact['public_id'] ?? ''), 'email' => $email, 'rules' => $rules, 'embed_attribution' => $embedAttribution]);
        $expiresAt = mg_contest_campaign_expiry($campaign);
        $rewardTitle = (string)$campaign['reward_template_title'];
        $walletMetadata = mg_public_campaign_metadata_with_embed(['campaign_type' => 'contest_giveaway', 'reward_template_id' => (string)$campaign['reward_template_public_id'], 'rules' => $rules, 'stamp_ledger_entry_id' => $stampLedger['entry']['entry_id'] ?? null], $embedAttribution);
        ...
        mg_contest_campaign_event($pdo, $merchantId, $campaignId, $walletDbId, $contactId, 'wallet_item.issued', ['wallet_item_id' => $walletPublicId, 'source_type' => $source, 'campaign_type' => $campaignType, 'rules' => $rules, 'embed_attribution' => $embedAttribution, 'pppm_bridge' => $bridge, 'merchant_crm' => $crm, 'merchant_notification' => $merchantNotification, 'stamp_ledger_entry_id' => $stampLedger['entry']['entry_id'] ?? null]);
    }
    $pdo->commit();
    $message = mg_contest_campaign_success_message($walletPublicId !== null, $alreadyIssued, $rules, $campaign);
    mg_ok(['contact_id' => (string)($contact['public_id'] ?? ''), 'wallet_item_id' => $walletPublicId, 'wallet_status' => $walletStatus, 'already_issued' => $alreadyIssued, 'reward_title' => $rewardTitle, 'expires_at' => $expiresAt, 'rules' => $rules, 'pppm_bridge' => $bridge, 'merchant_crm' => $crm, 'merchant_notification' => $merchantNotification, 'stamp_ledger' => $stampLedger, 'embed_attribution' => $embedAttribution], $message, $walletPublicId !== null && !$alreadyIssued ? 201 : 200);
