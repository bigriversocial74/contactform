...
function mg_stamp_card_json(mixed $value): array { if (is_array($value)) return $value; if (!is_string($value) || trim((string)$value) === '') return []; $decoded = json_decode((string)$value, true); return is_array($decoded) ? $decoded : []; }
function mg_stamp_card_find_user(PDO $pdo, string $email): ?int { $stmt = $pdo->prepare("SELECT id FROM users WHERE email=? AND status='active' LIMIT 1"); $stmt->execute([$email]); $id = (int)($stmt->fetchColumn() ?: 0); return $id > 0 ? $id : null; }
function mg_stamp_card_required_count(array $campaign): int { $rules = mg_stamp_card_json($campaign['rules_json'] ?? null); return max(1, min(100, (int)($rules['required_count'] ?? $rules['stamp_required_count'] ?? 5))); }
function mg_stamp_card_cooldown_hours(array $campaign): int { $rules = mg_stamp_card_json($campaign['rules_json'] ?? null); return max(0, min(8760, (int)($rules['cooldown_hours'] ?? $rules['stamp_cooldown_hours'] ?? 0))); }
function mg_stamp_card_label(array $campaign): string { $rules = mg_stamp_card_json($campaign['rules_json'] ?? null); $label = trim((string)($rules['stamp_label'] ?? 'Visit')); return $label !== '' ? mb_substr($label, 0, 40) : 'Visit'; }
function mg_stamp_card_cashier_required(array $campaign): bool { $rules = mg_stamp_card_json($campaign['rules_json'] ?? null); return array_key_exists('cashier_verification_required', $rules) ? !empty($rules['cashier_verification_required']) : true; }
...
function mg_stamp_card_verify_cashier_code(PDO $pdo, int $merchantId, string $rawCode): array
{
    ...
    $stmt = $pdo->prepare("SELECT mcc.id claim_code_id,mcc.public_id claim_code_public_id,mcc.location_id,mcc.label,mcc.code_last4,mcc.usage_limit,mcc.usage_count,ml.public_id location_public_id,ml.name location_name FROM merchant_claim_codes mcc INNER JOIN merchant_locations ml ON ml.id=mcc.location_id WHERE mcc.merchant_user_id=? AND mcc.code_hash=? AND mcc.status='active' AND ml.status='active' AND (mcc.valid_from IS NULL OR mcc.valid_from<=NOW()) AND (mcc.valid_until IS NULL OR mcc.valid_until>=NOW()) LIMIT 1 FOR UPDATE");
...
function mg_stamp_card_record_stamp(PDO $pdo, array $campaign, array $input, array $entry, array $embedAttribution, int $stampCount, int $requiredCount, ?array $verification): array
{
    ...
    $verificationMeta = $verification ? ['claim_code_id' => (string)$verification['claim_code_public_id'], 'claim_code_last4' => (string)$verification['code_last4'], 'location_id' => (string)$verification['location_public_id'], 'location_name' => (string)$verification['location_name'], 'verified_at' => gmdate('c')] : ['verified_at' => gmdate('c'), 'cashier_required' => false];
    $metadata = mg_public_campaign_metadata_with_embed(['campaign_type' => 'stamp_card_reward', 'campaign_public_id' => (string)$campaign['public_id'], 'crm_source' => $source, 'entry' => $entry, 'stamp_count' => $stampCount, 'required_count' => $requiredCount, 'stamps_remaining' => max(0, $requiredCount - $stampCount), 'stamp_result' => $stampCount >= $requiredCount ? 'unlocked' : 'recorded', 'stamp_verification' => $verificationMeta, 'crm_creation_boundary' => 'deferred_until_first_value_event', 'value_event' => false, 'ip' => mg_client_ip(), 'user_agent' => substr((string)($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255)], $embedAttribution);
    ...
    $crm = mg_merchant_crm_record_existing_contact_event($pdo, ['merchant_user_id' => $merchantId, 'campaign_id' => $campaignId, 'campaign_type' => 'stamp_card_reward', 'event_type' => 'stamp_card.stamped', 'source_type' => $source, 'source_public_id' => (string)($contact['public_id'] ?? ''), 'user_id' => $userId, 'email' => $email, 'phone' => $phone, 'name' => $name, 'metadata' => $metadata]);
    ...
    $event->execute([mg_stamp_card_uuid(), $merchantId, $campaignId, null, $contactId ?: null, 'stamp_card.stamped', json_encode(['campaign_type' => 'stamp_card_reward', 'source' => $source, 'email' => $email, 'entry' => $entry, 'stamp_count' => $stampCount, 'required_count' => $requiredCount, 'stamps_remaining' => max(0, $requiredCount - $stampCount), 'stamp_verification' => $verificationMeta, 'embed_attribution' => $embedAttribution, 'merchant_crm' => $crm, 'merchant_notification' => $merchantNotification, 'crm_creation_boundary' => 'deferred_until_first_value_event'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)]);
    if ($verification) $pdo->prepare('UPDATE merchant_claim_codes SET usage_count=usage_count+1,updated_at=NOW() WHERE id=?')->execute([(int)$verification['claim_code_id']]);
...
if ($campaignRef === '' || $email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) mg_fail('Invalid stamp card visit.', 422);
...
try {
    ...
    if ($cashierRequired) $verification = mg_stamp_card_verify_cashier_code($pdo, (int)$campaign['merchant_user_id'], (string)($input['cashier_code'] ?? $input['merchant_claim_code'] ?? ''));
    ...
    $previousStamps = 0;
    $lastStampAt = null;
    if ($existingContact) { $countStmt = $pdo->prepare("SELECT COUNT(*), MAX(created_at) FROM campaign_events WHERE campaign_id=? AND contact_id=? AND event_type='stamp_card.stamped'"); $countStmt->execute([(int)$campaign['id'], (int)$existingContact['id']]); $row = $countStmt->fetch(PDO::FETCH_NUM) ?: [0, null]; $previousStamps = (int)($row[0] ?? 0); $lastStampAt = $row[1] ?? null; }
    if ($cooldownHours > 0 && is_string($lastStampAt) && strtotime($lastStampAt) > (time() - ($cooldownHours * 3600))) { $pdo->rollBack(); mg_fail('This stamp card has a cooldown before the next stamp can be recorded.', 409); }
    $stampCount = $previousStamps + 1;
    $unlocked = $stampCount >= $requiredCount;
    $entry += ['stamp_label' => $stampLabel, 'stamp_count' => $stampCount, 'previous_stamp_count' => $previousStamps, 'required_count' => $requiredCount, 'stamps_remaining' => max(0, $requiredCount - $stampCount), 'stamp_result' => $unlocked ? 'unlocked' : 'recorded', 'stamp_cooldown_hours' => $cooldownHours, 'stamp_card_verified' => true, 'stamped_at' => gmdate('c')];
    ...
    if (!$unlocked) {
        ...
        mg_ok(['campaign_id' => (string)$campaign['public_id'], 'campaign_type' => 'stamp_card_reward', 'source' => 'stamp_card_reward', 'stamp_result' => 'recorded', 'stamp_count' => $stampCount, 'required_count' => $requiredCount, 'stamps_remaining' => $remaining, 'reward_unlocked' => false, 'contact_id' => (string)($record['contact']['public_id'] ?? ''), 'merchant_crm' => $record['crm'] ?? null, 'merchant_notification' => $record['merchant_notification'] ?? null, 'stamp_verification' => $record['verification'] ?? null, 'crm_creation_boundary' => 'deferred_until_first_value_event', 'entry' => $entry, 'embed_attribution' => $embedAttribution], $stampLabel . ' stamp verified. ' . $remaining . ' more to unlock your reward.', 200);
    }
    $pdo->commit();
...
function mg_public_campaign_engage_preprocess_input(PDO $pdo, array $input): array { $entry = $input['entry'] ?? []; if (!is_array($entry)) $entry = []; $entry = array_merge($entry, is_array($GLOBALS['mg_stamp_card_entry'] ?? null) ? $GLOBALS['mg_stamp_card_entry'] : []); $entry['stamp_result'] = 'unlocked'; $entry['stamp_card_verified'] = true; $input['entry'] = $entry; $input['campaign_type'] = 'stamp_card_reward'; return $input; }
