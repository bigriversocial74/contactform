...
try {
    ...
    $userId = (int) $user['id'];
    $existing = $pdo->prepare('SELECT public_id,status FROM wallet_items WHERE reward_template_id = ? AND user_id = ? AND source_type = \'agent_discovery\' AND status <> \'cancelled\' ORDER BY id DESC LIMIT 1');
    ...
    $wallet->execute([$walletPublicId, $userId, null, (int) $template['merchant_user_id'], (int) $template['id'], null, 'agent_discovery', $sessionEmail, 'issued', (int) $template['value_amount_cents'], (string) $template['currency'], (string) $template['title'], json_encode(['source' => 'agent_discovery', 'offer_id' => $offerId, 'approved_by_user' => true], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), $expiresAt]);
    ...
    $event->execute([mg_wallet_add_uuid(), (int) $template['merchant_user_id'], null, $walletDbId, null, 'agent_offer.added_to_wallet', json_encode(['email' => $sessionEmail, 'offer_id' => $offerId, 'approved_by_user' => true], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)]);
