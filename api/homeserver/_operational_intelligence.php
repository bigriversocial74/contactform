...
function mg_homeserver_operational_catalog(): array
{
    ...
    $catalog = [
        ...
        'crm.contacts' => mg_homeserver_dataset('CRM contacts', 'sensitive', array_merge($commonUses, ['relationship_management','campaign_targeting']), [
            mg_homeserver_source('merchant_crm_contacts', 'merchant_user_id', 'id', 'updated_at', ['id','public_id','merchant_user_id','user_id','email','phone','name','first_name','last_name','company','job_title','address','birthday','source','status','lifecycle_stage','relationship_type','tags_json','preferences_json','consent_json','created_at','updated_at']),
        ...
        'crm.activities' => mg_homeserver_dataset('CRM activities and timeline', 'restricted', array_merge($commonUses, ['relationship_management','follow_up']), [
            ...
            mg_homeserver_source('merchant_crm_events', 'merchant_user_id', 'id', 'created_at', ['id','public_id','merchant_user_id','contact_id','campaign_id','campaign_type','event_type','source_type','source_public_id','user_id','email','name','value_cents','metadata_json','created_at']),
        ], ['include_message_bodies']),
        ...
        'campaigns.performance' => mg_homeserver_dataset('Campaign performance and events', 'restricted', array_merge($commonUses, ['campaign_management','campaign_optimization']), [
            ...
            mg_homeserver_source('merchant_crm_events', 'merchant_user_id', 'id', 'created_at', ['id','public_id','merchant_user_id','campaign_id','campaign_type','event_type','source_type','source_public_id','user_id','email','name','value_cents','metadata_json','created_at']),
        ]),
...
function mg_homeserver_campaign_action(PDO $pdo, array $device, array $input): array
{
    ...
    $valueCents = max(0, (int)($input['value_cents'] ?? 0));
    $recipientCount = max(0, (int)($input['recipient_count'] ?? ($contactId !== '' ? 1 : 0)));
    ...
    if (!in_array($actionType, $allowedActions, true) || preg_match('/^[a-z0-9_]{3,80}$/', $campaignType) !== 1 || $idempotencyKey === '' || strlen($idempotencyKey) > 190) mg_fail('Invalid HomeServer campaign action request.', 422);
    ...
    if ($authorization['maximum_value_cents'] !== null && $valueCents > (int)$authorization['maximum_value_cents']) mg_fail('Campaign value exceeds the per-recipient authorization.', 403);
    ...
    $requiresApproval = $authorityLevel === 'approval_required'
        || ($authorization['approval_threshold_cents'] !== null && $valueCents > (int)$authorization['approval_threshold_cents']);
    if (in_array($actionType, $sendActions, true) && !in_array($authorityLevel, ['approval_required','authorized_execution'], true)) mg_fail('This campaign authorization does not permit sending.', 403);
    if (($input['merchant_approval_token'] ?? null) !== null) {
        $approvalHash = hash('sha256', (string)$input['merchant_approval_token']);
        $expectedHash = (string)($input['merchant_approval_hash'] ?? '');
        if ($expectedHash === '' || !hash_equals($expectedHash, $approvalHash)) mg_fail('Merchant approval token is invalid.', 403);
        $requiresApproval = false;
    }

    $request = $input;
    unset($request['merchant_approval_token']);
    ...
    if (!$requiresApproval && $actionType !== 'campaign.draft') {
        if (in_array($actionType, ['campaign.publish','campaign.pause','campaign.resume'], true)) {
            ...
            if ($stmt->rowCount() !== 1) mg_fail('Merchant campaign was not found or could not be updated.', 404);
            $providerResponse = ['campaign_id' => $campaignId, 'status' => $status, 'authority' => 'microgifter'];
        } elseif (in_array($actionType, $sendActions, true)) {
            ...
            $sendInput = [
                ...
                'reward_template_id' => (string)($input['reward_template_id'] ?? ''),
                'required_campaign_type' => $campaignType,
                'note' => trim((string)($input['message'] ?? $input['note'] ?? '')),
                'idempotency_key' => 'homeserver:' . $idempotencyKey,
            ];
            try {
                $providerResponse = mg_crm_campaign_send_for_contact($pdo, $merchantId, $merchantUser, $sendInput);
