...
function mg_zero_reward_require_authority_schema(PDO $pdo): void
{
    ...
    if ($missing !== []) throw new RuntimeException('Reward delivery schema is incomplete: ' . implode(', ', $missing));
}
...
function mg_zero_reward_template_version(PDO $pdo, array $input): array
{
    ...
    if (!$existing) {
        $template = mg_microgift_create_template($pdo, $merchantUserId, [
            'owner_type'=>'merchant','name'=>$title,'gift_type'=>'reward','visibility'=>'unlisted',
            'default_currency'=>$currency,'slug'=>$slug,
    ...
    mg_microgift_publish_version($pdo, $merchantUserId, (string)$created['version_id']);
    $stmt->execute([$merchantUserId, $slug]);
...
function mg_zero_reward_pppm_item(PDO $pdo, array $source, array $event, string $walletPublicId, array $input): array
{
    ...
    $item = mg_pppm_locked_by_id($pdo, (int)$pdo->lastInsertId());
...
function mg_zero_reward_issue_from_wallet(PDO $pdo, array $input): array
{
    ...
    if ($recipientUserId < 1) {
        return [
            'schema_ready'=>true,'pending_account_link'=>true,'wallet_item_id'=>$walletPublicId,
            'source_type'=>$sourceType,'source_reference'=>$sourceReference,
        ];
    }
    ...
    return [
        'schema_ready'=>true,'pending_account_link'=>false,'wallet_item_id'=>$walletPublicId,
        'recipient_user_id'=>$recipientUserId,'source_type'=>$sourceType,'source_reference'=>$sourceReference,
        'microgift_instance_id'=>(string)$instance['public_id'],'microgift_status'=>(string)$instance['status'],
        'pppm_item_db_id'=>(int)$pppmItem['id'],'pppm_item_id'=>(string)$pppmItem['public_id'],
        'pppm_status'=>(string)$pppmItem['status'],'action_center'=>$projection,'destination'=>'inbox',
    ];
}
