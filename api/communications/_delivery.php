...
function mg_delivery_recipient_identity(array $input): string
{
    $userId=max(0,(int)($input['recipient_user_id']??0));
    if($userId>0)return 'user:'.$userId;
    $snapshot=(array)($input['recipient_snapshot']??[]);
    $email=strtolower(trim((string)($snapshot['email']??'')));
    return filter_var($email,FILTER_VALIDATE_EMAIL)?'email:'.hash('sha256',$email):'external:none';
}
function mg_delivery_fingerprint(array $input): string
{
    $data=['recipient_identity'=>mg_delivery_recipient_identity($input),'channel'=>(string)($input['channel']??''),'template_key'=>(string)($input['template_key']??''),'category'=>(string)($input['category']??'transactional'),'payload'=>mg_delivery_redact((array)($input['payload']??[]))];
...
function mg_delivery_next_attempt(mixed $value): string
{
    $raw=trim((string)$value);
    if($raw==='')return gmdate('Y-m-d H:i:s');
    try{$date=new DateTimeImmutable($raw,new DateTimeZone('UTC'));}
    catch(Throwable){throw new MgDeliveryException('Invalid delivery schedule.',422);}
    $date=$date->setTimezone(new DateTimeZone('UTC'));
    $max=(new DateTimeImmutable('now',new DateTimeZone('UTC')))->modify('+1 year');
    if($date>$max)throw new MgDeliveryException('Delivery schedule is too far in the future.',422);
    return $date->format('Y-m-d H:i:s');
}
function mg_delivery_enqueue(PDO $pdo,array $input,?callable $failureHook=null): array
{
    mg_delivery_ensure_schema($pdo);
    $key=trim((string)($input['idempotency_key']??''));$channel=(string)($input['channel']??'');$template=trim((string)($input['template_key']??''));$recipient=max(0,(int)($input['recipient_user_id']??0));$category=(string)($input['category']??'transactional');
    $merchantId=max(0,(int)($input['merchant_user_id']??0));$campaignId=max(0,(int)($input['campaign_id']??0));$sourcePublic=mb_substr(trim((string)($input['source_public_id']??'')),0,190);$nextAttempt=mg_delivery_next_attempt($input['next_attempt_at']??null);
    $snapshot=(array)($input['recipient_snapshot']??[]);$externalEmail=strtolower(trim((string)($snapshot['email']??'')));$externalRecipient=$recipient<1&&$channel==='email'&&filter_var($externalEmail,FILTER_VALIDATE_EMAIL);
    if($key===''||strlen($key)>190||$template===''||strlen($template)>120||!in_array($channel,['in_app','email','sms','webhook'],true)||($recipient<1&&!$externalRecipient))throw new MgDeliveryException('Invalid message delivery request.',422);
    ...
    try{
        $jobRecipientId=$recipient>0&&mg_delivery_user_exists($pdo,$recipient)?$recipient:null;
        $find=$pdo->prepare('SELECT e.*,j.public_id job_public_id,j.status job_status,j.next_attempt_at FROM message_events e INNER JOIN message_delivery_jobs j ON j.message_event_id=e.id WHERE e.event_key=? LIMIT 1 FOR UPDATE');$find->execute([$key]);
        if($row=$find->fetch(PDO::FETCH_ASSOC)){if(!hash_equals((string)$row['event_fingerprint'],$fingerprint))throw new MgDeliveryException('Message event conflicts with the recorded payload.',409);if($owns)$pdo->commit();return ['event_id'=>$row['public_id'],'job_id'=>$row['job_public_id'],'status'=>$row['job_status'],'next_attempt_at'=>$row['next_attempt_at'],'duplicate'=>true];}
        ...
        if($jobRecipientId===null&&$recipient>0)$snapshot['recipient_user_id']=$recipient;
        $pdo->prepare('INSERT INTO message_delivery_jobs (public_id,message_event_id,recipient_user_id,merchant_user_id,campaign_id,source_public_id,channel,template_key,status,max_attempts,next_attempt_at,recipient_snapshot_json,payload_snapshot_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')->execute([$jobPublic,$eventId,$jobRecipientId,$merchantId>0?$merchantId:null,$campaignId>0?$campaignId:null,$sourcePublic!==''?$sourcePublic:null,$channel,$template,$status,max(1,min(10,(int)($input['max_attempts']??3))),$nextAttempt,mg_delivery_json($snapshot),mg_delivery_json($payload)]);
        if($failureHook)$failureHook('before_complete',['job_id'=>$jobPublic]);
        if($owns)$pdo->commit();return ['event_id'=>$eventPublic,'job_id'=>$jobPublic,'status'=>$status,'next_attempt_at'=>$nextAttempt,'duplicate'=>false];
    }catch(Throwable $e){if($owns&&$pdo->inTransaction())$pdo->rollBack();throw $e;}
}
function mg_delivery_claim_next(PDO $pdo): ?array
{
    mg_delivery_ensure_schema($pdo);
    $stmt=$pdo->query("SELECT * FROM message_delivery_jobs WHERE status IN ('queued','retrying') AND next_attempt_at<=NOW() ORDER BY next_attempt_at ASC,id ASC LIMIT 1 FOR UPDATE SKIP LOCKED");$job=$stmt->fetch(PDO::FETCH_ASSOC);if(!$job)return null;
...
function mg_delivery_process_job(PDO $pdo,array $job,array $providerResult,?callable $failureHook=null): array
{
    ...
    try{
        ...
        if($kind==='success'){$status='delivered';$pdo->prepare("UPDATE message_delivery_jobs SET status='delivered',attempt_count=?,provider_message_id=?,last_error=NULL,delivered_at=NOW(),updated_at=NOW() WHERE id=?")->execute([$attempt,$providerResult['provider_message_id']??null,(int)$job['id']]);}
        elseif($kind==='transient_failure'&&$attempt<(int)$job['max_attempts']){$status='retrying';$pdo->prepare("UPDATE message_delivery_jobs SET status='retrying',attempt_count=?,last_error=?,next_attempt_at=DATE_ADD(NOW(),INTERVAL ? SECOND),updated_at=NOW() WHERE id=?")->execute([$attempt,$providerResult['error_message']??'Transient failure',max(30,(int)($providerResult['retry_after_seconds']??60)),(int)$job['id']]);}
