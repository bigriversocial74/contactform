...
try{
    ...
    if($accountType==='merchant'){
        ...
        try{
            ...
            $workspaceDb=$pdo->prepare('SELECT id FROM merchant_workspaces WHERE merchant_user_id=? LIMIT 1');$workspaceDb->execute([(int)$result['user_id']]);$workspaceDbId=(int)$workspaceDb->fetchColumn();
    ...
    $redirect=$accountType==='merchant'?'/merchant-onboarding.php':'/inbox.php';
    mg_ok(['user'=>mg_public_user($user),'redirect'=>$redirect,'account_type'=>$accountType,'crm_reward_invites'=>$inviteBridge],'Account created.',201);
