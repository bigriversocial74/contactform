...
if ($method === 'GET') {
    ...
    mg_ok([
        'sources'=>mg_agent_memory_sources($pdo, $merchantId, 50),
...
if ($action === 'upload') {
    ...
    mg_ok(['source'=>$source,'sources'=>mg_agent_memory_sources($pdo, $merchantId, 50),'ai_status'=>mg_merchant_agent_ai_status($pdo, $user, $packageContext)], 'Memory source uploaded.', 201);
}
...
if ($action === 'website') {
    ...
    mg_ok(['source'=>$source,'sources'=>mg_agent_memory_sources($pdo, $merchantId, 50),'ai_status'=>mg_merchant_agent_ai_status($pdo, $user, $packageContext)], 'Website queued for memory scan.', 201);
}
...
if ($action === 'process') {
    ...
    mg_ok(['processed'=>$result['processed'],'remaining'=>$result['remaining'],'sources'=>mg_agent_memory_sources($pdo, $merchantId, 50),'ai_status'=>mg_merchant_agent_ai_status($pdo, $user, $packageContext)], 'Memory source processing complete.');
}
