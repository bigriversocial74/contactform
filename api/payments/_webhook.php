...
{
    if((int)$ids['amount_cents']>0&&(int)$ids['amount_cents']!==(int)$row['total_cents'])throw new MgPaymentWebhookException('Provider payment amount does not match the order.',409);
    if((string)$ids['currency']!==''&&!hash_equals((string)$row['currency'],(string)$ids['currency']))throw new MgPaymentWebhookException('Provider payment currency does not match the order.',409);
    if((int)$ids['application_fee_cents']>0&&(int)$ids['application_fee_cents']!==(int)$row['platform_fee_cents'])throw new MgPaymentWebhookException('Provider application fee does not match the order fee snapshot.',409);
}
...
function mg_payment_process_webhook_event(PDO $pdo,string $provider,array $event,string $payload): array
{
    ...
    if($existing){
        ...
        if(!$same){mg_security_log('critical','payment.webhook_idempotency_conflict','Provider event ID was replayed with a different signed payload.',['provider'=>$provider,'provider_event_id'=>$eventId,'event_type'=>$type]);throw new MgPaymentWebhookException('Webhook event conflicts with an existing provider event.',409);}
    ...
    $processed=false;$ids=mg_payment_webhook_identifiers($provider,$event);$row=null;$stampPurchase=null;
    if(in_array($type,$successTypes,true)){
        if($provider==='stripe'&&$type==='checkout.session.completed'&&$ids['payment_status']!==''&&$ids['payment_status']!=='paid'){$processed=false;}else{
        ...
        $row=mg_payment_webhook_find_order($pdo,$provider,$ids);
        if($row){
            if((string)$row['payment_status']==='paid'||(string)$row['intent_status']==='succeeded'){mg_security_log('warning','payment.webhook_stale_failure','Ignored payment failure received after successful capture.',['provider'=>$provider,'provider_event_id'=>$eventId,'order_id'=>(string)$row['order_id']]);$processed=true;}else{
            ...
            $stampPurchase=mg_payment_webhook_find_stamp_purchase($pdo,$provider,$ids);
            if($stampPurchase){$pdo->prepare("UPDATE payment_intents SET status='failed',failure_code=?,failure_message=?,updated_at=NOW() WHERE id=? AND status<>'succeeded'")->execute([$ids['failure_code']?:null,mb_substr((string)$ids['failure_message'],0,500),(int)$stampPurchase['payment_intent_db_id']]);$pdo->prepare("UPDATE stamp_purchases SET status='failed',updated_at=NOW() WHERE id=? AND status<>'credited'")->execute([(int)$stampPurchase['id']]);$processed=true;}
        }
    }
