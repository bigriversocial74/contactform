...
function mg_finance_refund_order(PDO $pdo, int $merchantUserId, int $actorUserId, array $input, ?callable $failureHook = null): array
{
    ...
    $stmt=$pdo->prepare("SELECT o.id order_db_id,o.public_id,o.buyer_user_id,o.merchant_user_id,o.total_cents,o.currency,o.payment_status,o.metadata_json,pi.id intent_db_id,pi.status intent_status FROM commerce_orders o INNER JOIN payment_intents pi ON pi.order_id=o.id AND pi.status='succeeded' WHERE o.public_id=? AND o.merchant_user_id=? LIMIT 1 FOR UPDATE");
    ...
    if($row=$existing->fetch(PDO::FETCH_ASSOC)){
        ...
        $creatorAffiliate=null;
        if((string)$row['status']==='succeeded'){
            $sum=$pdo->prepare("SELECT COALESCE(SUM(amount_cents),0) FROM payment_refunds WHERE order_id=? AND status='succeeded'");
            $sum->execute([(int)$order['order_db_id']]);
            $totalSucceeded=(int)$sum->fetchColumn();
            $creatorAffiliate=mg_creator_campaign_affiliate_record_refund(
                $pdo,$order,(string)$row['public_id'],(int)$row['amount_cents'],$totalSucceeded,$actorUserId
            );
        }
        return ['refund_id'=>(string)$row['public_id'],'status'=>(string)$row['status'],'duplicate'=>true,'entitlement_policy'=>null,'microgift_reconciliation'=>null,'creator_affiliate'=>$creatorAffiliate];
    }
    ...
    $policy=null;$microgifts=null;$creatorAffiliate=null;$newOrderStatus=(string)$order['payment_status'];
    if($status==='succeeded'){
        ...
        $creatorAffiliate=mg_creator_campaign_affiliate_record_refund($pdo,$order,$refundPublicId,$amountCents,$newTotal,$actorUserId);
        if($failureHook)$failureHook('after_creator_affiliate_refund',['order'=>$order,'refund_id'=>$refundPublicId,'creator_affiliate'=>$creatorAffiliate]);
        $metadata=json_encode(['refund_id'=>$refundPublicId,'amount_cents'=>$amountCents,'reason'=>$reason,'entitlement_policy'=>$policy,'microgift_reconciliation'=>$microgifts,'creator_affiliate'=>$creatorAffiliate],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);
    ...
    return ['refund_id'=>$refundPublicId,'status'=>$status,'duplicate'=>false,'order_status'=>$newOrderStatus,'entitlement_policy'=>$policy,'microgift_reconciliation'=>$microgifts,'creator_affiliate'=>$creatorAffiliate];
}
