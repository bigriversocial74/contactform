...
if ($method === 'PATCH' || $method === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            'UPDATE agents SET name = ?, category = ?, config_json = ?, version_no = version_no + 1, updated_at = NOW()
             WHERE id = ? AND user_id = ?'
        );
        $stmt->execute([$name, $category, $configJson, (int) $agent['id'], (int) $user['id']]);
        $updated = mg_agent_find_owned((int) $user['id'], $id, true);
        if (!$updated) {
            throw new RuntimeException('Updated agent could not be loaded.');
        }
...
if ($method === 'DELETE') {
    ...
    try {
        ...
        $agent = mg_agent_require_owned((int) $user['id'], $id, true);
        $stmt = $pdo->prepare(
            "UPDATE agents SET lifecycle_status = 'deleted', runtime_status = 'paused', deleted_at = NOW(), archived_at = COALESCE(archived_at, NOW()), paused_at = NOW(), version_no = version_no + 1, updated_at = NOW()
             WHERE id = ? AND user_id = ?"
        );
        $stmt->execute([(int) $agent['id'], (int) $user['id']]);
        $deleted = mg_agent_find_owned((int) $user['id'], $id, true);
        if (!$deleted) {
            throw new RuntimeException('Deleted agent could not be loaded.');
        }
