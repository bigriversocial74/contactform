...
function mg_platform_account_subscription_grant_merchant_role(PDO $pdo,int $userId): void
{
    ...
    try{
        ...
        if(function_exists('mg_security_log'))mg_security_log('warning','platform_subscription.role_grant_failed','Unable to grant merchant role for active platform package.',['exception'=>get_class($error)],$userId);
    }
}
...
{
    try{
        $pdo->prepare('INSERT INTO platform_subscription_events (public_id,account_subscription_id,event_type,from_status,to_status,actor_user_id,provider_key,provider_event_id,payload_json,created_at) VALUES (?,?,?,?,?,?,?,?,?,NOW())')
            ->execute([mg_public_uuid(),$accountSubscriptionId,$eventType,$fromStatus,$toStatus,$actorUserId,$payload['provider_key']??null,$payload['provider_event_id']??null,json_encode($payload,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR)]);
    }catch(Throwable $error){
        if(function_exists('mg_security_log'))mg_security_log('warning','platform_subscription.event_failed','Platform subscription event write failed.',['exception'=>$error->getMessage()],$actorUserId);
    }
}
...
function mg_platform_account_subscription_upsert(PDO $pdo,array $requestRow,array $package,array $providerRefs=[]): array
{
    ...
    $amount=(int)($requestRow['amount_cents']??0);
    if($amount<1&&$providerKey!=='admin_grant')$amount=mg_platform_package_amount_cents($package,$billingCycle);
    ...
    if($existing){
        ...
        mg_platform_account_subscription_event($pdo,$subscriptionId,'platform_subscription.package_changed',(string)$existing['status'],'active',$userId,$metadata+['provider_key'=>$providerKey]);
    }else{
        ...
        mg_platform_account_subscription_event($pdo,$subscriptionId,'platform_subscription.activated',null,'active',$userId,$metadata+['provider_key'=>$providerKey]);
    }

    mg_platform_account_subscription_grant_merchant_role($pdo,$userId);
    mg_subscription_provision_merchant_workspace($pdo,$userId,'','platform_subscription_activation');
