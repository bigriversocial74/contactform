...
function mg_subscription_stripe_process_webhook_event(PDO $pdo, array $event, string $payload): array
{
    ...
    if ($eventId === '' || $type === '') {
        throw new MgSubscriptionStripeWebhookException('Invalid Stripe webhook event.', 422);
    }

    $payloadHash = hash('sha256', $payload);
    $existingStmt = $pdo->prepare(
        'SELECT signature_valid,status,payload_hash,event_type
    ...
    if ($existing) {
        ...
        if (!$same) {
            throw new MgSubscriptionStripeWebhookException(
                'Webhook event conflicts with an existing signed payload.',
                409
            );
        }
        ...
        /* Previously ignored package lifecycle events are intentionally retried after
           this canonical dispatcher is deployed. */
        $pdo->prepare(
            "UPDATE payment_webhook_events
             SET status='processing',failure_message=NULL,received_at=NOW()
             WHERE provider_key=? AND provider_event_id=?"
        )->execute(['stripe', $eventId]);
    } else {
        $pdo->prepare(
            "INSERT INTO payment_webhook_events
                (public_id,provider_key,provider_event_id,event_type,signature_valid,status,payload_hash,payload_json,received_at)
             VALUES (?,?,?,?,1,'processing',?,?,NOW())"
        )->execute([mg_public_uuid(), 'stripe', $eventId, $type, $payloadHash, $payload]);
    }

    $activation = mg_subscription_package_webhook_try_process($pdo, 'stripe', $event);
    $processed = is_array($activation) && !empty($activation['processed']);
    $status = $processed ? 'processed' : 'ignored';

    $pdo->prepare(
        'UPDATE payment_webhook_events
         SET status=?,processed_at=NOW(),failure_message=NULL
         WHERE provider_key=? AND provider_event_id=?'
