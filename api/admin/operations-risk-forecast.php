...
function mg_risk_rows(PDO $pdo, string $sql): array { return $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC) ?: []; }
function mg_risk_row(PDO $pdo, string $sql): array { return $pdo->query($sql)->fetch(PDO::FETCH_ASSOC) ?: []; }
function mg_risk_level(int $score): string { if ($score >= 85) { return 'critical'; } if ($score >= 60) { return 'high'; } if ($score >= 35) { return 'medium'; } return 'low'; }
...
function mg_risk_forecast_payload(PDO $pdo): array
{
    ...
    $signals = ['forecasted_sla_breaches'=>(int)($queue['near_sla_breach_total'] ?? 0),'current_sla_breaches'=>(int)($queue['breached_total'] ?? 0),'queue_overload_lanes'=>count($workload),'repeat_incident_modes'=>count($repeat),'overdue_prevention_tasks'=>(int)($reviews['overdue_followups'] ?? 0),'unresolved_aging'=>(int)($queue['unresolved_aging_total'] ?? 0),'automation_stale'=>$automationStale,'active_incidents'=>(int)($incidents['active_incidents'] ?? 0)];
    $riskScore = min(100, min(25, $signals['forecasted_sla_breaches'] * 7) + min(25, $signals['current_sla_breaches'] * 8) + min(15, $signals['queue_overload_lanes'] * 5) + min(15, $signals['repeat_incident_modes'] * 5) + min(15, $signals['overdue_prevention_tasks'] * 5) + min(10, $signals['unresolved_aging'] * 2) + ($automationStale ? 10 : 0) + min(15, $signals['active_incidents'] * 5));
    ...
    return ['risk'=>['score'=>$riskScore,'level'=>mg_risk_level($riskScore),'next_failure_point'=>$nextFailure],'signals'=>$signals,'recommended_actions'=>mg_risk_recommendations($signals),'forecast'=>['sla_breaches_24h'=>$signals['forecasted_sla_breaches'],'queue_overload_lanes'=>array_map(static fn(array $r): array => ['admin_name'=>(string)$r['admin_name'],'active_total'=>(int)$r['active_total'],'critical_total'=>(int)$r['critical_total'],'forecasted_breaches'=>(int)$r['forecasted_breaches']], $workload),'repeat_incident_modes'=>array_map(static fn(array $r): array => ['mode_slug'=>(string)$r['mode_slug'],'total'=>(int)$r['total'],'recent_30'=>(int)$r['recent_30'],'recent_90'=>(int)$r['recent_90']], $repeat),'automation'=>['last_completed_at'=>$lastCompleted ?: null,'stale'=>$automationStale === 1,'recent_failed_runs'=>(int)($automation['recent_failed_runs'] ?? 0)],'prevention'=>['open_followups'=>(int)($reviews['open_followups'] ?? 0),'overdue_followups'=>$signals['overdue_prevention_tasks']],'queue'=>['active_total'=>(int)($queue['active_total'] ?? 0),'overdue_total'=>(int)($queue['overdue_total'] ?? 0),'unassigned_total'=>(int)($queue['unassigned_total'] ?? 0),'unresolved_aging_total'=>$signals['unresolved_aging']]],'score'=>['section'=>'Predictive operations risk forecast','score'=>10,'max'=>10,'status'=>'cleared'],'generated_at'=>gmdate('Y-m-d H:i:s')];
}
