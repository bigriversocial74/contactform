...
if (!$allowed) {
    mg_security_log('warning', 'admin_agent.phase6_stream_denied', 'Main Admin Agent Phase 6 stream permission denied.', [], $actorId);
    http_response_code(403);
    exit;
}

mg_rate_limit('admin.agent.phase6.stream', 'user:' . $actorId, 30, 60);
...
function mg_admin_agent_phase6_sse(string $event, array $payload, ?int $id = null): void
{
...
$environment = mg_admin_agent_phase6_environment((string) ($_GET['environment_key'] ?? 'production'));
echo "retry: 5000\n\n";

if (!mg_admin_agent_schema_ready($pdo)) {
    mg_admin_agent_phase6_sse('schema', ['phase1_ready' => false, 'migration' => 'database/20260718_main_admin_agent_phase1.sql']);
    exit;
}
...
$snapshot['phase6_schema'] = mg_admin_agent_phase6_schema_state($pdo);
$snapshot['phase6_ready'] = mg_admin_agent_phase6_ready($pdo);
$snapshot['generated_at'] = gmdate('Y-m-d H:i:s');
mg_admin_agent_phase6_sse('snapshot', $snapshot);
...
for ($iteration = 0; $iteration < 8; $iteration++) {
    ...
    if ($events !== []) {
        ...
        mg_admin_agent_phase6_sse('events', ['events' => $events, 'cursor' => $after, 'generated_at' => gmdate('Y-m-d H:i:s')], $after);
    } else {
        mg_admin_agent_phase6_sse('heartbeat', ['cursor' => $after, 'scheduler' => mg_admin_agent_phase6_ready($pdo) ? mg_admin_agent_phase6_heartbeat_state($pdo, $environment) : null, 'generated_at' => gmdate('Y-m-d H:i:s')]);
    }
