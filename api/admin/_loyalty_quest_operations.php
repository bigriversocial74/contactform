...
require_once dirname(__DIR__) . '/notifications/_helpers.php';

function mg_lqo_table_ready(PDO $pdo,string $table,array $columns=[]): bool
{
    try{
        $stmt=$pdo->prepare('SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=?');
        $stmt->execute([$table]);$found=array_map('strval',$stmt->fetchAll(PDO::FETCH_COLUMN)?:[]);
...
function mg_lqo_campaigns(PDO $pdo,string $status='all',string $query='',int $limit=100): array
{
    ...
    $query=mb_strtolower(trim($query));$limit=max(1,min(200,$limit));
    ...
    $deliverySelect=$deliveryReady?'COALESCE(d.delivery_failed,0) delivery_failed,COALESCE(d.delivery_stuck,0) delivery_stuck':'0 delivery_failed,0 delivery_stuck';
    $where=["c.campaign_type='loyalty_quest'"];$params=[];
    if($status!=='all'){$where[]='c.status=?';$params[]=$status;}
    if($query!==''){$like='%'.$query.'%';$where[]="(LOWER(c.title) LIKE ? OR LOWER(COALESCE(mw.display_name,pp.display_name,u.display_name,u.full_name,u.email,'')) LIKE ? OR LOWER(c.public_id) LIKE ?)";array_push($params,$like,$like,$like);}
    $sql="SELECT c.id,c.public_id,c.public_slug,c.title,c.status,c.starts_at,c.ends_at,c.issued_count,c.claimed_count,c.redeemed_count,c.updated_at,c.merchant_user_id,
          ...
          LEFT JOIN (
              ...
              FROM loyalty_quest_participations GROUP BY campaign_id,merchant_user_id
          ...
          LEFT JOIN (
              ...
              FROM loyalty_quest_evidence GROUP BY campaign_id,merchant_user_id
          ...
          LEFT JOIN (
              ...
              FROM wallet_items WHERE source_type='loyalty_quest' AND status<>'cancelled' GROUP BY campaign_id,merchant_user_id
    ...
    foreach($stmt->fetchAll(PDO::FETCH_ASSOC)?:[] as $row){
        $items[]=[
            'id'=>(string)$row['public_id'],'slug'=>$row['public_slug']??null,'title'=>(string)$row['title'],'status'=>(string)$row['status'],'starts_at'=>$row['starts_at']??null,'ends_at'=>$row['ends_at']??null,'updated_at'=>$row['updated_at']??null,
            'merchant'=>['id'=>(int)$row['merchant_user_id'],'name'=>(string)$row['merchant_name']],
            'participants'=>(int)$row['participants'],'pending_review'=>(int)$row['pending_review'],'completed'=>(int)$row['completed'],'submitted_evidence'=>(int)$row['submitted_evidence'],'oldest_submitted_at'=>$row['oldest_submitted_at']??null,
            'inbox_delivered'=>(int)$row['inbox_delivered'],'claimed'=>(int)$row['claimed_count'],'redeemed'=>(int)$row['rewards_redeemed'],'delivery_failed'=>(int)$row['delivery_failed'],'delivery_stuck'=>(int)$row['delivery_stuck'],
            'actions'=>[
...
function mg_lqo_evidence_queue(PDO $pdo,int $limit=100): array
{
    ...
    $sql="SELECT lqe.id,lqe.public_id,lqe.evidence_type,lqe.created_at,lqp.public_id participation_public_id,lqp.contact_id,
          ...
          ORDER BY lqe.created_at ASC,lqe.id ASC LIMIT {$limit}";
    $rows=$pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC)?:[];$items=[];
    foreach($rows as $row)$items[]=[
        'id'=>(string)$row['public_id'],'type'=>(string)$row['evidence_type'],'created_at'=>$row['created_at'],'age_hours'=>(int)$row['age_hours'],'participation_id'=>(string)$row['participation_public_id'],
...
function mg_lqo_delivery_queue(PDO $pdo,int $limit=100): array
{
    ...
    $sql="SELECT j.id,j.public_id,j.status,j.attempt_count,j.max_attempts,j.next_attempt_at,j.last_error,j.recipient_snapshot_json,j.created_at,j.updated_at,
          ...
          ORDER BY CASE WHEN j.status='processing' THEN 0 WHEN j.status='dead_letter' THEN 1 ELSE 2 END,j.updated_at ASC,j.id ASC LIMIT {$limit}";
    $rows=$pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC)?:[];$items=[];
    foreach($rows as $row){
        $recipient=json_decode((string)($row['recipient_snapshot_json']??''),true);if(!is_array($recipient))$recipient=[];
        $items[]=[
            'id'=>(string)$row['public_id'],'status'=>(string)$row['status'],'attempt_count'=>(int)$row['attempt_count'],'max_attempts'=>(int)$row['max_attempts'],'next_attempt_at'=>$row['next_attempt_at']??null,'last_error'=>$row['last_error']??null,'created_at'=>$row['created_at']??null,'updated_at'=>$row['updated_at']??null,
            'recipient'=>['name'=>(string)($recipient['name']??''),'email_masked'=>mg_lqo_mask_email((string)($recipient['email']??''))],
            'campaign'=>['id'=>(string)$row['campaign_public_id'],'title'=>(string)$row['campaign_title']],
            'merchant'=>['id'=>(int)$row['merchant_user_id'],'name'=>(string)$row['merchant_name']],
...
{
    $summary=['campaigns'=>count($campaigns),'active'=>0,'paused'=>0,'pending_review'=>count($evidence),'evidence_over_24h'=>0,'delivery_failures'=>count($deliveries),'stale_processing'=>0,'participants'=>0,'completed'=>0,'inbox_delivered'=>0,'redeemed'=>0];
...
function mg_lqo_recent_events(PDO $pdo,int $limit=50): array
{
    ...
    $stmt=$pdo->query("SELECT ce.public_id,ce.event_type,ce.event_context_json,ce.created_at,c.public_id campaign_public_id,c.title campaign_title,c.merchant_user_id
                      FROM campaign_events ce INNER JOIN campaigns c ON c.id=ce.campaign_id AND c.merchant_user_id=ce.merchant_user_id
    ...
    $items=[];foreach($stmt->fetchAll(PDO::FETCH_ASSOC)?:[] as $row){$context=json_decode((string)($row['event_context_json']??''),true);if(!is_array($context))$context=[];$items[]=['id'=>(string)$row['public_id'],'event_type'=>(string)$row['event_type'],'created_at'=>$row['created_at'],'campaign'=>['id'=>(string)$row['campaign_public_id'],'title'=>(string)$row['campaign_title']],'operator_user_id'=>(int)($context['operator_user_id']??0),'reason'=>(string)($context['reason']??'')];}
    return $items;
}
...
{
    $stmt=$pdo->prepare("SELECT * FROM campaigns WHERE public_id=? AND campaign_type='loyalty_quest' LIMIT 1 FOR UPDATE");$stmt->execute([$campaignRef]);$campaign=$stmt->fetch(PDO::FETCH_ASSOC);
    if(!$campaign)throw new RuntimeException('Loyalty Quest not found.');
    $old=(string)$campaign['status'];$new=$old;
    if($action==='pause'){
        if(!in_array($old,['active','scheduled'],true))throw new DomainException('Only active or scheduled quests can be paused.');
        $new='paused';$pdo->prepare("UPDATE campaigns SET status='paused',updated_at=NOW() WHERE id=?")->execute([(int)$campaign['id']]);
        ...
        $new='completed';$pdo->prepare("UPDATE campaigns SET status='completed',ends_at=CASE WHEN ends_at IS NULL OR ends_at>NOW() THEN NOW() ELSE ends_at END,updated_at=NOW() WHERE id=?")->execute([(int)$campaign['id']]);
    }else throw new InvalidArgumentException('Invalid campaign action.');
    $eventType='quest.admin_'.$action.'d';
    $eventId=mg_lqo_campaign_event($pdo,(int)$campaign['merchant_user_id'],(int)$campaign['id'],null,$eventType,['operator_user_id'=>$adminId,'reason'=>$reason,'old_status'=>$old,'new_status'=>$new]);
    mg_audit('admin.loyalty_quest_'.$action.'d','campaign',['campaign_id'=>$campaignRef,'merchant_user_id'=>(int)$campaign['merchant_user_id'],'old_status'=>$old,'new_status'=>$new,'reason'=>$reason,'campaign_event_id'=>$eventId],$adminId);
...
function mg_lqo_review_nudge(PDO $pdo,int $adminId,string $evidenceRef,string $reason): array
{
    $stmt=$pdo->prepare("SELECT lqe.id,lqe.public_id,lqe.created_at,lqp.contact_id,lqp.public_id participation_public_id,c.id campaign_id,c.public_id campaign_public_id,c.title,c.merchant_user_id
                        ...
                        INNER JOIN loyalty_quest_participations lqp ON lqp.id=lqe.participation_id AND lqp.merchant_user_id=lqe.merchant_user_id
                        INNER JOIN campaigns c ON c.id=lqe.campaign_id AND c.merchant_user_id=lqe.merchant_user_id AND c.campaign_type='loyalty_quest'
                        WHERE lqe.public_id=? AND lqe.status='submitted' LIMIT 1 FOR UPDATE");$stmt->execute([$evidenceRef]);$row=$stmt->fetch(PDO::FETCH_ASSOC);
    if(!$row)throw new RuntimeException('Submitted Loyalty Quest evidence not found.');
    $cooldown=$pdo->prepare("SELECT COUNT(*) FROM campaign_events WHERE campaign_id=? AND merchant_user_id=? AND event_type='quest.admin_review_nudged' AND created_at>=DATE_SUB(NOW(),INTERVAL 12 HOUR)");$cooldown->execute([(int)$row['campaign_id'],(int)$row['merchant_user_id']]);
    if((int)$cooldown->fetchColumn()>0)throw new DomainException('This merchant was already nudged within the last 12 hours.');
    $eventId=mg_lqo_campaign_event($pdo,(int)$row['merchant_user_id'],(int)$row['campaign_id'],(int)$row['contact_id'],'quest.admin_review_nudged',['operator_user_id'=>$adminId,'reason'=>$reason,'evidence_id'=>$evidenceRef,'participation_id'=>(string)$row['participation_public_id']]);
    if(mg_lqo_table_ready($pdo,'operational_alerts',['user_id','alert_type','status'])){
        mg_create_operational_alert($pdo,(int)$row['merchant_user_id'],'loyalty_quest_review_backlog','warning','Loyalty Quest evidence needs review','An administrator flagged submitted evidence that has remained in the review queue.','/merchant-quest-reviews.php?campaign='.rawurlencode((string)$row['campaign_public_id']),['merchant_user_id'=>(int)$row['merchant_user_id'],'metadata'=>['evidence_id'=>$evidenceRef,'campaign_id'=>(string)$row['campaign_public_id']]]);
    }
    mg_audit('admin.loyalty_quest_review_nudged','loyalty_quest_evidence',['evidence_id'=>$evidenceRef,'campaign_id'=>(string)$row['campaign_public_id'],'merchant_user_id'=>(int)$row['merchant_user_id'],'reason'=>$reason,'campaign_event_id'=>$eventId],$adminId);
...
function mg_lqo_retry_delivery(PDO $pdo,int $adminId,string $jobRef,string $reason): array
{
    ...
    $stmt=$pdo->prepare("SELECT j.*,c.public_id campaign_public_id,c.merchant_user_id campaign_merchant_id
                        ...
                        INNER JOIN message_events me ON me.id=j.message_event_id AND me.event_type LIKE 'loyalty_quest.%'
                        INNER JOIN campaigns c ON c.id=j.campaign_id AND c.merchant_user_id=j.merchant_user_id AND c.campaign_type='loyalty_quest'
                        WHERE j.public_id=? LIMIT 1 FOR UPDATE");$stmt->execute([$jobRef]);$job=$stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $pdo->prepare("UPDATE message_delivery_jobs SET status='queued',max_attempts=?,next_attempt_at=NOW(),last_error=NULL,failed_at=NULL,updated_at=NOW() WHERE id=?")->execute([$maxAttempts,(int)$job['id']]);
    $eventId=mg_lqo_campaign_event($pdo,(int)$job['merchant_user_id'],(int)$job['campaign_id'],null,'quest.admin_delivery_retried',['operator_user_id'=>$adminId,'reason'=>$reason,'delivery_job_id'=>$jobRef,'old_status'=>(string)$job['status'],'attempt_count'=>(int)$job['attempt_count'],'max_attempts'=>$maxAttempts]);
    mg_audit('admin.loyalty_quest_delivery_retried','message_delivery_job',['job_id'=>$jobRef,'campaign_id'=>(string)$job['campaign_public_id'],'merchant_user_id'=>(int)$job['merchant_user_id'],'reason'=>$reason,'campaign_event_id'=>$eventId],$adminId);
