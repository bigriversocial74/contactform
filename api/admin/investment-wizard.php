...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($workspaceId !== '') {
            mg_ok(['workspace' => mg_investment_workspace_detail($pdo, $workspaceId)], 'Investment workspace loaded.');
        }
    ...
    $result = match ($action) {
        ...
        'save_scenario' => mg_investment_save_scenario($pdo, $actor, $input),
        'clone_scenario' => mg_investment_clone_scenario($pdo, $actor, mg_investment_text($input['scenario_id'] ?? '', 36, 36, 'Scenario identifier')),
        'save_budget' => mg_investment_replace_budget($pdo, $actor, $input),
        'save_goals' => mg_investment_replace_goals($pdo, $actor, $input),
        'save_metrics' => mg_investment_save_metrics($pdo, $actor, $input),
        'save_documents' => mg_investment_save_documents($pdo, $actor, $input),
        'adopt_round' => mg_investment_adopt_round($pdo, $actor, $input),
        'update_round' => mg_investment_update_round($pdo, $actor, $input),
