...
function mg_content_review_subject(PDO $pdo, array $report): array
{
    ...
    if (($type === 'profile' || $type === 'user') && !empty($report['subject_user_id'])) {
        $stmt = $pdo->prepare(
            'SELECT u.id,u.public_id,u.display_name,u.full_name,u.email,u.status,u.created_at,u.updated_at,
                    pp.public_id profile_id,pp.slug,pp.display_name profile_name,pp.headline,pp.biography,
    ...
    if ($type === 'post' && !empty($report['feed_post_id'])) {
        ...
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $row['media'] = mg_content_review_json($row['media_json'] ?? null);
            unset($row['media_json']);
            return ['type'=>'post','available'=>true,'post'=>$row];
        }
    }
    ...
    if ($type === 'media' && !empty($report['asset_id'])) {
        $stmt = $pdo->prepare(
            'SELECT a.id,a.public_id,a.owner_user_id,a.asset_type,a.original_filename,a.mime_type,a.byte_size,
             ...
             LEFT JOIN feed_posts fp ON fp.id=fpa.feed_post_id
        ...
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $row['preview_url'] = mg_storage_asset_public_url((string)$row['public_id']);
            return ['type'=>'media','available'=>true,'media'=>$row];
        }
    }
    ...
    if ($type === 'message' && !empty($report['message_id'])) {
        $stmt = $pdo->prepare(
            'SELECT m.id,m.public_id,m.sender_user_id,m.body,m.moderation_status,m.created_at,m.updated_at,
                    mt.public_id thread_id,mt.subject thread_subject,
                    COALESCE(NULLIF(u.display_name,\'\'),u.full_name,u.email) sender_name
    ...
    return [
        'type'=>$type,
        'available'=>false,
        'snapshot'=>mg_content_review_json($report['subject_snapshot_json'] ?? null),
    ];
}

function mg_content_review_account(PDO $pdo, int $userId): ?array
{
    ...
    foreach ([
        ...
        $counter->execute([$userId]);
        $counts[$key] = (int)$counter->fetchColumn();
    }

    $restrictions = [];
    if (mg_content_review_table_exists($pdo, 'user_moderation_restrictions')) {
        $restrictionStmt = $pdo->prepare(
        ...
        $restrictions = $restrictionStmt->fetchAll(PDO::FETCH_ASSOC);
    }

    return ['user'=>$user,'counts'=>$counts,'restrictions'=>$restrictions];
}

function mg_content_review_history(PDO $pdo, int $reportId): array
{
    $stmt = $pdo->prepare(
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $items[] = [
            ...
            'metadata'=>mg_content_review_json($row['metadata_json'] ?? null),
