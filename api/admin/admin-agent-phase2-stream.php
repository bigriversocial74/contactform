...
if(!$allowed){
    mg_security_log('warning','admin_agent.phase2_stream_denied','Main Admin Agent Phase 2 stream permission denied.',[],$actorId);
    http_response_code(403);
    exit;
}

mg_rate_limit('admin.agent.phase2.stream','user:'.$actorId,30,60);
...
function mg_admin_agent_phase2_sse(string $event,array $payload,?int $id=null):void
{
...
if(!mg_admin_agent_schema_ready($pdo)){
    mg_admin_agent_phase2_sse('schema',['phase1_ready'=>false,'migration'=>'database/20260718_main_admin_agent_phase1.sql']);
    exit;
}
...
mg_admin_agent_phase2_sse('snapshot',$snapshot);
...
for($iteration=0;$iteration<8;$iteration++){
    ...
    if($events!==[]){
        ...
        mg_admin_agent_phase2_sse('events',['events'=>$events,'cursor'=>$after,'generated_at'=>gmdate('Y-m-d H:i:s')],$after);
    }else{
        mg_admin_agent_phase2_sse('heartbeat',['cursor'=>$after,'generated_at'=>gmdate('Y-m-d H:i:s')]);
    }
