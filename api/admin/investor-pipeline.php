...
try {
    ...
    if ($method==='GET') {
        ...
        mg_ok($result,'Investor Pipeline data loaded.');
    }

    if ($method!=='POST') mg_fail('Method not allowed.',405);

    mg_investment_require_permission($actor,'admin.investor_pipeline.manage');
    mg_rate_limit('admin.investor_pipeline.write','user:'.$actorId,180,60);
    $input=mg_input();
    ...
    header('Cache-Control: private, no-store, max-age=0');
    mg_ok($result,'Investor Pipeline changes saved.');
} catch (MgInvestmentException $error) {
    mg_fail($error->getMessage(),$error->httpStatus());
} catch (Throwable $error) {
    mg_fail_unexpected($error,'admin.investor_pipeline.failed','Unable to update Investor Pipeline operations.',500,[],$actorId);
}
