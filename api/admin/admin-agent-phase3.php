...
function mg_admin_agent_phase3_api_has(array $actor,string $permission): bool
{
...
function mg_admin_agent_phase3_api_require(array $actor,string $permission): void
{
    if(mg_admin_agent_phase3_api_has($actor,$permission)) return;
    mg_audit('permission_denied','security',['permission'=>$permission,'area'=>'main_admin_agent_phase3'],(int)$actor['id']);
    mg_security_log('warning','admin_agent.phase3_permission_denied','Main Admin Agent Phase 3 permission denied.',['permission'=>$permission],(int)$actor['id']);
    mg_fail('Permission denied.',403);
}

function mg_admin_agent_phase3_api_options(array $source): array
{
    return [
...
function mg_admin_agent_phase3_api_permissions(array $actor): array
{
    return [
        'chat'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.chat'),
        'manage'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.manage'),
        'actions'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.actions'),
        'escalations'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.escalations'),
        'deployments'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.deployments'),
        'incidents'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.incidents'),
        'releases'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.releases'),
        'briefs'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.briefs'),
        'execute'=>mg_admin_agent_phase3_api_has($actor,'admin.admin_agent.execute'),
    ];
}

function mg_admin_agent_phase3_api_state(PDO $pdo,int $actorId,array $options,array $actor): array
{
    $payload=mg_admin_agent_phase3_ready($pdo)?mg_admin_agent_phase3_state($pdo,$actorId,$options):(mg_admin_agent_phase2_ready($pdo)?mg_admin_agent_phase2_state($pdo,$actorId,$options):mg_admin_agent_state_runtime($pdo,$actorId,$options));
    $payload['phase3_schema']=mg_admin_agent_phase3_schema_state($pdo);
    $payload['phase3_ready']=mg_admin_agent_phase3_ready($pdo);
    $payload['permissions']=mg_admin_agent_phase3_api_permissions($actor);
    return $payload;
}

try{
    if($method==='GET'){
        mg_rate_limit('admin.agent.phase3.read','user:'.$actorId,240,60);
        mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.view');
        ...
        $payload=mg_admin_agent_phase3_api_state($pdo,$actorId,mg_admin_agent_phase3_api_options($_GET),$actor);
        header('Cache-Control: private, no-store, max-age=0'); header('Vary: Cookie, Authorization');
        mg_ok($payload,'Main Admin Agent Phase 3 loaded.');
    }

    if($method==='POST'){
        mg_rate_limit('admin.agent.phase3.write','user:'.$actorId,90,60);
        $input=mg_input(); mg_require_csrf_for_write($input); mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.view');
        ...
        if($action==='send_message'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.chat');
            ...
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.manage'); $result=mg_admin_agent_apply_finding_action_runtime($pdo,$actorId,$input);
        }elseif($action==='request_action'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.actions');
            ...
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.deployments');
            ...
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.escalations');
            if(!mg_admin_agent_phase2_ready($pdo)) mg_fail('Main Admin Agent Phase 2 SQL migration is required.',409,['schema'=>mg_admin_agent_phase2_schema_state($pdo)]);
            $result=mg_admin_agent_phase2_acknowledge_escalation($pdo,$actorId,$input);
        }elseif($action==='review_action'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.execute');
            if(!mg_admin_agent_phase2_ready($pdo)) mg_fail('Main Admin Agent Phase 2 SQL migration is required.',409,['schema'=>mg_admin_agent_phase2_schema_state($pdo)]);
            ...
            $result=mg_admin_agent_phase3_execute_action($pdo,$actorId,$input);
        }elseif($action==='incident_workspace_action'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.incidents');
            if(!mg_admin_agent_phase3_ready($pdo)) mg_fail('Main Admin Agent Phase 3 SQL migration is required.',409,['schema'=>mg_admin_agent_phase3_schema_state($pdo)]);
            $result=mg_admin_agent_phase3_incident_action($pdo,$actorId,$input);
        }elseif($action==='evaluate_release'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.releases');
            if(!mg_admin_agent_phase3_ready($pdo)) mg_fail('Main Admin Agent Phase 3 SQL migration is required.',409,['schema'=>mg_admin_agent_phase3_schema_state($pdo)]);
            $result=mg_admin_agent_phase3_evaluate_release($pdo,(string)($input['environment_key']??'production'));
        }elseif($action==='update_brief'){
            mg_admin_agent_phase3_api_require($actor,'admin.admin_agent.briefs');
            if(!mg_admin_agent_phase3_ready($pdo)) mg_fail('Main Admin Agent Phase 3 SQL migration is required.',409,['schema'=>mg_admin_agent_phase3_schema_state($pdo)]);
            $result=mg_admin_agent_phase3_update_brief($pdo,$actorId,$input);
        }else throw new InvalidArgumentException('Unknown Main Admin Agent Phase 3 action.');
        $options=mg_admin_agent_phase3_api_options($input);
        ...
        $state=mg_admin_agent_phase3_api_state($pdo,$actorId,$options,$actor);
        header('Cache-Control: private, no-store, max-age=0'); header('Vary: Cookie, Authorization');
