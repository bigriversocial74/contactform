...
if (!$allowed) {
    mg_security_log('warning', 'admin_agent.phase5_stream_denied', 'Main Admin Agent Phase 5 stream permission denied.', [], $actorId);
    http_response_code(403);
    exit;
}

mg_rate_limit('admin.agent.phase5.stream', 'user:' . $actorId, 30, 60);
...
function mg_admin_agent_phase5_sse(string $event, array $payload, ?int $id = null): void
{
...
$environment = mg_admin_agent_phase5_clean_environment((string) ($_GET['environment_key'] ?? 'production'));
echo "retry: 5000\n\n";

if (!mg_admin_agent_schema_ready($pdo)) {
    mg_admin_agent_phase5_sse('schema', ['phase1_ready' => false, 'migration' => 'database/20260718_main_admin_agent_phase1.sql']);
    exit;
}

$options = ['after' => $after, 'event_limit' => 100, 'domain' => $domain, 'finding_status' => 'active', 'environment_key' => $environment];
if (mg_admin_agent_phase5_ready($pdo)) $snapshot = mg_admin_agent_phase5_state($pdo, $actorId, $options);
...
$snapshot['phase5_schema'] = mg_admin_agent_phase5_schema_state($pdo);
$snapshot['phase5_ready'] = mg_admin_agent_phase5_ready($pdo);
$snapshot['generated_at'] = gmdate('Y-m-d H:i:s');
mg_admin_agent_phase5_sse('snapshot', $snapshot);
...
for ($iteration = 0; $iteration < 8; $iteration++) {
    ...
    if ($events !== []) {
        ...
        mg_admin_agent_phase5_sse('events', ['events' => $events, 'cursor' => $after, 'generated_at' => gmdate('Y-m-d H:i:s')], $after);
    } else {
        mg_admin_agent_phase5_sse('heartbeat', ['cursor' => $after, 'generated_at' => gmdate('Y-m-d H:i:s')]);
    }
