...
try {
    ...
    $ins = $pdo->prepare("INSERT INTO crm_reward_invites (public_id,merchant_user_id,campaign_id,contact_id,reward_template_id,email,name,status,note,idempotency_key,invite_url,sent_at,expires_at,metadata_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,'sent',?,?,?,?,?,NOW(),NOW())");
    $ins->execute([$inviteId, $merchantId, (int)$contact['campaign_id'], (int)$contact['id'], (int)$template['id'], $email, (string)($contact['name'] ?? ''), $note, $idem, $inviteUrl, date('Y-m-d H:i:s'), $inviteExpiresAt, json_encode($meta, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)]);
    $delivery = null;
    try {
        $delivery = mg_delivery_enqueue($pdo, ['idempotency_key' => 'crm-reward-invite-email:' . $idem, 'event_type' => 'campaign.outbound_email', 'category' => 'campaign', 'channel' => 'email', 'template_key' => 'campaign.crm_reward_invite', 'recipient_snapshot' => ['email' => $email, 'name' => (string)($contact['name'] ?? '')], 'payload' => mg_crm_reward_invite_email_payload($contact, $template, $inviteId, $inviteUrl, $note, $inviteExpiresAt), 'max_attempts' => 3]);
