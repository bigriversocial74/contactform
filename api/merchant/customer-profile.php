...
function mg_cp_date(?string $value): ?string { return $value ?: null; }
function mg_cp_label(string $value): string { $value=str_replace(['_','-'], ' ', $value);return ucwords($value); }
function mg_cp_initials(string $name,string $email=''): string { $base=trim($name)?:trim($email);$parts=preg_split('/\s+/', $base)?:[];$out='';foreach($parts as $p){$out.=mb_substr((string)$p,0,1);if(mb_strlen($out)>=2)break;}return strtoupper($out?:'C'); }
...
function mg_cp_ensure_crm_contact(PDO $pdo,int $merchantId,array $cc): array
{
    ...
    $record=mg_merchant_crm_record_event($pdo,['merchant_user_id'=>$merchantId,'campaign_id'=>(int)$cc['campaign_id'],'campaign_type'=>(string)($cc['source']??'campaign_contact'),'event_type'=>'customer_profile.created_from_campaign_contact','source_type'=>(string)($cc['source']??'campaign_contact'),'source_public_id'=>(string)$cc['public_id'],'user_id'=>$userId?:null,'email'=>$email,'phone'=>(string)($cc['phone']??''),'name'=>(string)($cc['name']??''),'metadata'=>['campaign_contact_id'=>(string)$cc['public_id']]]);
...
{
    $ids=[];$email=strtolower((string)($contact['primary_email']??''));$userId=(int)($contact['user_id']??0);
    if($email!==''||$userId>0){
        $where=[];$params=[$merchantId];
        if($email!==''){$where[]='email=?';$params[]=$email;}
        if($userId>0){$where[]='user_id=?';$params[]=$userId;}
        $stmt=$pdo->prepare('SELECT public_id FROM campaign_contacts WHERE merchant_user_id=? AND ('.implode(' OR ',$where).')');
        $stmt->execute($params);
...
function mg_cp_contact_db_ids(PDO $pdo,int $merchantId,array $contact): array
{
    ...
    if($email!==''||$userId>0){
        ...
        if($email!==''){$where[]='email=?';$params[]=$email;}
        if($userId>0){$where[]='user_id=?';$params[]=$userId;}
        $stmt=$pdo->prepare('SELECT id FROM campaign_contacts WHERE merchant_user_id=? AND ('.implode(' OR ',$where).')');
        $stmt->execute($params);
...
{
    $userId=(int)($contact['user_id']??0);$where=['merchant_user_id=?'];$params=[$merchantId];$sub=[];
    if($userId>0){$sub[]='user_id=?';$params[]=$userId;}
    if($contactDbIds){$sub[]='contact_id IN ('.implode(',',array_fill(0,count($contactDbIds),'?')).')';array_push($params,...$contactDbIds);} if(!$sub)return ['total'=>0,'claimed'=>0,'redeemed'=>0,'open'=>0,'open_value_cents'=>0,'value_cents'=>0,'currency'=>'USD','last_reward'=>null];
...
function mg_cp_wallet_items(PDO $pdo,int $merchantId,array $contact,array $contactDbIds,int $limit=10): array
{
    ...
    if($contactDbIds){$sub[]='wi.contact_id IN ('.implode(',',array_fill(0,count($contactDbIds),'?')).')';array_push($params,...$contactDbIds);} if(!$sub)return [];
    $where[]='('.implode(' OR ',$sub).')';
    ...
    $stmt->execute($params);
    return array_map(function($r){return ['id'=>(string)$r['public_id'],'title'=>(string)$r['title_snapshot'],'campaign'=>(string)($r['campaign_title']??''),'campaign_id'=>(string)($r['campaign_public_id']??''),'campaign_type'=>(string)($r['campaign_type']??''),'status'=>(string)$r['status'],'value'=>mg_cp_money((int)$r['value_cents_snapshot'],(string)$r['currency_snapshot']),'sent_at'=>$r['issued_at']??null,'claimed_at'=>$r['claimed_at']??null,'redeemed_at'=>$r['redeemed_at']??null,'expires_at'=>$r['expires_at']??null];},$stmt->fetchAll(PDO::FETCH_ASSOC));
}

function mg_cp_messages(PDO $pdo,int $merchantId,array $contact,array $contactPublicIds,int $limit=6): array
{
    if(!$contactPublicIds)return [];$in=implode(',',array_fill(0,count($contactPublicIds),'?'));
    $sql="SELECT m.public_id,m.body,m.sender_user_id,m.created_at,mt.public_id thread_public_id,COALESCE(NULLIF(u.display_name,''),NULLIF(u.full_name,''),u.email,'Microgifter user') sender_name FROM message_threads mt INNER JOIN messages m ON m.thread_id=mt.id LEFT JOIN users u ON u.id=m.sender_user_id WHERE mt.conversation_key LIKE 'crm:%' AND SUBSTRING_INDEX(SUBSTRING(mt.conversation_key,5),':',1) IN ({$in}) ORDER BY m.created_at DESC,m.id DESC LIMIT {$limit}";
    $stmt=$pdo->prepare($sql);$stmt->execute($contactPublicIds);
    return array_map(fn($r)=>['id'=>(string)$r['public_id'],'thread_id'=>(string)$r['thread_public_id'],'body'=>(string)$r['body'],'sender_name'=>(string)$r['sender_name'],'mine'=>(int)$r['sender_user_id']===$merchantId,'created_at'=>$r['created_at']??null],$stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function mg_cp_notes(PDO $pdo,int $merchantId,int $crmContactId,int $limit=5): array
{
    ...
    return array_map(fn($r)=>['id'=>(string)$r['public_id'],'note'=>(string)$r['note'],'author'=>(string)$r['author_name'],'created_at'=>$r['created_at']??null,'updated_at'=>$r['updated_at']??null],$stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function mg_cp_campaign_sources(PDO $pdo,int $merchantId,int $crmContactId): array
{
    ...
    $stmt->execute([$merchantId,$crmContactId]);
    return array_map(fn($r)=>['campaign'=>(string)($r['campaign_title']?:mg_cp_label((string)$r['campaign_type'])),'campaign_id'=>(string)($r['campaign_public_id']??''),'type'=>(string)$r['campaign_type'],'first_seen'=>$r['first_event_at']??null,'last_seen'=>$r['last_event_at']??null,'interactions'=>(int)$r['event_count']],$stmt->fetchAll(PDO::FETCH_ASSOC));
}

function mg_cp_redemptions(PDO $pdo,int $merchantId,array $contact,int $limit=8): array
{
    $userId=(int)($contact['user_id']??0); if($userId<1)return [];
    ...
    $stmt->execute([$merchantId,$userId]);
    return array_map(fn($r)=>['id'=>(string)$r['public_id'],'gift_id'=>(string)$r['gift_public_id'],'location'=>(string)($r['location_name']??''),'amount'=>mg_cp_money((int)$r['amount_cents'],(string)$r['currency']),'status'=>(string)$r['status'],'redeemed_at'=>$r['redeemed_at']??null],$stmt->fetchAll(PDO::FETCH_ASSOC));
}

function mg_cp_events(PDO $pdo,int $merchantId,int $crmContactId,array $contactDbIds,array $wallets,array $messages,array $redemptions,array $notes): array
{
    $events=[];
    ...
    $stmt->execute([$merchantId,$crmContactId]);
    foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $r)$events[]=['type'=>(string)$r['event_type'],'title'=>mg_cp_label((string)$r['event_type']),'body'=>mg_cp_label((string)$r['campaign_type']).' · '.mg_cp_label((string)$r['source_type']),'at'=>$r['created_at']??null,'icon'=>'➤','tone'=>'is-blue'];
    foreach($wallets as $w){$events[]=['type'=>'reward','title'=>'Reward '.mg_cp_label((string)$w['status']),'body'=>(string)$w['title'],'at'=>$w['sent_at']??null,'icon'=>'🎁','tone'=>'is-blue']; if(!empty($w['claimed_at']))$events[]=['type'=>'claim','title'=>'Reward Claimed','body'=>(string)$w['title'],'at'=>$w['claimed_at'],'icon'=>'✓','tone'=>'is-green']; if(!empty($w['redeemed_at']))$events[]=['type'=>'redeem','title'=>'Reward Redeemed','body'=>(string)$w['title'],'at'=>$w['redeemed_at'],'icon'=>'✓','tone'=>'is-green'];}
    foreach($messages as $m)$events[]=['type'=>'message','title'=>!empty($m['mine'])?'Merchant Replied':'Customer Sent Message','body'=>(string)$m['body'],'at'=>$m['created_at']??null,'icon'=>!empty($m['mine'])?'↩':'💬','tone'=>!empty($m['mine'])?'is-blue':'is-purple'];
    foreach($redemptions as $r)$events[]=['type'=>'redemption','title'=>'Reward Redeemed','body'=>trim(($r['amount']??'').' · '.($r['location']??''),' ·'),'at'=>$r['redeemed_at']??null,'icon'=>'✓','tone'=>'is-green'];
    foreach($notes as $n)$events[]=['type'=>'note','title'=>'CRM Note Added','body'=>(string)$n['note'],'at'=>$n['created_at']??null,'icon'=>'✎','tone'=>'is-orange'];
...
{
    $contactDbIds=mg_cp_contact_db_ids($pdo,$merchantId,$contact);$contactPublicIds=mg_cp_contact_public_ids($pdo,$merchantId,$contact);
    $walletStats=mg_cp_wallet_stats($pdo,$merchantId,$contact,$contactDbIds);$wallets=mg_cp_wallet_items($pdo,$merchantId,$contact,$contactDbIds,12);$messages=mg_cp_messages($pdo,$merchantId,$contact,$contactPublicIds,8);$notes=mg_cp_notes($pdo,$merchantId,(int)$contact['id'],6);$campaigns=mg_cp_campaign_sources($pdo,$merchantId,(int)$contact['id']);$redemptions=mg_cp_redemptions($pdo,$merchantId,$contact,8);
    $tags=mg_cp_json($contact['tags_json']??null);if(!$tags)$tags=array_values(array_filter([(string)($contact['lifecycle_stage']??''),(string)($contact['last_campaign_type']??'')]));
    $claimed=max(0,$walletStats['claimed']);$total=max(0,$walletStats['total']);$redemptionRate=$total>0?(int)round(($claimed/$total)*100):0;$tipCents=0;$tipCount=0;
    ...
    return ['customer'=>['id'=>(string)$contact['public_id'],'user_id'=>(int)($contact['user_id']??0),'name'=>(string)($contact['display_name']?:'Customer'),'email'=>(string)($contact['primary_email']??''),'phone'=>(string)($contact['primary_phone']??''),'initials'=>mg_cp_initials((string)($contact['display_name']??''),(string)($contact['primary_email']??'')),'status'=>(string)($contact['crm_status']??'active'),'stage'=>(string)($contact['lifecycle_stage']??'lead'),'tags'=>$tags,'first_seen_at'=>$contact['first_seen_at']??null,'last_activity_at'=>$latest,'source_campaign'=>mg_cp_label((string)($contact['last_campaign_type']??'unknown')),'preferred_location'=>$redemptions[0]['location']??'—'],
        ...
        'messages'=>$messages,'rewards'=>$wallets,'tips'=>[],'campaign_sources'=>$campaigns,'notes'=>$notes,'redemptions'=>$redemptions,'timeline'=>mg_cp_events($pdo,$merchantId,(int)$contact['id'],$contactDbIds,$wallets,$messages,$redemptions,$notes)];
}

function mg_cp_activity_chart(array $wallets): array
{
    $months=[];for($i=5;$i>=0;$i--){$key=date('Y-m',strtotime('-'.$i.' months'));$months[$key]=['label'=>date("M 'y",strtotime($key.'-01')),'sent'=>0,'claimed'=>0];}
    foreach($wallets as $w){$k=!empty($w['sent_at'])?date('Y-m',strtotime((string)$w['sent_at'])):'';if(isset($months[$k]))$months[$k]['sent']++;$c=!empty($w['claimed_at'])?date('Y-m',strtotime((string)$w['claimed_at'])):'';if(isset($months[$c]))$months[$c]['claimed']++;}
    return array_values($months);
}

$method=strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'));
$user=$method==='POST'?mg_require_permission('merchant.campaigns.manage'):mg_require_permission('merchant.campaigns.view');$merchantId=(int)$user['id'];$pdo=mg_db();mg_merchant_ensure_workspace($pdo,$user);
if($method==='POST'){
    $input=mg_input();mg_require_csrf_for_write($input);$contactId=strtolower(trim((string)($input['contact_id']??'')));$note=trim((string)($input['note']??''));if(!preg_match('/^[0-9a-f-]{36}$/i',$contactId)||$note===''||mb_strlen($note)>4000)mg_fail('A valid customer and note are required.',422);
    $stmt=$pdo->prepare('SELECT * FROM merchant_crm_contacts WHERE public_id=? AND merchant_user_id=? LIMIT 1');$stmt->execute([$contactId,$merchantId]);$contact=$stmt->fetch(PDO::FETCH_ASSOC);if(!$contact)mg_fail('Customer profile not found for this merchant.',404);
    $publicId=mg_cp_uuid();$pdo->prepare('INSERT INTO merchant_crm_notes (public_id,merchant_user_id,crm_contact_id,author_user_id,note,visibility,created_at,updated_at) VALUES (?,?,?,?,?,\'merchant_internal\',NOW(),NOW())')->execute([$publicId,$merchantId,(int)$contact['id'],$merchantId,$note]);
...
$contact=mg_cp_find_contact($pdo,$merchantId);mg_ok(mg_cp_build_profile($pdo,$merchantId,$contact));
