...
{
 return ['id'=>(string)$row['public_id'],'campaign_id'=>(string)($row['campaign_public_id']??''),'campaign_title'=>(string)($row['campaign_title']??''),'email'=>(string)$row['email'],'phone'=>(string)($row['phone']??''),'name'=>(string)($row['name']??''),'source'=>(string)$row['source'],'opt_in_status'=>(string)$row['opt_in_status'],'has_account'=>(int)($row['user_id']??0)>0,'email_verified'=>!empty($row['email_verified_at']),'wallet_count'=>(int)($row['wallet_count']??0),'issued_count'=>(int)($row['issued_count']??0),'claimed_count'=>(int)($row['claimed_count']??0),'redeemed_count'=>(int)($row['redeemed_count']??0),'emails_queued_count'=>(int)($row['emails_queued_count']??0),'emails_delivered_count'=>(int)($row['emails_delivered_count']??0),'emails_failed_count'=>(int)($row['emails_failed_count']??0),'created_at'=>$row['created_at']??null,'updated_at'=>$row['updated_at']??null];
}
