...
try {
    ...
    if($method==='GET' && $action==='products'){
        $stmt=$pdo->prepare("SELECT p.id,p.public_id,v.id version_internal_id,v.public_id version_id,v.title,v.description,v.unit_value_cents,v.currency,
            (SELECT a.storage_key FROM catalog_product_version_assets pva INNER JOIN catalog_assets a ON a.id=pva.asset_id WHERE pva.product_version_id=v.id AND a.asset_type='image' ORDER BY pva.sort_order,pva.id LIMIT 1) image_url
    ...
    if($method==='GET' && $action==='merchants'){
        $q=trim((string)($_GET['q']??''));
        $where="u.id<>? AND u.status='active' AND (ms.id IS NOT NULL OR r.slug='merchant')"; $params=[$merchantId];
        if($q!==''){ $like='%'.str_replace(['%','_'],['\\%','\\_'],mb_strtolower($q)).'%'; $where.=" AND (LOWER(COALESCE(ms.display_name,u.display_name,u.full_name,u.email)) LIKE ? OR LOWER(u.email) LIKE ?)"; array_push($params,$like,$like); }
        $stmt=$pdo->prepare("SELECT DISTINCT u.id merchant_user_id,COALESCE(ms.display_name,u.display_name,u.full_name,u.email) merchant_name,u.email FROM users u LEFT JOIN merchant_storefronts ms ON ms.merchant_user_id=u.id AND ms.status<>'archived' LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id WHERE {$where} ORDER BY merchant_name LIMIT 50");
        $stmt->execute($params); $rows=[];
        foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row){$rate=mg_commission_resolve_merchant_rate($pdo,(int)$row['merchant_user_id'],['initialize'=>true]);$row['commission_rate_bps']=(int)$rate['commission_rate_bps'];$row['commission_source']=$rate['rate_source'];$rows[]=$row;}
        mg_ok(['merchants'=>$rows]);
    }
    ...
    if($method==='POST' && $action==='add_component'){
        ...
        $stmt=$pdo->prepare("SELECT p.id product_id,p.merchant_user_id,v.id version_id,v.title,v.description,v.unit_value_cents,v.public_id version_public_id,(SELECT a.storage_key FROM catalog_product_version_assets pva INNER JOIN catalog_assets a ON a.id=pva.asset_id WHERE pva.product_version_id=v.id AND a.asset_type='image' ORDER BY pva.sort_order,pva.id LIMIT 1) image_url FROM catalog_products p INNER JOIN catalog_product_versions v ON v.id=p.current_version_id WHERE p.public_id=? AND p.status='published' AND v.version_status='published' LIMIT 1 FOR UPDATE");
    ...
    if($method==='POST' && $action==='invite'){
        ...
        $amount=max(1,(int)($input['customer_amount_cents']??0)); $defaultRate=mg_commission_resolve_merchant_rate($pdo,$invitee,['initialize'=>true]);
        $terms=['product_public_id'=>trim((string)($input['product_id']??'')),'customer_amount_cents'=>$amount,'commission_rate_bps'=>mg_commission_normalize_bps($input['commission_rate_bps']??$defaultRate['commission_rate_bps']),'inventory_commitment'=>max(0,(int)($input['inventory_commitment']??0)),'settlement_policy'=>$input['settlement_policy']??'after_payment','claim_policy'=>$input['claim_policy']??'standard','refund_terms'=>trim((string)($input['refund_terms']??''))];
