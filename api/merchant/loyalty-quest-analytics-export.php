...
try{
    ...
    $report=mg_lqa_report($pdo,$merchantId,$days,$campaignRef);
    ...
    if($format==='json'){
        ...
        fputcsv($out,[]);fputcsv($out,['Privacy note','This export contains aggregate campaign metrics only. It excludes names, emails, user IDs, proof content, claim codes, QR secrets, and precise coordinates.']);
        fclose($out);
    }
    mg_audit('merchant.loyalty_quest_analytics_exported','campaign',['days'=>$days,'campaign_id'=>$campaignRef?:null,'format'=>$format,'contains_personal_data'=>false],$merchantId);
    exit;
}catch(InvalidArgumentException $error){mg_fail($error->getMessage(),422);}catch(RuntimeException $error){mg_fail($error->getMessage(),404);}catch(Throwable $error){mg_security_log('error','merchant.loyalty_quest_analytics_export_failed','Unable to export Loyalty Quest analytics.',['exception_class'=>$error::class],$merchantId);mg_fail('Unable to export Loyalty Quest analytics.',500);}
