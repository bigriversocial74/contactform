...
if($locationRef!==''){
    $location=mg_merchant_location_find_by_public_id($pdo,$workspaceId,$ownerMerchantId,$locationRef);
...
if($productRef!==''){
    ...
    $stmt->execute([$productRef,$ownerMerchantId]);$productId=(int)($stmt->fetchColumn()?:0);
...
$stmt->execute([$ownerMerchantId,$scopeKey,$horizon]);$snapshot=$stmt->fetch(PDO::FETCH_ASSOC);
...
$signals->execute([$ownerMerchantId,$locationId,$productId]);
...
$params=[$ownerMerchantId,$locationId,$productId,$windowEnd->format('Y-m-d H:i:s'),$windowStart->format('Y-m-d H:i:s'),$windowStart->format('Y-m-d H:i:s')];
