...
function mg_campaign_public_url(array $row): string
{
    ...
    $query = $type === 'qr_reward_drop' && !empty($row['qr_code_token']) ? ('?token=' . rawurlencode((string) $row['qr_code_token'])) : ('?campaign=' . rawurlencode($ref));
    return $path . $query;
}
...
function mg_campaign_reward_template_id(PDO $pdo, int $merchantId, string $publicId): ?int
{
    ...
    $stmt = $pdo->prepare("SELECT id FROM reward_templates WHERE public_id = ? AND merchant_user_id = ? AND status = 'active' LIMIT 1");
    $stmt->execute([$publicId, $merchantId]);
    $id = (int) ($stmt->fetchColumn() ?: 0);
    if ($id <= 0) mg_fail('Reward template must be active before it can be attached to a campaign.', 409);
    return $id;
}
...
if ($method === 'GET') {
    try {
        ...
        $sql = 'SELECT c.*, rt.public_id reward_template_public_id, rt.title reward_template_title FROM campaigns c LEFT JOIN reward_templates rt ON rt.id = c.reward_template_id WHERE c.merchant_user_id = ?';
        $params = [$merchantId];
        if (in_array($status, $allowedStatus, true)) { $sql .= ' AND c.status = ?'; $params[] = $status; }
        $sql .= ' ORDER BY c.updated_at DESC, c.id DESC LIMIT 100';
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        mg_ok(['campaigns' => array_map('mg_campaign_row', $stmt->fetchAll()), 'schema_ready' => true]);
...
if (($campaignId !== '' && (strlen($campaignId) !== 36 || !preg_match('/^[a-f0-9-]{36}$/', $campaignId))) || $title === '' || mb_strlen($title) > 180 || !in_array($campaignType, ['newsletter_signup', 'contest_giveaway', 'qr_reward_drop', 'referral_reward', 'birthday_vip', 'agent_offer'], true) || !in_array($status, ['draft', 'active', 'paused', 'ended', 'archived'], true)) {
...
try {
    if ($campaignId === '') {
        ...
        $stmt = $pdo->prepare('INSERT INTO campaigns (public_id,merchant_user_id,reward_template_id,campaign_type,title,description,form_headline,form_description,success_message,status,starts_at,ends_at,quantity_limit,per_user_limit,agent_discoverable,public_slug,qr_code_token,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())');
        ...
        $stmt = $pdo->prepare('UPDATE campaigns SET reward_template_id=?,campaign_type=?,title=?,description=?,form_headline=?,form_description=?,success_message=?,status=?,starts_at=?,ends_at=?,quantity_limit=?,per_user_limit=?,agent_discoverable=?,public_slug=?,updated_at=NOW() WHERE id=? AND public_id=? AND merchant_user_id=?');
    ...
    $select = $pdo->prepare('SELECT c.*, rt.public_id reward_template_public_id, rt.title reward_template_title FROM campaigns c LEFT JOIN reward_templates rt ON rt.id = c.reward_template_id WHERE c.id = ? AND c.merchant_user_id = ? LIMIT 1');
    $select->execute([$dbId, $merchantId]);
    $row = $select->fetch();
    if (!$row) mg_fail('Campaign could not be loaded.', 500);
    mg_audit('merchant.campaign_saved', 'campaign', ['campaign_id' => $campaignId,'campaign_type' => $campaignType,'status' => $status,'reward_template_id'=>$input['reward_template_id'] ?? null,'public_url' => mg_campaign_public_url($row)], $merchantId);
    mg_ok(['campaign' => mg_campaign_row($row), 'schema_ready' => true], $message, 201);
} catch (Throwable $error) {
    mg_security_log('error', 'merchant.campaigns.save_failed', 'Unable to save campaign.', ['exception_class' => $error::class, 'message'=>$error->getMessage()], $merchantId);
