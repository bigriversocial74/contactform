...
try {
    ...
    $stmt = $pdo->prepare('SELECT c.*, rt.id reward_template_db_id, rt.public_id reward_template_public_id, rt.title reward_template_title, rt.value_amount_cents, rt.currency, rt.expiration_rule, rt.expiration_days, rt.expires_at, rt.quantity_limit reward_template_quantity_limit, rt.issued_count reward_template_issued_count FROM campaigns c INNER JOIN reward_templates rt ON rt.id = c.reward_template_id WHERE c.public_id = ? AND c.merchant_user_id = ? AND c.campaign_type = \'contest_giveaway\' LIMIT 1');
    ...
    mg_winner_event($pdo, $merchantId, (int) $campaign['id'], null, (int) $contact['id'], 'contest.winner_selected', ['contact_id' => (string) $contact['public_id']]);
    ...
    mg_winner_event($pdo, $merchantId, (int) $campaign['id'], $walletDbId, (int) $contact['id'], 'wallet_item.issued', ['wallet_item_id' => $walletPublicId, 'source_type' => 'contest_winner']);

    $pdo->commit();
    mg_ok(['wallet_item_id' => $walletPublicId, 'wallet_status' => 'issued', 'already_issued' => false, 'expires_at' => $expiresAt], 'Contest winner reward issued.', 201);
} catch (Throwable $error) {
    if ($pdo->inTransaction()) $pdo->rollBack();
    mg_security_log('error', 'merchant.contest_winner.failed', 'Unable to issue contest winner reward.', ['exception_class' => $error::class], $merchantId);
