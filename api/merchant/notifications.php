...
function mg_merchant_notification_alert_row(array $row): array
{
    ...
    return ['source'=>'alert','id'=>(string)$row['public_id'],'type'=>$type,'kind'=>mg_merchant_notification_kind($type,$context),'status'=>$status,'is_unread'=>in_array($status,['open'],true),'severity'=>(string)($row['severity']??'info'),'title'=>(string)($row['title']??'Merchant alert'),'body'=>(string)($row['body']??''),'action_url'=>mg_merchant_notification_action_url($type,$row['action_url']??null,$context),'context'=>$context,'created_at'=>$row['created_at']??null,'updated_at'=>$row['updated_at']??$row['created_at']??null];
}
...
function mg_merchant_notification_notification_row(array $row): array
{
    ...
    return ['source'=>'notification','id'=>(string)$row['public_id'],'type'=>$type,'kind'=>mg_merchant_notification_kind($type,$context),'status'=>empty($row['read_at'])?'unread':'read','is_unread'=>empty($row['read_at']),'severity'=>'info','title'=>(string)($row['title']??'Merchant notification'),'body'=>(string)($row['body']??''),'action_url'=>mg_merchant_notification_action_url($type,$row['action_url']??null,$context),'context'=>$context,'created_at'=>$row['created_at']??null,'updated_at'=>$row['updated_at']??$row['created_at']??null];
}
...
function mg_merchant_notifications_fetch(PDO $pdo, int $merchantUserId, string $filter, int $limit): array
{
    ...
    } catch (Throwable $error) { $notifications = []; }
    $items = array_merge($alerts, $notifications);
    usort($items, function(array $a, array $b): int { return (strtotime((string)($b['created_at']??''))?:0) <=> (strtotime((string)($a['created_at']??''))?:0); });
...
function mg_merchant_notifications_counts(PDO $pdo, int $merchantUserId): array
{
    ...
    foreach (array_keys($counts) as $filter) $counts[$filter] = count(mg_merchant_notifications_fetch($pdo, $merchantUserId, $filter, 100));
    return $counts;
}
...
function mg_merchant_notifications_mark(PDO $pdo, int $merchantUserId, string $source, string $publicId, string $action): void
{
    ...
    $status = match($action) {'resolve'=>'resolved','dismiss'=>'dismissed',default=>'acknowledged'};
...
mg_ok(['workspace_id'=>(string)$workspace['public_id'],'filter'=>$filter,'counts'=>mg_merchant_notifications_counts($pdo,$merchantUserId),'items'=>mg_merchant_notifications_fetch($pdo,$merchantUserId,$filter,$limit)]);
