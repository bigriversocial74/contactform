...
function mg_scanner_claim_identifier(mixed $value): string
{
    ...
    if (is_array($parts)) {
        if (!empty($parts['query'])) {
            ...
            foreach (['gift','gift_id','id','item','g','claim','code','wallet_item_id'] as $key) {
    ...
    foreach ($queryCandidates as $candidate) {
        ...
        if (preg_match('/GFT-[A-Z0-9-]{4,32}/i', $candidate, $match)) return strtoupper($match[0]);
        if (preg_match('/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i', $candidate, $match)) return strtolower($match[0]);
    }
    $plain = strtoupper(trim($decoded));
    if (preg_match('/^GFT-[A-Z0-9-]{4,32}$/', $plain)) return $plain;
    mg_fail('This scan does not look like a Microgifter gift or claim QR code.', 422);
}

function mg_scanner_claim_links(PDO $pdo, array $gift, string $giftPublicId): array
{
    $links = [
        'claimed_url' => '/claimed.php?gift=' . rawurlencode($giftPublicId),
        'claim_route' => '/claim.php?gift=' . rawurlencode($giftPublicId),
    ];
    try {
        $stmt = $pdo->prepare('SELECT ac.public_id,ac.folder FROM microgift_inbox_items ac INNER JOIN pppm_legacy_gift_map m ON m.pppm_item_id=ac.instance_id WHERE m.gift_id=? ORDER BY ac.updated_at DESC LIMIT 1');
        $stmt->execute([(int)$gift['id']]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $folder = in_array((string)$row['folder'], ['inbox','sent','claimed'], true) ? (string)$row['folder'] : 'claimed';
            $links['action_center_item_id'] = (string)$row['public_id'];
            $links['action_center_url'] = '/' . $folder . '.php?item=' . rawurlencode((string)$row['public_id']);
        }
    } catch (Throwable) {}
    return $links;
}
...
if (strlen($locationPublicId) !== 36 || !preg_match('/^[a-f0-9-]{36}$/', $locationPublicId)) mg_fail('Choose a merchant location for this scanner.', 422);
...
try {
    ...
    $links = mg_scanner_claim_links($pdo, $gift, $giftPublicId);
    ...
    if (!$claim) {
        ...
        $pdo->prepare("INSERT INTO gift_claims (public_id,gift_id,location_id,status,failed_attempts,expires_at,created_at,updated_at) VALUES (?,?,?,'pending',0,?,NOW(),NOW())")->execute([$claimPublicId,(int) $gift['id'],(int) $location['id'],$gift['expires_at'] ?? null]);
        $claimStmt->execute([(int) $gift['id']]);
        $claim = $claimStmt->fetch(PDO::FETCH_ASSOC);
    }
    ...
    if ((string) $claim['status'] === 'redeemed' || (string) $gift['status'] === 'claimed') {
        $pdo->commit();
        mg_ok(['gift_id'=>$giftPublicId,'claim_id'=>(string)$claim['public_id'],'location_id'=>$locationPublicId,'location_name'=>(string)$location['name'],'verified'=>true,'redeemed'=>true,'already_redeemed'=>true,'links'=>$links], 'Gift already redeemed.');
    }
    ...
    $pdo->prepare('INSERT INTO gift_claim_attempts (claim_id,actor_user_id,successful,ip_hash,user_agent_hash,created_at) VALUES (?,?,?,?,?,NOW())')->execute([(int)$claim['id'],$merchantUserId,1,$ipHash,$uaHash]);
    ...
    if ($action === 'verify' || ($action === 'redeem' && $requireConfirm && !$confirmed)) {
        $pdo->commit();
        mg_audit('gift.scanner_claim_verified', 'gift', ['gift_id'=>$giftPublicId,'location_id'=>$locationPublicId,'confirmation_required'=>$requireConfirm], $merchantUserId);
        mg_ok(['gift_id'=>$giftPublicId,'claim_id'=>(string)$claim['public_id'],'location_id'=>$locationPublicId,'location_name'=>(string)$location['name'],'claim_code_last4'=>(string)($claimCode['code_last4'] ?? ''),'verified'=>true,'redeemed'=>false,'needs_confirmation'=>$action==='redeem'&&$requireConfirm&&!$confirmed,'links'=>$links,'gift'=>['title'=>(string)($gift['title'] ?? 'Microgift'),'value_cents'=>(int)($gift['value_cents'] ?? 0),'currency'=>(string)($gift['currency'] ?? 'USD')]], $action === 'verify' ? 'Gift verified for this scanner location.' : 'Gift verified. Confirm redemption before claiming voucher.');
    }
    ...
    mg_gift_event($pdo, (int) $gift['id'], $merchantUserId, 'claimed', ['claim_id'=>(string)$claim['public_id'],'location_id'=>$locationPublicId,'source'=>'scanner','claim_code_last4'=>(string)($claimCode['code_last4'] ?? '')]);
    ...
    if ((int) ($gift['sender_user_id'] ?? 0) > 0) {
        ...
        $notificationStmt->execute([mg_public_uuid(),(int)$gift['sender_user_id'],'gift_claimed','Gift redeemed','A merchant location successfully redeemed your gift.',$links['claimed_url'],(int)$gift['id']]);
    }

    $pdo->commit();
    mg_audit('gift.scanner_claim_redeemed', 'gift', ['gift_id'=>$giftPublicId,'location_id'=>$locationPublicId,'claim_code_last4'=>(string)($claimCode['code_last4'] ?? '')], $merchantUserId);
    mg_event('gift.scanner_claim_redeemed', ['gift_id'=>$giftPublicId,'location_id'=>$locationPublicId], $merchantUserId);
    mg_ok(['gift_id'=>$giftPublicId,'claim_id'=>(string)$claim['public_id'],'location_id'=>$locationPublicId,'location_name'=>(string)$location['name'],'claim_code_last4'=>(string)($claimCode['code_last4'] ?? ''),'verified'=>true,'redeemed'=>true,'links'=>$links,'gift'=>['title'=>(string)($gift['title'] ?? 'Microgift'),'value_cents'=>(int)($gift['value_cents'] ?? 0),'currency'=>(string)($gift['currency'] ?? 'USD')]], 'Gift redeemed.');
} catch (Throwable $error) {
    if ($pdo->inTransaction()) $pdo->rollBack();
    mg_security_log('error', 'merchant.scanner_claim_failed', 'Scanner claim failed.', ['identifier'=>$identifier,'location_id'=>$locationPublicId,'exception_class'=>$error::class,'message'=>$error->getMessage()], $merchantUserId);
