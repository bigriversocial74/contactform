...
if (function_exists('mg_rate_limit')) {
    mg_rate_limit('merchant.hosted_games.integration', 'user:' . $actorId, 30, 300);
}
...
try {
    ...
    $pdo->commit();
    mg_audit('merchant.hosted_game.integration_configured', 'hosted_game', [
    ...
    ], 'Distribution Program, campaign, reward inventory, and game API integration configured.');
} catch (MgHostedGameException|InvalidArgumentException $e) {
    if ($pdo->inTransaction()) $pdo->rollBack();
    mg_fail($e->getMessage(), 409);
} catch (Throwable $e) {
    if ($pdo->inTransaction()) $pdo->rollBack();
    error_log('Hosted game merchant integration failed: ' . $e->getMessage());
