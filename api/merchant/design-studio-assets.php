...
function mg_design_project_status(mixed $value): string
{
    $status = strtolower(trim((string) $value));
    return in_array($status, ['draft', 'ready_for_review', 'approved', 'exported', 'archived'], true) ? $status : 'draft';
}

function mg_design_template_status(mixed $value): string
{
    $status = strtolower(trim((string) $value));
    return in_array($status, ['draft', 'active', 'archived'], true) ? $status : 'draft';
}
...
if ($method === 'GET') {
    ...
    $templatesStmt = $pdo->prepare("SELECT * FROM merchant_design_templates WHERE status <> 'archived' AND template_type=? AND (workspace_id=? OR template_scope IN ('system','admin')) ORDER BY FIELD(template_scope,'merchant','admin','system'), updated_at DESC LIMIT 100");
    ...
    if ($mode === 'bootstrap' || $mode === 'projects') {
        $projectsStmt = $pdo->prepare("SELECT p.*,t.public_id template_public_id,q.public_id qr_public_id FROM merchant_design_projects p LEFT JOIN merchant_design_templates t ON t.id=p.template_id LEFT JOIN merchant_qr_codes q ON q.id=p.qr_code_id WHERE p.workspace_id=? AND p.status <> 'archived' ORDER BY p.updated_at DESC LIMIT 50");
        $projectsStmt->execute([$workspaceId]);
        $projects = array_map('mg_design_format_project', $projectsStmt->fetchAll());
    }

    mg_ok(['templates' => $templates, 'projects' => $projects, 'workspace_id' => (string) $workspace['public_id']]);
}
...
if ($action === 'save_template') {
    ...
    $status = mg_design_template_status($input['status'] ?? 'active');
    ...
    $pdo->prepare('INSERT INTO merchant_design_templates (public_id,workspace_id,merchant_user_id,template_scope,template_type,format_key,name,description,status,width_px,height_px,print_width_in,print_height_in,bleed_in,layout_json,default_copy_json,render_config_json,qr_required,is_presigned,signature_hash,signed_at,created_by_user_id,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,NOW(),NOW())')
        ->execute([
            ...
            $description,
            $status,
            mg_design_int_or_null($input['width_px'] ?? null),
            mg_design_int_or_null($input['height_px'] ?? null),
            mg_design_decimal_or_null($input['print_width_in'] ?? null),
            mg_design_decimal_or_null($input['print_height_in'] ?? null),
            mg_design_decimal_or_null($input['bleed_in'] ?? null),
            $layoutJson,
            $copyJson,
            $renderJson,
            !empty($input['qr_required']) ? 1 : 0,
            1,
            $signatureHash,
            $userId,
            $userId,
        ]);
    $stmt = $pdo->prepare('SELECT * FROM merchant_design_templates WHERE public_id=? LIMIT 1');
    $stmt->execute([$publicId]);
    mg_audit('merchant.design_template_saved', 'merchant_design_template', ['template_id' => $publicId, 'format_key' => $formatKey, 'template_type' => $type], $userId);
...
if ($action === 'save_project') {
    ...
    $status = mg_design_project_status($input['status'] ?? 'draft');
    ...
    if ($projectId !== '') {
        ...
        $pdo->prepare('UPDATE merchant_design_projects SET template_id=?,qr_code_id=?,project_type=?,format_key=?,name=?,status=?,canvas_json=?,copy_json=?,media_json=?,print_options_json=?,export_manifest_json=?,updated_by_user_id=?,updated_at=NOW(),archived_at=IF(?="archived",COALESCE(archived_at,NOW()),NULL) WHERE id=?')
            ->execute([$templateDbId, $qrDbId, $type, $formatKey, $name, $status, $canvasJson, $copyJson, $mediaJson, $printOptionsJson, $exportJson, $userId, $status, (int) $existing['id']]);
    } else {
        $projectId = mg_merchant_uuid();
        $pdo->prepare('INSERT INTO merchant_design_projects (public_id,workspace_id,merchant_user_id,template_id,qr_code_id,project_type,format_key,name,status,canvas_json,copy_json,media_json,print_options_json,export_manifest_json,created_by_user_id,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')
            ->execute([$projectId, $workspaceId, $userId, $templateDbId, $qrDbId, $type, $formatKey, $name, $status, $canvasJson, $copyJson, $mediaJson, $printOptionsJson, $exportJson, $userId, $userId]);
    }

    $fresh = $pdo->prepare('SELECT p.*,t.public_id template_public_id,q.public_id qr_public_id FROM merchant_design_projects p LEFT JOIN merchant_design_templates t ON t.id=p.template_id LEFT JOIN merchant_qr_codes q ON q.id=p.qr_code_id WHERE p.public_id=? AND p.workspace_id=? LIMIT 1');
    $fresh->execute([$projectId, $workspaceId]);
    mg_audit('merchant.design_project_saved', 'merchant_design_project', ['project_id' => $projectId, 'format_key' => $formatKey, 'project_type' => $type], $userId);
...
if ($action === 'queue_ai_job') {
    ...
    $pdo->prepare("INSERT INTO merchant_design_ai_jobs (public_id,workspace_id,merchant_user_id,project_id,provider_key,model_key,generation_type,prompt_json,status,approval_status,requested_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,'queued','pending',?,NOW(),NOW())")
        ->execute([$jobId, $workspaceId, $userId, $projectDbId, mg_design_clean_text($input['provider_key'] ?? '', '', 80) ?: null, mg_design_clean_text($input['model_key'] ?? '', '', 120) ?: null, $generationType, $promptJson, $userId]);
    mg_audit('merchant.design_ai_job_queued', 'merchant_design_ai_job', ['job_id' => $jobId, 'generation_type' => $generationType], $userId);
