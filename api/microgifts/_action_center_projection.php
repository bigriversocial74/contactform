...
function mg_action_center_projection_upsert(PDO $pdo,array $instance,int $userId,string $folder,string $state,array $context=[]): string
{
    ...
    $senderUserId=(int)($instance['issuer_user_id']??0);
    $recipientUserId=(int)($instance['recipient_user_id']??$instance['owner_user_id']??0);
    ...
    $projectionAt=(string)($instance['issued_at']??date('Y-m-d H:i:s'));
    $firstReceivedAt=$projectionAt;
    $sentAt=$folder==='sent'?$projectionAt:null;
    ...
    if($row){
        $pdo->prepare('UPDATE microgift_inbox_items SET folder=?,state=?,sender_user_id=?,recipient_user_id=?,redemption_id=COALESCE(?,redemption_id),merchant_user_id=COALESCE(?,merchant_user_id),location_id=COALESCE(?,location_id),can_tip=?,first_received_at=COALESCE(first_received_at,?),sent_at=COALESCE(sent_at,?),claimed_at=COALESCE(?,claimed_at),redeemed_at=COALESCE(?,redeemed_at),updated_at=NOW() WHERE id=?')->execute([$folder,$state,$senderUserId?:null,$recipientUserId?:null,$redemptionId,$merchantUserId,$locationId,$canTip,$firstReceivedAt,$sentAt,$claimedAt,$redeemedAt,(int)$row['id']]);
        return (string)$row['public_id'];
    }

    $publicId=mg_microgift_uuid();
    $pdo->prepare('INSERT INTO microgift_inbox_items (public_id,instance_id,user_id,folder,sender_user_id,recipient_user_id,state,redemption_id,merchant_user_id,location_id,can_tip,first_received_at,sent_at,claimed_at,redeemed_at,metadata_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')->execute([$publicId,$instanceId,$userId,$folder,$senderUserId?:null,$recipientUserId?:null,$state,$redemptionId,$merchantUserId,$locationId,$canTip,$firstReceivedAt,$sentAt,$claimedAt,$redeemedAt,mg_microgift_json([])]);
    return $publicId;
}
...
function mg_action_center_receive(PDO $pdo,int $instanceId,int $userId,?int $senderUserId=null): string
{
    mg_action_center_require_transaction($pdo);
    ...
    if($senderUserId!==null)$instance['issuer_user_id']=$senderUserId;
    $instance['recipient_user_id']=$userId;
    return mg_action_center_projection_upsert($pdo,$instance,$userId,mg_action_center_recipient_folder($instance),mg_action_center_state($instance));
}

function mg_action_center_sent(PDO $pdo,int $instanceId,int $senderUserId,int $recipientUserId): array
{
    mg_action_center_require_transaction($pdo);
    $stmt=$pdo->prepare('SELECT * FROM microgift_instances WHERE id=? LIMIT 1 FOR UPDATE');
    $stmt->execute([$instanceId]);
    $instance=$stmt->fetch(PDO::FETCH_ASSOC);
    if(!$instance)throw new RuntimeException('Microgift instance not found for Action Center projection.');
    $instance['issuer_user_id']=$senderUserId;
    $instance['recipient_user_id']=$recipientUserId;
    $state=mg_action_center_state($instance);

    if($senderUserId===$recipientUserId){
        $recipientPublicId=mg_action_center_projection_upsert($pdo,$instance,$recipientUserId,mg_action_center_recipient_folder($instance),$state);
        return ['sent_item_id'=>null,'recipient_inbox_item_id'=>$recipientPublicId];
    }

    $sentPublicId=mg_action_center_projection_upsert($pdo,$instance,$senderUserId,'sent',$state,['can_tip'=>0]);
    $recipientPublicId=mg_action_center_projection_upsert($pdo,$instance,$recipientUserId,mg_action_center_recipient_folder($instance),$state);
