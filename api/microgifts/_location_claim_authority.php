...
{
    // Existing merchant codes are intentionally case-sensitive.
    return trim($value);
}
...
function mg_location_claim_resolve_authority(PDO $pdo,int $merchantId,string $locationPublicId,int $actorId,string $submittedCode): array
{
    ...
    if(mb_strlen($normalized)<4||mb_strlen($normalized)>64){
    ...
    $now=time();
    if($claimCode['valid_from']!==null&&strtotime((string)$claimCode['valid_from'])>$now){
        throw new MgLocationClaimAuthorityException('invalid_claim_code','Location claim authority could not be verified.');
    }
    if($claimCode['valid_until']!==null&&strtotime((string)$claimCode['valid_until'])<=$now){
        throw new MgLocationClaimAuthorityException('invalid_claim_code','Location claim authority could not be verified.');
    }
    if($claimCode['usage_limit']!==null&&(int)$claimCode['usage_count']>=(int)$claimCode['usage_limit']){
        throw new MgLocationClaimAuthorityException('invalid_claim_code','Location claim authority could not be verified.');
    }
...
function mg_location_claim_increment_usage(PDO $pdo,int $claimCodeId): void
{
    ...
    if($stmt->rowCount()!==1){
        throw new MgLocationClaimAuthorityException('invalid_claim_code','Claim-code usage could not be reserved.');
    }
}
