...
function mg_ac_voucher_claim_microgift(PDO $pdo, string $actionItemId, string $merchantCode, string $voucherToken, int $userId): void
{
    ...
    if (mg_ac_voucher_completed_microgift_redemption($pdo, (int)$voucher['instance_internal_id'])) {
        mg_ac_voucher_fail('This gift has already been claimed. A refund must be issued before it can be claimed again.', 409);
    }
    ...
    $claimCode = mg_ac_voucher_find_claim_code($pdo, $merchantCode, [
...
function mg_ac_voucher_claim_wallet(PDO $pdo, string $actionItemId, string $walletId, string $merchantCode, string $voucherToken, int $userId, string $userEmail): void
{
    ...
    $pdo->prepare("INSERT INTO wallet_item_redemptions (public_id,wallet_item_id,user_id,merchant_user_id,location_id,location_reference,amount_cents,currency,status,idempotency_key,source_reference,redeemed_at,metadata_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,'completed',?,?,NOW(),?,NOW(),NOW())")->execute([
        $redemptionPublicId,
        (int)$wallet['id'],
        $userId,
        (int)$claimCode['merchant_user_id'],
        (int)$claimCode['location_internal_id'],
        $locationPublicId,
        (int)($wallet['value_cents_snapshot'] ?? 0),
        (string)($wallet['currency_snapshot'] ?? 'USD'),
        $idempotencyKey,
        'action_center_manual:' . $locationPublicId,
        json_encode($eventContext, JSON_UNESCAPED_SLASHES),
    ]);
    ...
    mg_event('wallet.action_center_manual_redeemed', ['wallet_item_id' => $walletId, 'location_id' => $locationPublicId], (int)$claimCode['merchant_user_id']);
...
try {
    ...
    if ($walletId !== null) {
        mg_ac_voucher_claim_wallet($pdo, $actionItemId, $walletId, $merchantCode, $voucherToken, $userId, $userEmail);
    }
    mg_ac_voucher_claim_microgift($pdo, $actionItemId, $merchantCode, $voucherToken, $userId);
} catch (MgAcVoucherClaimError $error) {
    if ($pdo->inTransaction()) $pdo->rollBack();
    mg_security_log('warning', 'action_center.voucher_manual_claim_rejected', $error->getMessage(), [
        'action_item_id' => $actionItemId,
        'status' => $error->status,
    ], $userId);
