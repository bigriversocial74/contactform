...
function mg_ac_wallet_user_email(array $user): string
{
...
function mg_ac_wallet_event(PDO $pdo,array $item,string $eventType,array $context=[]): string
{
    ...
    $pdo->prepare('INSERT INTO campaign_events (public_id,merchant_user_id,campaign_id,wallet_item_id,contact_id,event_type,event_context_json,created_at) VALUES (?,?,?,?,?,?,?,NOW())')
        ->execute([
            ...
            $eventType,
            mg_ac_wallet_json($context+['wallet_item_id'=>(string)$item['public_id']]),
        ]);
    return $publicId;
}
...
function mg_ac_wallet_merchant_target(PDO $pdo,array $item): array
{
    ...
    try{
        ...
        if($workspacePublicId!=='')return ['target_type'=>'merchant','target_reference'=>$workspacePublicId,'merchant_user_id'=>$merchantUserId,'merchant_workspace_id'=>$workspacePublicId];
    }catch(Throwable){}
...
function mg_ac_wallet_public_item(array $row): array
{
    ...
    return [
        ...
        'can_message'=>in_array($state,['redeemable','redeemed'],true)?1:0,
        'read_at'=>null,
        ...
        'metadata_json'=>mg_ac_wallet_json($metadata),
        ...
        'product_type'=>$campaignType!==''?mg_ac_wallet_type_label($campaignType):'Campaign reward',
        'message'=>$message,
...
function mg_ac_wallet_counts(PDO $pdo,int $userId,string $email): array
{
    $params=[$userId];
    $where=['wi.pppm_item_id IS NULL',"wi.status<>'cancelled'",mg_ac_wallet_identity_where(strtolower(trim($email)),$params)];
    $sql="SELECT wi.status,wi.expires_at FROM wallet_items wi LEFT JOIN campaign_contacts cc ON cc.id=wi.contact_id WHERE ".implode(' AND ',$where);
    $stmt=$pdo->prepare($sql);
    $stmt->execute($params);
...
function mg_ac_wallet_counts_merge(array $base,array $wallet): array
{
    foreach($wallet as $folder=>$values){
        if(!isset($base[$folder]))$base[$folder]=['total'=>0,'unread'=>0];
        $base[$folder]['total']+=(int)($values['total']??0);
        $base[$folder]['unread']+=(int)($values['unread']??0);
    }
    return $base;
}
