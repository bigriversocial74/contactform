...
if($actionItemId===''||$message===''||$idempotencyKey===''){
    mg_fail('Sent item, follow-up message, and idempotency key are required.',422);
}
...
try{
    ...
    if((int)$instance['action_sender_user_id']!==(int)$user['id']){
        throw new RuntimeException('Only the sender of this transfer can follow up.');
    }
    ...
    if((int)$instance['owner_user_id']!==$recipientUserId||(int)$instance['recipient_user_id']!==$recipientUserId){
        throw new RuntimeException('This recipient no longer owns the Microgift. Only the most recent sender can follow up.');
    }
    if(!in_array((string)$instance['status'],['issued','delivered'],true)||!empty($instance['claimed_at'])||!empty($instance['redeemed_at'])){
        throw new RuntimeException('Follow Up is unavailable after the Microgift is claimed, redeemed, expired, revoked, or otherwise closed.');
    }
    ...
    mg_ok([
        ...
        'status'=>'accepted',
        'duplicate'=>$result['duplicate'],
    ],$result['duplicate']?'Existing Follow Up result returned.':'Follow Up sent.',$result['duplicate']?200:202);
    ...
    mg_fail('Unable to send this Follow Up.',500);
}
