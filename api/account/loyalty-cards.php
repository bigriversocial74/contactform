...
function mg_loyalty_campaign_image(array $row): string
{
    ...
    foreach (['media_image_url','campaign_image_url','stamp_card_image_url','cover_image_url','image_url'] as $key) {
    ...
    $rewardCover = mg_loyalty_safe_url($pack['cover_image_url'] ?? '');
    if ($rewardCover !== '') return $rewardCover;
    return '';
}
...
function mg_loyalty_card_public(array $row): array
{
    ...
    $merchantName = trim((string)($row['merchant_profile_display_name'] ?? '')) ?: (trim((string)($row['merchant_user_display_name'] ?? '')) ?: 'Microgifter merchant');
...
function mg_loyalty_find_campaign(PDO $pdo, string $campaignRef): ?array
{
    ...
    $stmt = $pdo->prepare("SELECT id,public_id,public_slug,merchant_user_id,campaign_type,status FROM campaigns WHERE campaign_type='stamp_card_reward' AND status IN ('active','paused','ended') AND (public_id=? OR public_slug=?) LIMIT 1");
...
if ($method === 'GET') {
    ...
    try {
        if ($campaignRef !== '') {
            ...
            mg_ok(['saved' => $status === 'saved', 'status' => $status ?: 'none', 'campaign_id' => (string)$campaign['public_id'], 'schema_ready' => true]);
        }

        $sql = "SELECT sc.public_id saved_public_id, sc.saved_at,
                       c.public_id campaign_public_id, c.public_slug, c.title, c.description, c.form_headline, c.form_description, c.rules_json, c.status campaign_status,
                       u.display_name merchant_user_display_name,
                       pp.display_name merchant_profile_display_name, pp.headline merchant_profile_headline, pp.cover_url merchant_profile_cover_url,
                       rt.title reward_template_title, rt.description reward_template_description, rt.metadata_json reward_metadata_json,
                       cc.id contact_id,
                       (SELECT COUNT(*) FROM campaign_events ce WHERE ce.campaign_id=c.id AND ce.contact_id=cc.id AND ce.event_type='stamp_card.stamped') stamp_count,
                       (SELECT MAX(ce2.created_at) FROM campaign_events ce2 WHERE ce2.campaign_id=c.id AND ce2.contact_id=cc.id AND ce2.event_type='stamp_card.stamped') last_stamp_at
                ...
                LEFT JOIN campaign_contacts cc ON cc.campaign_id=c.id AND (cc.user_id=? OR cc.email=?)
                WHERE sc.user_id=? AND sc.status='saved' AND c.campaign_type='stamp_card_reward'
                ORDER BY sc.updated_at DESC, sc.id DESC
                LIMIT 100";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$userId, $email, $userId]);
        ...
        mg_security_log('warning', 'account.loyalty_cards.unavailable', 'Saved loyalty cards unavailable.', ['exception_class' => $error::class, 'message' => $error->getMessage()], $userId);
        mg_ok(['cards' => [], 'count' => 0, 'schema_ready' => false], 'Saved loyalty cards unavailable until the saved-cards schema is installed.');
    }
}
...
if ($method === 'POST') {
    ...
    if (!in_array($action, ['save','unsave','toggle'], true) || $campaignRef === '') mg_fail('Invalid saved card request.', 422);
    try {
        $campaign = mg_loyalty_find_campaign($pdo, $campaignRef);
        ...
        $next = $action === 'toggle' ? ($current === 'saved' ? 'archived' : 'saved') : ($action === 'unsave' ? 'archived' : 'saved');
        $metadata = json_encode(['campaign_type' => 'stamp_card_reward', 'source' => 'public_stamp_card', 'updated_from' => 'loyalty_card_toggle'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        $upsert = $pdo->prepare("INSERT INTO customer_saved_campaign_cards (public_id,user_id,campaign_id,merchant_user_id,status,metadata_json,saved_at,updated_at) VALUES (?,?,?,?,?,?,NOW(),NOW()) ON DUPLICATE KEY UPDATE status=VALUES(status), metadata_json=VALUES(metadata_json), updated_at=NOW(), saved_at=IF(VALUES(status)='saved', COALESCE(saved_at,NOW()), saved_at)");
        $upsert->execute([mg_loyalty_uuid(), $userId, (int)$campaign['id'], (int)$campaign['merchant_user_id'], $next, $metadata]);
        mg_ok(['saved' => $next === 'saved', 'status' => $next, 'campaign_id' => (string)$campaign['public_id'], 'schema_ready' => true], $next === 'saved' ? 'Loyalty card saved.' : 'Loyalty card removed.');
    } catch (Throwable $error) {
        mg_security_log('error', 'account.loyalty_cards.toggle_failed', 'Unable to update saved loyalty card.', ['exception_class' => $error::class, 'message' => $error->getMessage()], $userId);
