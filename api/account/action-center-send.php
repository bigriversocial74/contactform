...
function mg_action_center_regift_stamp_status(?array $stampLedger): array
{
    ...
    return [
        ...
        'stamp_actor_user_id' => $stampLedger['actor_user_id'] ?? null,
        'stamp_debit_status' => $stampLedger['debit_status'] ?? ($stampLedger['entry']['entry_id'] ?? null ? 'debited' : 'not_required'),
        'stamp_shortfall' => $stampLedger['shortfall'] ?? null,
    ];
}

function mg_action_center_merchant_sponsored_regift_stamp(PDO $pdo, array $actorUser, int $sponsorUserId, string $idempotencyKey, array $options = []): array
{
    $actorUserId = (int)($actorUser['id'] ?? 0);
    $sponsorUserId = max(0, $sponsorUserId);
    $sourceType = trim((string)($options['source_type'] ?? 'merchant_sponsored_regift')) ?: 'merchant_sponsored_regift';
    $sourceId = trim((string)($options['source_id'] ?? $idempotencyKey));
    $reference = trim((string)($options['reference'] ?? 'regift_send'));
    $quantity = max(1, (int)($options['quantity'] ?? 1));
    $metadata = is_array($options['metadata'] ?? null) ? $options['metadata'] : [];

    $base = [
        'debit_status' => 'not_required',
        'debit_applied' => false,
        'sponsor_user_id' => $sponsorUserId ?: null,
        'actor_user_id' => $actorUserId ?: null,
        'action_key' => 'regift_send',
    ...
    if ($available < $required) {
        ...
        if (function_exists('mg_audit')) {
            mg_audit('stamps.merchant_sponsored_regift_shortfall', 'stamp_ledger', $payload, $actorUserId ?: null);
        }
        return $payload;
    }

    $debit = mg_stamp_debit($pdo, $sponsorUserId, $actorUserId, 'regift_send', $quantity, $sponsorKey, [
        'source_type' => $sourceType,
        'source_id' => $sourceId,
        'reference' => $reference,
        'reason_code' => 'merchant_sponsored_regift',
        'note' => 'Customer regift sponsored by original merchant Stamp balance.',
        'metadata' => $metadata + [
    ...
    return $debit + [
        'debit_status' => !empty($debit['idempotent']) ? 'idempotent' : 'debited',
        'debit_applied' => true,
        'sponsor_user_id' => $sponsorUserId,
...
function mg_action_center_send_wallet_item(PDO $pdo, array $item, array $sender, int $recipientUserId, string $actionItemId, string $idempotencyKey, string $message, ?array $stampLedger = null): array
{
...
try {
    ...
    if ($walletId !== null) {
        ...
        $stampLedger = mg_action_center_merchant_sponsored_regift_stamp($pdo, $user, (int)($walletItem['merchant_user_id'] ?? 0), $idempotencyKey, [
            'source_type' => 'merchant_sponsored_wallet_regift',
            'source_id' => $walletId,
            'reference' => $actionItemId,
            'metadata' => [
                'wallet_item_id' => $walletId,
                'recipient_user_id' => $recipientUserId,
                'original_merchant_user_id' => (int)($walletItem['merchant_user_id'] ?? 0),
            ],
        ]);
        $result = mg_action_center_send_wallet_item($pdo, $walletItem, $user, $recipientUserId, $actionItemId, $idempotencyKey, $message, $stampLedger);
    ...
    $stampLedger = mg_action_center_merchant_sponsored_regift_stamp($pdo, $user, (int)($instance['issuer_user_id'] ?? 0), $idempotencyKey, [
        'source_type' => 'merchant_sponsored_microgift_regift',
        ...
        'metadata' => [
            ...
            'recipient_user_id' => $recipientUserId,
            'original_issuer_user_id' => (int)($instance['issuer_user_id'] ?? 0),
        ],
    ]);
    $stampStatus = mg_action_center_regift_stamp_status($stampLedger);
    ...
    $transfer = mg_pppm_transfer_owner_canonical(
        ...
        $idempotencyKey,
        (int)$user['id'],
        ['microgift_instance_id' => (string)$instance['public_id'], 'action_item_id' => $actionItemId] + $stampStatus
    );
    ...
    $deliveryEvent = mg_microgift_delivery_event(
        ...
        $idempotencyKey,
        $actionItemId,
        ['pppm_item_id' => $pppmPublicId, 'transfer_id' => $transfer['transfer_id'] ?? null, 'transfer_type' => 'regift'] + $stampStatus
    );
