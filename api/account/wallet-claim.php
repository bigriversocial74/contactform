...
try {
    ...
    $pdo->prepare('UPDATE wallet_items SET user_id = ?, pppm_item_id = COALESCE(pppm_item_id, ?), status = \'claimed\', viewed_at = COALESCE(viewed_at,NOW()), claimed_at = COALESCE(claimed_at,NOW()), updated_at = NOW() WHERE id = ?')
        ->execute([$userId, (int) ($bridge['pppm_item_db_id'] ?? 0), (int) $item['id']]);
    mg_wc_event($pdo, $item, 'wallet_item.claimed', ['pppm_bridge' => $bridge, 'claim' => $claimResult, 'action_center' => $projection]);

    $pdo->commit();
    mg_ok(['wallet_item_id' => $walletId, 'wallet_status' => 'claimed', 'pppm_bridge' => $bridge, 'claim' => $claimResult, 'action_center' => $projection], 'Wallet item claimed.');
