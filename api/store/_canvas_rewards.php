...
/**
 * Store Canvas reward delivery bridge.
 *
 * Sends merchant-selected campaign rewards to the active Store Canvas customer
 * using the existing campaign wallet / IN-OUT Box system. No new tables are
 * required: wallet_items is the customer-visible reward record, campaign_events
 * and mg_store_session_events are the audit trail, and notifications alert the
 * customer.
 */
...
{
    if (!mg_store_reward_required_tables($pdo)) {
        mg_fail('Store Canvas reward delivery requires Stage 12 campaigns/rewards and Stage 20 Store Canvas tables.', 503);
    }
}
...
function mg_store_reward_optional_text(mixed $value, int $max): string
{
    ...
    if ($text === '') return '';
    if (mb_strlen($text) > $max) throw new InvalidArgumentException('Reward note is too long.');
    return $text;
}
...
function mg_store_reward_expiry(array $template, mixed $expirationDays = null, mixed $expiresAt = null): ?string
{
    ...
    if ($days !== '') {
        ...
        return gmdate('Y-m-d H:i:s', time() + ($count * 86400));
    }
...
function mg_store_reward_options(PDO $pdo, int $merchantUserId): array
{
    ...
    $campaignStmt = $pdo->prepare(
        "SELECT c.*,rt.public_id reward_template_public_id,rt.title reward_template_title
         FROM campaigns c
    ...
    return [
        ...
        'campaigns' => $campaigns,
        'templates' => $templates,
        'can_send_reward' => $campaigns !== [] && $templates !== [],
    ];
}
...
function mg_store_reward_load_active_session(PDO $pdo, int $merchantUserId, string $sessionPublicId): array
{
    ...
    $stmt = $pdo->prepare(
        "SELECT s.*,cp.display_name customer_name,cp.avatar_url customer_avatar_url,cp.slug customer_slug,u.email customer_email
         FROM mg_store_sessions s
         INNER JOIN users u ON u.id=s.customer_user_id
         LEFT JOIN public_profiles cp ON cp.user_id=s.customer_user_id
         WHERE s.public_id=?
           AND s.merchant_user_id=?
...
function mg_store_reward_issue(PDO $pdo, array $merchantUser, string $sessionPublicId, string $campaignPublicId, ?string $templatePublicId, string $note = '', mixed $expirationDays = null, mixed $expiresAt = null, string $idempotencyKey = ''): array
{
    ...
    try {
        ...
        $campaignStmt = $pdo->prepare(
            "SELECT c.*,rt.public_id attached_template_public_id
             FROM campaigns c
             LEFT JOIN reward_templates rt ON rt.id=c.reward_template_id
             WHERE c.public_id=? AND c.merchant_user_id=? AND c.status='active'
             LIMIT 1 FOR UPDATE"
        );
        ...
        if ($templatePublicId === null) {
            $templatePublicId = (string)($campaign['attached_template_public_id'] ?? '');
        ...
        $dupeStmt = $pdo->prepare(
            "SELECT COUNT(*)
             ...
             WHERE merchant_user_id=?
               AND user_id=?
               AND campaign_id=?
               AND reward_template_id=?
               AND status IN ('issued','viewed','claimed','redeemed')
        ...
        if ($stampLimit !== null && $stampLimit !== '') {
            ...
            if ((int)$stampUsage->fetchColumn() >= max(0, (int)$stampLimit) && !mg_package_limit_value($packageContext, 'stamp_overage_enabled')) {
                throw new RuntimeException('Monthly stamp limit reached. Upgrade your package or enable stamp overage.');
            }
        }
        ...
        $insert = $pdo->prepare(
            'INSERT INTO wallet_items
             (public_id,user_id,contact_id,merchant_user_id,reward_template_id,campaign_id,pppm_item_id,source_type,source_id,status,value_cents_snapshot,currency_snapshot,title_snapshot,metadata_json,issued_at,expires_at,created_at,updated_at)
             VALUES (?,?,NULL,?,?,?,?,?,?,'issued',?,?,?,?,NOW(),?,NOW(),NOW())'
        );
        $insert->execute([
            $walletPublicId,
            $customerUserId,
            ...
            'manual_send',
            $sessionPublicId,
            (int)$template['value_amount_cents'],
        ...
        $notificationId = mg_create_notification(
            ...
            $customerUserId,
            'campaign_reward',
            'You received a reward from ' . $merchantLabel,
            (string)$template['title'],
            '/inbox.php?item=' . rawurlencode('wallet-' . $walletPublicId),
            [
                'actor_user_id' => $merchantId,
                'event_key' => 'reward.store_canvas.' . strtolower($walletPublicId),
                'merchant_user_id' => $merchantId,
                'store_session_id' => $sessionPublicId,
