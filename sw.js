...
function safeAssetUrl(value, fallback) {
  try {
    ...
    return fallback || MG_DEFAULT_ICON;
  }
}
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    payload = { title: 'Microgifter update', body: 'Open Microgifter for details.' };
  }
  ...
  const options = {
    ...
    icon: safeAssetUrl(payload.icon, MG_DEFAULT_ICON),
    badge: safeAssetUrl(payload.badge, MG_DEFAULT_ICON),
...
self.addEventListener('notificationclick', (event) => {
  ...
  const trackOpen = notificationId
    ? fetch('/api/pwa/notification-open.php', {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'X-Requested-With': 'MicrogifterPWA'
        },
