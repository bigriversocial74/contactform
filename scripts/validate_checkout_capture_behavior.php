...
function mg_checkout_assert(bool $condition,string $message): void {if(!$condition)throw new RuntimeException($message);}
function mg_checkout_scalar(PDO $pdo,string $sql,array $params=[]): mixed {$stmt=$pdo->prepare($sql);$stmt->execute($params);return $stmt->fetchColumn();}
function mg_checkout_balanced(PDO $pdo,int $groupId): bool {$stmt=$pdo->prepare('SELECT entry_type,SUM(amount_cents) total FROM ledger_entries WHERE transaction_group_id=? GROUP BY entry_type');$stmt->execute([$groupId]);$s=[];foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row)$s[(string)$row['entry_type']]=(int)$row['total'];return ($s['debit']??0)>0&&($s['debit']??0)===($s['credit']??-1);}
function mg_checkout_order_microgift_count(PDO $pdo,int $orderId): int {return (int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM microgift_instances mi INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference WHERE oi.order_id=? AND mi.source_type=?',[$orderId,'commerce_order_item']);}
function mg_checkout_order_action_count(PDO $pdo,int $orderId,int $userId): int {return (int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM microgift_inbox_items ac INNER JOIN microgift_instances mi ON mi.id=ac.instance_id INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference WHERE oi.order_id=? AND ac.user_id=?',[$orderId,$userId]);}

$pdo=mg_db();$runId='checkout_'.bin2hex(random_bytes(8));
$summary=[
    ...
    'session_created'=>false,'session_replay'=>false,'session_conflict_rejected'=>false,
    ...
    'entitlements_granted'=>false,'asset_access_gated'=>false,'microgifts_issued'=>false,
    'microgifts_linked_to_pppm'=>false,'action_center_projected'=>false,'fulfillment_replay_safe'=>false,
...
try{
    ...
    $buyerId=mg_it_user($pdo,$buyerEmail,'Checkout Buyer');$merchantId=mg_it_user($pdo,$merchantEmail,'Checkout Merchant');
    ...
    $conflictDraft=mg_checkout_fixture_draft($pdo,$fixture,'conflict');$checkoutConflict=false;
    ...
    mg_checkout_assert($checkoutConflict,'Conflicting checkout replay was accepted.');$summary['checkout_conflict_rejected']=true;
    ...
    mg_checkout_assert($sessionReplay['duplicate']===true,'Payment session replay was not idempotent.');$summary['session_replay']=true;
    ...
    mg_checkout_assert($sessionConflict,'Conflicting payment session replay was accepted.');$summary['session_conflict_rejected']=true;
    ...
    mg_checkout_assert($groupId>0&&mg_checkout_balanced($pdo,$groupId),'Paid-order ledger group is missing or unbalanced.');$summary['ledger_balanced']=true;
    $wallet=mg_wallet_resolve($pdo,'merchant',$merchantId,'USD');$balances=mg_wallet_balances($pdo,(int)$wallet['id']);
    mg_checkout_assert((int)$balances['available_cents']===2500,'Merchant available balance does not match paid order policy.');$summary['merchant_balance_reconciled']=true;
    ...
    mg_checkout_assert((int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM pppm_items WHERE issuance_request_id=? AND owner_user_id=? AND status=?',[$requestId,$buyerId,'available'])===2,'PPPM ownership or quantity is wrong.');$summary['pppm_issued']=true;
    mg_checkout_assert((int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM entitlements e INNER JOIN commerce_order_items oi ON oi.id=e.commerce_order_item_id WHERE oi.order_id=? AND e.entitled_user_id=? AND e.status=?',[$orderId,$buyerId,'active'])===2,'Entitlements were not granted exactly once.');$summary['entitlements_granted']=true;
    mg_checkout_assert(mg_entitlement_resolve_active($pdo,$buyerId,$catalog['asset_public'])!==null,'Asset was not accessible after payment.');$summary['asset_access_gated']=true;

    mg_checkout_assert(mg_checkout_order_microgift_count($pdo,$orderId)===2,'Paid order did not issue one Microgift per purchased unit.');$summary['microgifts_issued']=true;
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(DISTINCT mi.pppm_item_id) FROM microgift_instances mi INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference INNER JOIN pppm_items p ON p.id=mi.pppm_item_id WHERE oi.order_id=? AND mi.source_type='commerce_order_item' AND mi.owner_user_id=? AND mi.recipient_user_id=? AND p.owner_user_id=? AND p.status='available'",[$orderId,$buyerId,$buyerId,$buyerId])===2,'Purchased Microgifts are not linked one-to-one with buyer-owned PPPM items.');$summary['microgifts_linked_to_pppm']=true;
    ...
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM microgift_inbox_items ac INNER JOIN microgift_instances mi ON mi.id=ac.instance_id INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference WHERE oi.order_id=? AND ac.user_id=? AND ac.folder='inbox' AND ac.state='claimable'",[$orderId,$buyerId])===2,'Paid-order buyer Action Center state is inconsistent.');
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM microgift_inbox_items ac INNER JOIN microgift_instances mi ON mi.id=ac.instance_id INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference WHERE oi.order_id=? AND ac.user_id=? AND ac.folder='sent' AND ac.state='claimable'",[$orderId,$merchantId])===2,'Paid-order merchant Sent projection is inconsistent.');$summary['action_center_projected']=true;
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM microgift_instances mi INNER JOIN commerce_order_items oi ON oi.public_id=mi.source_reference INNER JOIN pppm_items p ON p.id=mi.pppm_item_id WHERE oi.order_id=? AND mi.status='issued' AND p.owner_user_id=mi.owner_user_id",[$orderId])===2,'Purchased Microgifts are not lifecycle-ready.');$summary['lifecycle_ready']=true;

    $receipt=$pdo->prepare('SELECT * FROM receipts WHERE order_id=? LIMIT 1');$receipt->execute([$orderId]);$receiptRow=$receipt->fetch(PDO::FETCH_ASSOC);
    mg_checkout_assert((string)$receiptRow['status']==='finalized'&&(int)$receiptRow['subtotal_cents']===2500&&(int)$receiptRow['total_cents']===2500&&(int)$receiptRow['tax_cents']===0&&(int)$receiptRow['platform_fee_cents']===0,'Receipt snapshot is inconsistent.');$summary['receipt_consistent']=true;
    ...
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM order_audit_events WHERE order_id=? AND event_type='microgift.issued_from_paid_order'",[$orderId])===1,'Microgift fulfillment audit event is missing or duplicated.');$summary['audit_consistent']=true;
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM notifications WHERE user_id IN (?,?) AND type IN ('payment_succeeded','merchant_payment_received')",[$buyerId,$merchantId])===2,'Capture notifications were not created once.');$summary['notifications_once']=true;
    ...
    $summary['capture_replay']=true;$summary['fulfillment_replay_safe']=true;
    ...
    mg_checkout_assert($captureConflict,'Conflicting capture replay was accepted.');$summary['capture_conflict_rejected']=true;

    $failedDraft=mg_checkout_fixture_draft($pdo,$fixture,'failed');$failedOrder=mg_checkout_create_order($pdo,$buyerId,$failedDraft['draft_public'],'failed-order-'.$runId);$failedOrderId=(int)mg_checkout_scalar($pdo,'SELECT id FROM commerce_orders WHERE public_id=?',[$failedOrder['order']['order_id']]);
    $failedSession=mg_payment_create_checkout_session($pdo,$buyerId,$failedOrder['order']['order_id'],'failed-session-'.$runId);$failedIntentId=(int)mg_checkout_scalar($pdo,'SELECT id FROM payment_intents WHERE public_id=?',[$failedSession['payment_intent_id']]);
    ...
    $failedCapture=false;try{mg_finance_record_paid_order($pdo,$failedOrderId,$failedIntentId,'failed-provider-'.$runId,$buyerId);}catch(MgCaptureWorkflowException){$failedCapture=true;}
    ...
    mg_checkout_assert(mg_checkout_order_action_count($pdo,$failedOrderId,$buyerId)===0,'Failed capture created Action Center projections.');$summary['failed_capture_no_fulfillment']=true;

    $rollbackDraft=mg_checkout_fixture_draft($pdo,$fixture,'rollback');$rollbackOrder=mg_checkout_create_order($pdo,$buyerId,$rollbackDraft['draft_public'],'rollback-order-'.$runId);$rollbackOrderId=(int)mg_checkout_scalar($pdo,'SELECT id FROM commerce_orders WHERE public_id=?',[$rollbackOrder['order']['order_id']]);
    $rollbackSession=mg_payment_create_checkout_session($pdo,$buyerId,$rollbackOrder['order']['order_id'],'rollback-session-'.$runId);$rollbackIntentId=(int)mg_checkout_scalar($pdo,'SELECT id FROM payment_intents WHERE public_id=?',[$rollbackSession['payment_intent_id']]);
    $beforeGroups=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM ledger_transaction_groups');$beforeNotifications=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM notifications');
    ...
    mg_checkout_assert((string)mg_checkout_scalar($pdo,'SELECT status FROM receipts WHERE order_id=?',[$rollbackOrderId])==='pending','Post-ledger failure finalized receipt.');$summary['post_ledger_failure_rolled_back']=true;

    $fulfillmentDraft=mg_checkout_fixture_draft($pdo,$fixture,'fulfillment-rollback');$fulfillmentOrder=mg_checkout_create_order($pdo,$buyerId,$fulfillmentDraft['draft_public'],'fulfillment-rollback-order-'.$runId);$fulfillmentOrderId=(int)mg_checkout_scalar($pdo,'SELECT id FROM commerce_orders WHERE public_id=?',[$fulfillmentOrder['order']['order_id']]);
    $fulfillmentSession=mg_payment_create_checkout_session($pdo,$buyerId,$fulfillmentOrder['order']['order_id'],'fulfillment-rollback-session-'.$runId);$fulfillmentIntentId=(int)mg_checkout_scalar($pdo,'SELECT id FROM payment_intents WHERE public_id=?',[$fulfillmentSession['payment_intent_id']]);
    $beforeGroups=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM ledger_transaction_groups');$beforeNotifications=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM notifications');$beforePppm=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM pppm_items');$beforeEntitlements=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM entitlements');$beforeMicrogifts=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM microgift_instances');$beforeActions=(int)mg_checkout_scalar($pdo,'SELECT COUNT(*) FROM microgift_inbox_items');
    ...
    mg_checkout_assert((string)mg_checkout_scalar($pdo,'SELECT status FROM receipts WHERE order_id=?',[$fulfillmentOrderId])==='pending','Post-fulfillment failure finalized receipt.');$summary['post_fulfillment_failure_rolled_back']=true;
    ...
    mg_checkout_assert((int)mg_checkout_scalar($pdo,"SELECT COUNT(*) FROM microgift_instances WHERE idempotency_key LIKE ?",['commerce-order-item:%'.$runId.'%'])===0,'Checkout Microgifts remain.');$summary['fixtures_clean']=true;
