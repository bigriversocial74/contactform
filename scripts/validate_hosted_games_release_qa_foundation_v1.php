...
$must=static function(string $path,array $needles)use(&$errors,$read):void{$content=$read($path);foreach($needles as $needle){if(!str_contains($content,$needle))$errors[]="{$path} missing contract: {$needle}";}};
$forbid=static function(string $path,array $needles)use(&$errors,$read):void{$content=$read($path);foreach($needles as $needle){if(str_contains($content,$needle))$errors[]="{$path} contains forbidden contract: {$needle}";}};
...
$must('includes/hosted-game-upload.php',["'draft'",'package_zip_storage_key','package_zip_bytes','Unable to preserve the original uploaded ZIP','current live release']);
...
$must('includes/hosted-game-preview.php',['mg_hosted_game_preview_access','mg_hosted_game_preview_session','mg_hosted_game_preview_runtime','test_reward_','simulated_delivered','inventory_consumed'=>false,'hosted_game_test_runs','hosted_game_test_events','hosted_game_test_state','mg_hosted_game_preview_reset']);
...
$must('assets/js/hosted-game-preview.js',["'telemetry'",'sdk_request','runtime_error','data-console-tab','data-viewport','data-preview-reset','preview-runtime.php']);
...
$must('assets/js/hosted-game-releases.js',['Upload draft release','Compare manifests','Roll back to this release','Download original ZIP','Preview & test','Run health check']);
