...
$checks[]=['name'=>'permission-matrix authority','ok'=>str_contains($permissions,"'admin.loyalty_quests' => ['campaign.manage']")&&str_contains($api,"mg_require_permission('campaign.manage')")&&str_contains($page,"mg_require_admin_page_permission('admin.loyalty_quests')")];
$checks[]=['name'=>'admin sidebar integration','ok'=>str_contains($sidebar,"'loyalty-quests'")&&str_contains($sidebar,'/admin/loyalty-quests.php')&&str_contains($sidebar,'Loyalty Quests')];
...
$checks[]=['name'=>'safe delivery recovery','ok'=>str_contains($service,"['failed','dead_letter']")&&str_contains($service,"\$job['status']==='processing'")&&str_contains($service,'time()-900')&&str_contains($service,'min(10')&&str_contains($service,"status='queued'")];
...
$checks[]=['name'=>'ownership authority documented','ok'=>str_contains($docs,'Microgift → PPPM → Inbox')&&str_contains($docs,'does not approve or reject participant evidence')&&str_contains($page,'PPPM ownership')];
