...
$checks[]=['name'=>'guarded production installer','ok'=>lqpf_has($installer,["require __DIR__.'/install-lock.php'",'lqi_guard_installer();','lqi_sql_statements','local_quest_production_foundation_v1.sql',"'driver'=>'mysql'",'lqi_atomic_write','lqi_api_test','lqi_write_lock();','assets/form-review.js'])];
...
$checks[]=['name'=>'professional public landing page','ok'=>lqpf_has($contents['cover'],["header('Location: install.php')",'assets/public-site.css','meta name="description"','application/ld+json','Featured local quests','From local action to connected reward.','A wallet built for what happens after the quest.','Merchant and community programs'])&&str_contains($contents['public_css'],'.hero-visual')&&str_contains($contents['public_css'],'.wallet-section')];
...
$checks[]=['name'=>'production schema version package','ok'=>lqpf_has($contents['migration'],['CREATE TABLE IF NOT EXISTS lqr_schema_versions','2026.07.10-production-foundation-v1'])];
