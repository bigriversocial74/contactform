...
$check(str_contains($helper, "mg_security_log('warning', 'admin.mcp_runtime_credentials.generated'"), 'credential_security_event');
...
$check(str_contains($helper, 'SELECT id FROM mcp_connections WHERE public_id=? LIMIT 1 FOR UPDATE'), 'simple_connection_row_lock');
$check(!str_contains($helper, 'GROUP BY c.id,u.id,cl.id,mw.id" . $suffix'), 'aggregate_lock_removed');
$check(str_contains($helper, 'Expired connections cannot be resumed.'), 'expired_resume_denied');
