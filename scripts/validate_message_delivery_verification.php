...
function must_contain_any(string $path, array $needles, array &$failures): void
{
    $source = file_get_contents(dirname(__DIR__) . '/' . $path);
    if (!is_string($source)) {
    ...
    $failures[] = $path . ' missing one of: ' . implode(', ', $needles);
}
foreach ([
...
must_contain_any('assets/js/messages-center.js', ['Delivery context', 'mg-thread-details-popover', 'contextItems(source)'], $failures);
