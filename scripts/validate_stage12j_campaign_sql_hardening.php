...
$stage12bFile = $root . '/database/stage_12b_campaign_events_agent_context.sql';
...
$stage12bSql = is_file($stage12bFile) ? (string) file_get_contents($stage12bFile) : '';
$checks = [
    ...
    'hardening_is_pdo_safe' => !str_contains($hardeningSql, 'DELIMITER') && !str_contains($hardeningSql, 'CREATE PROCEDURE') && !str_contains($hardeningSql, 'ALTER TABLE'),
    'hardening_marker' => str_contains($hardeningSql, 'stage_12j_campaign_feature_sql_hardening'),
    'stage12b_campaign_events_campaign_nullable' => str_contains($stage12bSql, 'MODIFY campaign_id BIGINT UNSIGNED NULL'),
    ...
    'campaign_events_campaign_nullable' => str_contains($fullSql, 'campaign_id BIGINT UNSIGNED NULL'),
    'full_import_idempotent_index_helper' => str_contains($fullSql, 'mg_add_index_if_missing') && str_contains($fullSql, 'information_schema.statistics'),
