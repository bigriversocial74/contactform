...
$checks[]=['name'=>'risk review precedes progress and reward','ok'=>str_contains($submit,'$requiresReview = !$verificationPassed || $integrity[\'decision\'] === \'review\'')&&str_contains($submit,"$evidenceStatus = $requiresReview ? 'submitted' : 'verified'")&&str_contains($submit,"status='pending_review'")&&strpos($submit,'if ($requiresReview)')<strpos($submit,'$newProgress =')&&strpos($submit,'$newProgress =')<strpos($submit,'mg_lqr_issue_reward')];
