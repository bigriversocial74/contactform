...
$expect=static function(bool $ok,string $label)use(&$failures,&$passes):void{
    ...
    if($ok)$passes++;else$failures[]=$label;
};
...
try{
    ...
    $expect(
        ...
        &&str_contains($helper,"'action_item'")
        &&str_contains($helper,"'counts'")
    ...
    foreach(['read','unread','archive','restore']as$name){
        $source=$read('api/account/action-center-'.$name.'.php');
        $expect(
    ...
    $expect(
        ...
        &&str_contains($actions,"typeof api.refresh === 'function'"),
    ...
    $expect(
        ...
        'Action Center loads Contract v2, Runtime v4, and Mutation v1 in order'
    );
    $expect(
        ...
        'Exact-recipient regift delegates list reconciliation to Mutation v1'
    );
    $expect(
        ...
        'Voucher preparation and manual redemption retain their specialized flow'
    );
    ...
    $expect(str_contains($authorities['regift'],'FOR UPDATE')&&str_contains($authorities['regift'],'mg_pppm_transfer_owner_canonical(')&&str_contains($authorities['regift'],'idempotency_key'),'Regift keeps locking, PPPM ownership, and idempotency');
    $expect(str_contains($authorities['claim'],'mg_microgift_claim_canonical(')&&str_contains($authorities['claim'],'FOR UPDATE')&&str_contains($authorities['claim'],'idempotency_key'),'Claim keeps canonical claim, locking, and replay authority');
    $expect(str_contains($authorities['follow_up'],'Only the most recent sender can follow up.')&&str_contains($authorities['follow_up'],'mg_message_send_microgift(')&&!str_contains($authorities['follow_up'],'mg_pppm_transfer_owner_canonical('),'Follow Up retains messaging without ownership mutation');
    $expect(str_contains($authorities['message'],'Message recipient does not match this transfer.')&&str_contains($authorities['message'],'mg_message_send_microgift(')&&str_contains($authorities['message'],'idempotencyKey'),'Message retains participant binding and idempotency');
    $expect(str_contains($authorities['tip'],"mg_require_permission('tips.create')")&&str_contains($authorities['tip'],'mg_tip_create(')&&str_contains($authorities['tip'],"folder']!=='claimed'"),'Tip retains permission, eligibility, and tip-service authority');
    $expect(str_contains($authorities['voucher_token'],'mg_claim_voucher_issue_token(')&&str_contains($authorities['voucher_token'],'mg_wallet_claim_voucher_issue_token(')&&str_contains($authorities['voucher_token'],'900'),'Voucher preparation retains signed short-lived tokens');
    $expect(str_contains($authorities['voucher_claim'],'mg_ac_voucher_assert_not_locked(')&&str_contains($authorities['voucher_claim'],'merchant_claim_codes')&&str_contains($authorities['voucher_claim'],'action_center_voucher_claim_attempts'),'Manual voucher redemption retains lockout and merchant-code authority');
    $expect(str_contains($authorities['scanner'],"mg_require_permission('merchant.gifts.redeem')")&&str_contains($authorities['scanner'],"['verify','redeem']")&&str_contains($authorities['scanner'],'mg_claim_voucher_require_active(')&&str_contains($authorities['scanner'],'mg_wallet_claim_voucher_require_active('),'Merchant scanner retains permission and signed-token authority');
    $expect(str_contains($docs,'Mutation Contract v1')&&str_contains($docs,'separate')&&str_contains($docs,'No SQL required.'),'Architecture and SQL boundary are documented');
}catch(Throwable$error){$failures[]=$error->getMessage();echo'[FAIL] '.$error->getMessage().PHP_EOL;}
