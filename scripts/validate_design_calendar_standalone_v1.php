...
$loadedTemplateCss = $read('assets/css/design-studio-loaded-template-v1.css');
$calendarCss = $read('assets/css/design-calendar-standalone.css');
$designJs = $read('assets/js/personal-agent-design-studio.js');
$creativeSaveJs = $read('assets/js/design-studio-creative-save.js');
...
$checks = [
    ...
    'Design header displays merchant profile URL instead of studio label' => str_contains($designView, 'mg-agent-design-profile-link') && str_contains($designView, '<?= mg_e($designDestination) ?>') && !str_contains($designView, 'Merchant Design Studio') && !str_contains($designView, 'mg-agent-design-profile-card'),
    'Print save and download actions live in the header and stay hidden until loaded' => str_contains($designView, 'data-design-header-actions hidden') && str_contains($designView, 'data-design-save-asset') && str_contains($designView, 'data-design-download') && str_contains($designJs, 'showHeaderActions(true)') && str_contains($designJs, 'showHeaderActions(false)') && !str_contains($creativeSaveJs, "addButton(app.querySelector('.mg-agent-design-actions')") && str_contains($loadedTemplateCss, '.mg-agent-design-header-actions[hidden]'),
    'Print workspace shows templates first and opens a dedicated loaded section' => str_contains($designView, 'data-design-template-browser') && str_contains($designView, 'data-design-loaded-section hidden') && str_contains($designView, 'data-design-close-template') && str_contains($designJs, 'templateBrowser.hidden = true') && str_contains($designJs, 'loadedSection.hidden = false') && str_contains($designJs, 'closeTemplateButton?.addEventListener'),
    'Selected format card and empty helper copy are removed from loaded preview' => !str_contains($designView, 'mg-agent-design-object-summary') && !str_contains($designView, 'data-design-empty') && !str_contains($designView, 'Select a template from the side rail') && str_contains($loadedTemplateCss, '.mg-agent-design-loaded-section'),
