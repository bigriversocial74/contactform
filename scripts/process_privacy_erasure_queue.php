...
require_once dirname(__DIR__) . '/includes/privacy/finalization-operations.php';
...
$where = ['pr.request_type="delete"','pr.user_id IS NOT NULL','pr.status IN ("approved","restricted","partially_completed")'];
$params = [];
if ($requestId > 0) {
    $where[] = 'pr.id=?';
    $params[] = $requestId;
} elseif (!$force) {
    $where[] = 'COALESCE(pr.grace_ends_at,pr.response_due_at)<=NOW()';
}
...
foreach ($items as $item) {
    ...
    try {
        ...
        $result = mg_privacy_finalize_with_operations($pdo,(int) $item['id'],null,$force);
        $pdo->commit();
        $status = (string) ($result['status'] ?? 'unknown');
        if ($status === 'completed') {
            $completed++;
        } elseif (in_array($status,['blocked_by_hold','partially_completed'],true)) {
            $blocked++;
        } elseif (!empty($result['not_due'])) {
            $notDue++;
        }
        printf("  result=%s%s%s\n",$status,!empty($result['receipt_hash'])?' receipt='.$result['receipt_hash']:'',!empty($result['pending_handoffs'])?' pending_handoffs='.$result['pending_handoffs']:'');
