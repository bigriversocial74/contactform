...
$checks = [
    ...
    'Resolver uses the exact purchased order-line version with instance fallback' => str_contains($source['resolver'], 'LEFT JOIN commerce_order_items coi ON coi.id=i.commerce_order_item_id') && str_contains($source['resolver'], 'cpv.id=COALESCE(coi.product_version_id,i.product_version_id)') && str_contains($source['resolver'], "'exact_instance_version'"),
    'Resolver uses product thumbnail before cover and blocks storefront fallback' => str_contains($source['resolver'], "pva.role IN ('thumbnail','cover')") && str_contains($source['resolver'], "FIELD(pva.role,'thumbnail','cover')") && str_contains($source['resolver'], "if (\$productId !== '') \$item['merchant_avatar_url'] = '';"),
