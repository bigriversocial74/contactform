...
$files = [
    ...
    'builder' => $root . '/assets/js/builder-stage4b.js',
...
$checks = [
    ...
    'Product builder stores the primary product photo as thumbnail media' => str_contains($builder, "var assets = { thumbnail: ''") && str_contains($builder, 'var directUrl = assetUrls.thumbnail'),
    'Resolver selects the product thumbnail before the cover image' => str_contains($resolver, "pva.role IN ('thumbnail','cover')") && str_contains($resolver, "FIELD(pva.role,'thumbnail','cover')") && str_contains($resolver, "ready_image.asset_type='image'"),
    'Catalog-linked gifts cannot fall through to the merchant storefront image' => str_contains($resolver, "if (\$productId !== '') \$item['merchant_avatar_url'] = '';"),
    ...
    'Contract presentation resolves exact catalog product media before gift fallbacks' => strpos($contract, 'catalog_product_version_cover') !== false && strpos($contract, 'catalog_product_version_cover') < strpos($contract, 'gift.custom_image'),
    ...
    'Runtime v4 renders the Contract presentation image before generic merchant fallback' => str_contains($runtimeV4, 'p.presentation.image_url||p.merchant.avatar_url') && str_contains($runtimeV4, 'const image=safeUrl('),
