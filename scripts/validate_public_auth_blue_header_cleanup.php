...
try {
    ...
    $expect(
        str_contains($authCss, 'body.mg-auth-page[data-authenticated="false"]{')
        && str_contains($authCss, 'body.mg-auth-page[data-authenticated="false"] .mg-main{')
        ...
        'Auth pages override the logged-out shell with the main index white background'
    );
    ...
    $expect(
        str_contains($page, "'auth-pages'=>['styles'=>['/assets/css/auth-page.css?v=6.1.0']]")
        && str_contains($signin, 'class="mg-auth-shell"'),
        'Auth pages load the v6.1 cache-busted shared stylesheet without changing the sign-in structure'
    );
