...
$hardeningFiles=['includes/investment/investment-audit-hardening.php'];for($i=2;$i<=14;$i++)$hardeningFiles[]='includes/investment/investment-audit-hardening-v'.$i.'.php';
$required=array_merge([
    'database/20260724_investor_module_audit_hardening_v1.sql','assets/js/investor-module-audit-v1.js','docs/investment/investor-module-audit-10of10.md',
    '.github/workflows/investor-module-audit-10of10.yml','scripts/validate_investor_module_publication_v12.php','scripts/validate_investor_module_exact_approval_v13.php',
],$hardeningFiles);
foreach($required as $path)if(!is_file($root.'/'.$path))$failures[]='Required file missing: '.$path;

$service=$read('includes/investment/investment-service.php');$accessApi=$read('api/investment/access-request.php');$wizardApi=$read('api/admin/investment-wizard.php');$pipelineApi=$read('api/admin/investor-pipeline.php');$diligenceApi=$read('api/admin/investor-diligence.php');$closingApi=$read('api/admin/investment-closing.php');$governanceApi=$read('api/admin/investor-governance.php');$portalApi=$read('api/investment/portal.php');
$hardening='';foreach($hardeningFiles as $path)$hardening.=$read($path);
$h1=$read($hardeningFiles[0]);$h3=$read($hardeningFiles[2]);$h4=$read($hardeningFiles[3]);$h5=$read($hardeningFiles[4]);$h6=$read($hardeningFiles[5]);$h7=$read($hardeningFiles[6]);$h8=$read($hardeningFiles[7]);$h9=$read($hardeningFiles[8]);$h10=$read($hardeningFiles[9]);$h11=$read($hardeningFiles[10]);$h12=$read($hardeningFiles[11]);$h13=$read($hardeningFiles[12]);$h14=$read($hardeningFiles[13]);
...
// Architecture — 15.
$layerNeedles=['investment-audit-hardening.php'];for($i=2;$i<=14;$i++)$layerNeedles[]='investment-audit-hardening-v'.$i.'.php';
$check('Architecture',5,$all($service,$layerNeedles),'All fourteen audited authority layers load after the Phase 1–5 services.');
$check('Architecture',5,
    ...
    &&$all($governanceApi,['mg_investment_governance_dashboard_audited_v2','mg_investment_governance_save_meeting_audited_v3','mg_investment_governance_save_packet_document_audited_v2','mg_investment_governance_save_notice_audited_v5','mg_investment_governance_save_obligation_audited_v3'])
    &&$all($portalApi,['mg_investment_portal_data_v5_final3','mg_investment_portal_event_v5_final3']),'Every investment API routes through the final audited authority.');
$check('Architecture',5,$all($pipelineApi,["'dashboard'=>mg_investment_pipeline_dashboard_audited","'sync_profiles'=>"])&&$all($closingApi,["'dashboard'=>mg_investment_closing_dashboard_audited","'sync'=>mg_investment_closing_sync_audited"])&&str_contains($governanceApi,'mg_investment_governance_dashboard_audited_v2'),'Read dashboards are separated from explicit protected synchronization.');
...
$check('Security & privacy',4,$all($h6,['A document cannot be less restricted than its data-room folder',"\$folders[(string)\$folder['public_id']]"])&&$all($h8,['funding_verification_source="maker_checker"',"\$portalRound['governance']=null"])&&str_contains($h1,'investor_visible=1'),'Folder, selected-investor, consent-visibility and maker/checker-funded boundaries are enforced.');
...
$check('Publication & governance',3,$all($h12,['Approve the exact investment document version','Approve the exact tax-document version','The published tax document must exactly match its approved version and metadata'])&&!str_contains($h12,'v.prepared_by'),'Official and tax publication require exact approved data and valid schema.');
$check('Publication & governance',3,$all($h13,['Approve the exact data-room document version','Approve the exact Q&A entry','Approve the exact investor communication','Approve the exact funded-investor meeting summary','Approve the exact board packet document','Approve the exact written consent','Approve the exact material notice','Active or investor-visible rights must exactly match','Approve the exact reporting obligation','investment_board_packet_document_published']),'Every investor-visible content transition requires exact prior approval, including atomic packet publication.');
$check('Publication & governance',3,$all($h14,['Published meeting status and counsel authority must exactly match','Published notice counsel authority must exactly match','Published obligation status and counsel-review requirement must exactly match'])&&$all($governanceApi,['mg_investment_governance_save_meeting_audited_v3','mg_investment_governance_save_notice_audited_v5','mg_investment_governance_save_obligation_audited_v3']),'Investor-visible status and approval metadata cannot change during publication.');

// Runtime and data integrity — 15.
...
foreach(['mg_investment_decimal_input_audited','mg_investment_update_round_audited','mg_investment_pipeline_save_interest_audited_v2','mg_investment_closing_sync_audited','mg_investment_financial_decide_audited_v3','mg_investment_portal_data_v5_final3','mg_investment_governance_dashboard_audited_v2','mg_investment_dataroom_save_document_audited_v2','mg_investment_governance_save_packet_document_audited_v2','mg_investment_governance_save_meeting_audited_v3','mg_investment_governance_save_notice_audited_v5','mg_investment_governance_save_obligation_audited_v3'] as $function)if(!function_exists($function))$failures[]='Loaded service is missing audited authority: '.$function;
try{if(mg_investment_decimal_input_audited('$1,234.50','Test amount')!=='1234.50')$failures[]='Exact decimal normalization failed.';try{mg_investment_decimal_input_audited('1.234','Test amount');$failures[]='Exact decimal validation accepted more than two decimal places.';}catch(MgInvestmentException){}}catch(Throwable $error){$failures[]='Exact decimal test failed unexpectedly: '.$error->getMessage();}
foreach($categories as $category=>$result)echo sprintf("[CATEGORY] %s: %d/%d\n",$category,(int)($result['score']??0),(int)$result['maximum']);echo "[SCORE] {$score}/{$maximum}\n";
