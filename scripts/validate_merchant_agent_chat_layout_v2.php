...
$checks = [
    'page loads canonical layout last' => str_contains($page, "'/assets/css/merchant-agent-chat-layout-v2.css?v=2.0.0',"),
    ...
    'composer is a viewport grid row instead of fixed overlay' => str_contains($css, 'grid-template-rows:minmax(0,1fr) auto auto!important')
        && str_contains($css, '.mg-agent-chat-layout-v2 .mg-agent-chat-composer-shell')
        && str_contains($css, 'position:relative!important')
        && !str_contains($css, '.mg-agent-chat-layout-v2 .mg-agent-chat-composer-shell{\n  position:fixed'),
    ...
    'mobile composer remains one clean four-column row' => str_contains($css, 'grid-template-columns:40px minmax(0,1fr) 40px 40px!important')
    ...
    'dedicated drawer supports compact tablet and mobile viewports' => str_contains($js, "matchMedia('(max-width: 980px)')")
...
echo "Merchant agent chat layout v2 contract passed at 10.0/10.\n";
