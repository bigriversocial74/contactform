...
$service=$read('includes/investment/investment-service.php');$v12=$read('includes/investment/investment-audit-hardening-v12.php');$v15=$read('includes/investment/investment-audit-hardening-v15.php');$wizardApi=$read('api/admin/investment-wizard.php');$governanceApi=$read('api/admin/investor-governance.php');$workflow=$read('.github/workflows/investor-module-audit-10of10.yml');

$pass(str_contains($service,'investment-audit-hardening-v12.php')&&str_contains($service,'investment-audit-hardening-v15.php'),'Service loader includes approved-version and serialized publication hardening.');
$pass(str_contains($wizardApi,'mg_investment_save_documents_audited_v3')&&str_contains($v15,'mg_investment_save_documents_audited_v2'),'Investment Wizard routes serialized writes to approved-version document publication.');
$pass(str_contains($governanceApi,'mg_investment_governance_save_tax_document_audited_v4')&&str_contains($v15,'mg_investment_governance_save_tax_document_audited_v3'),'Governance routes serialized writes to approved-version tax publication.');
...
$pass(str_contains($v15,"'official_documents'")&&str_contains($v15,"'tax_document'"),'Official and tax document transitions use entity serialization.');
...
$pass(function_exists('mg_investment_save_documents_audited_v3'),'Loaded service exposes serialized official-document publication authority.');
$pass(function_exists('mg_investment_governance_save_tax_document_audited_v4'),'Loaded service exposes serialized tax-document publication authority.');

if($failures!==[]){foreach($failures as $failure)fwrite(STDERR,"[PUBLICATION AUDIT FAILURE] {$failure}\n");exit(1);}echo "Investor module publication v12/v15: {$checks}/{$checks} checks passed.\n";
