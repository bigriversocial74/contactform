...
$stmt = $pdo->query("SELECT COUNT(*) FROM permissions WHERE slug IN ('merchant.intelligence.view','creator.campaign_tracking.view_own')");
$check((int) $stmt->fetchColumn() === 2, 'Phase 10 reused permissions are unavailable.');
$stmt = $pdo->query("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND ((table_name='creator_campaign_tracking_events' AND column_name IN('status','event_type','is_unique','occurred_at')) OR (table_name='creator_campaign_attributions' AND column_name IN('status','attributed_at')) OR (table_name='creator_campaign_earning_events' AND column_name IN('amount_minor','currency')) OR (table_name='creator_campaign_payouts' AND column_name IN('amount_minor','currency','status')))");
$check((int) $stmt->fetchColumn() === 11, 'Phase 10 reporting columns are incomplete.');
...
echo json_encode(['ok'=>true,'sql_required'=>false,'authoritative_tables'=>count($required),'duplicate_metric_store'=>false,'currency_safe'=>true,'accepted_events_only'=>true], JSON_PRETTY_PRINT | JSON_THROW_ON_ERROR) . PHP_EOL;
