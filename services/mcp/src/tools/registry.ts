...
function bridgeError(error: unknown): CanonicalBridgeError {
  return error instanceof CanonicalBridgeError
    ? error
...
export function createInternalMcpServer(dependencies: ToolRegistryDependencies): McpServer {
  const server = new McpServer(
    { name: "microgifter-mcp", version: "0.4.0", description: "Microgifter MCP read and review-only draft server" },
    ...
    server.registerTool(
      ...
      async () => {
        ...
        const output = {
          ...
          expires_at: dependencies.connection.expiresAt ?? null,
        };
    ...
    server.registerTool(
      "microgifter.catalog.search",
      {
        ...
        inputSchema: {
          ...
          cursor: z.string().max(900).optional(),
        },
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
      },
      async (arguments_) => {
