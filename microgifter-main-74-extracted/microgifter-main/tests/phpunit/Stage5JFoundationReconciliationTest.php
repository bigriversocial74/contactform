...
final class Stage5JFoundationReconciliationTest extends TestCase
{
    public function testSchemaRestoresOriginalStageFiveResources(): void
    {
        ...
        foreach(['carts','cart_items','checkout_drafts','order_fee_snapshots','order_status_history','receipts','order_audit_events'] as $table){self::assertStringContainsString('CREATE TABLE IF NOT EXISTS '.$table,$sql);}
    }

    public function testStage5JRunnerAppliesCommerceMicrogiftCompatibilityBeforeFoundation(): void
    {
    ...
    public function testOrderCreationConsumesOneOpenDraftAndCreatesImmutableRecords(): void
    {
        ...
        self::assertIsString($endpoint);self::assertIsString($service);
        ...
        foreach(['FOR UPDATE','checkout_drafts','commerce_orders','commerce_order_items','order_fee_snapshots','receipts','mg_order_history','mg_order_event'] as $needle){self::assertStringContainsString($needle,$service);}
    ...
    public function testPaymentSessionStartsFromAnExistingOrder(): void
    {
        ...
        self::assertStringContainsString('Payment session idempotency key is already bound to this order.',$service);
