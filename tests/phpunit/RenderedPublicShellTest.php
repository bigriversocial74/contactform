...
final class RenderedPublicShellTest extends TestCase
{
    private function fetchPage(string $path): string
    {
        ...
        if ($base === '') {
            self::markTestSkipped('MG_TEST_BASE_URL is not configured.');
        }
        ...
        if (!is_string($html)) {
            self::markTestSkipped('MG_TEST_BASE_URL is configured but not reachable for rendered public shell test: ' . $base . $path);
        }
        return $html;
    }

    /** @dataProvider publicPageProvider */
    public function testPublicPagesRenderOneUniversalHeader(string $path, string $pageId): void
    {
        $html = $this->fetchPage($path);
        self::assertSame(1, substr_count($html, 'data-mg-universal-header'), $path);
        self::assertStringContainsString('data-header-variant="logged-out"', $html, $path);
        ...
        self::assertSame(0, substr_count($html, 'data-header-page-controls'), $path);
    ...
    public function testLearnMoreKeepsPresentationAssetsWithoutHeaderControl(): void
    {
        $html = $this->fetchPage('/learn-more.php');
        self::assertStringContainsString('/assets/js/agent-presentation.js', $html, '/learn-more.php');
        self::assertStringContainsString('/assets/css/universal-header.css', $html, '/learn-more.php');
    }
    ...
    public function testAuthPagesRenderAuthShellWithoutPresentationAssets(string $path): void
    {
        ...
        self::assertStringNotContainsString('/assets/css/agent-presentation-layout.css', $html, $path);
