...
final class TaskAgentMemoryPermissionSafeRetrievalV1ContractTest extends TestCase
{
    public function testMemoryAuthoritySanitizesAndScopesEveryOperation(): void
    {
        ...
        foreach(['mg_task_agent_memory_sensitive','mg_task_agent_memory_sanitize','mg_task_agent_memory_list','mg_task_agent_memory_save','mg_task_agent_memory_archive','mg_task_agent_memory_search','mg_task_agent_memory_for_model'] as $marker)self::assertStringContainsString($marker,$memory);
    ...
    public function testDeterministicRetrievalRunsBeforeAnthropicAndUsesSafeContext(): void
    {
        ...
        $router=file_get_contents($root.'/includes/task-agent-intent-router.php');
        $memory=file_get_contents($root.'/includes/task-agent-memory.php');
        foreach([$runtime,$ai,$router,$memory] as $value)self::assertIsString($value);
        ...
        $synthesisPosition=strpos($runtime,'mg_task_agent_ai_synthesis');
        self::assertNotFalse($routePosition);
        self::assertNotFalse($synthesisPosition);
        self::assertLessThan($synthesisPosition,$routePosition);
        self::assertStringContainsString('mg_task_agent_model_context',$router);
        self::assertStringContainsString('Permission-safe focused context JSON',$ai);
