...
final class CreatorCampaignMcpCanonicalV13cContractTest extends TestCase
{
    ...
    {
        $page=$this->read('account-creator-campaign-actions.php').$this->read('includes/mcp-creator-campaign-actions/owner-page-view.php');$owner=$this->read('includes/mcp-creator-campaign-actions/owner-service.php');$execution=$this->read('includes/mcp-creator-campaign-actions/execution-service.php');
        self::assertStringContainsString('mg_mcp_creator_campaign_action_decide',$page);self::assertStringContainsString('mg_mcp_creator_campaign_action_execute',$page);self::assertStringContainsString('confirm_execute',$page);self::assertStringContainsString('Execute approved action',$page);
        self::assertStringContainsString("status='approved'",$owner);self::assertStringContainsString('MCP_CREATOR_CAMPAIGN_ACTION_STATE_CHANGED',$execution);self::assertStringContainsString('mg_mcp_automation_authorize_grant_action',$execution);self::assertStringContainsString('approval_expires_at',$execution);
    }
    ...
    public function testExecutionUsesOnlyNativeServicesAndCanonicalReceipts():void
    {
        ...
        foreach(['mg_creator_campaign_transition_status','mg_creator_campaign_application_review_merchant','mg_creator_campaign_invitation_create_merchant','mg_creator_campaign_agreement_offer_merchant','mg_creator_campaign_participant_transition_merchant','mg_creator_campaign_submission_review_merchant','mg_creator_campaign_attribution_override_merchant','mg_creator_campaign_earning_decide_merchant','mg_creator_campaign_payout_create','mg_creator_campaign_dispute_transition'] as $function)self::assertStringContainsString($function,$execution);
        self::assertStringContainsString('INSERT INTO mcp_action_receipts',$execution);self::assertStringContainsString('before_state_token',$execution);self::assertStringContainsString('after_state_token',$execution);self::assertStringNotContainsString('provider_reference',$execution);
    }
    ...
    {
        $earning=$this->read('includes/creator-campaigns/earning-service.php');$sql=$this->read('database/20260722_creator_campaign_mcp_canonical_actions_v13c_single_install.sql');
        self::assertStringContainsString('merchant.creator_earnings.manage',$earning);self::assertStringContainsString('mg_creator_campaign_compensation_reverse',$earning);self::assertStringContainsString('creator_campaign_earning_reviews',$earning);self::assertStringContainsString('merchant.creator_earnings.manage',$sql);
    }
}
