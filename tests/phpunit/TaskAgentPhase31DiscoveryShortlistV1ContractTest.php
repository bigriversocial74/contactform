...
final class TaskAgentPhase31DiscoveryShortlistV1ContractTest extends TestCase
{
    ...
    public function testDiscoveryAndShortlistUsePublishedCanonicalProductsWithoutAi(): void
    {
        ...
        foreach([
            ...
            'mg_task_agent_shortlist_add',
            'mg_task_agent_shortlist_remove',
            'owner_user_id=? AND agent_id=?',
            'multi_agent.discovery_completed',
            'multi_agent.shortlist_added',
            'multi_agent.shortlist_removed',
    ...
    public function testApiAndCanvasExposeOnlyReviewAndShortlistActions(): void
    {
        ...
        foreach(['mg_agent_require_owned','mg_require_csrf_for_write',"$action==='discover_products'","$action==='add_shortlist'","$action==='remove_shortlist'","'used_ai'=>false"] as $marker)self::assertStringContainsString($marker,$api);
    ...
    public function testComparisonReceivesOnlyCompactPublishedShortlistContext(): void
    {
        ...
        foreach([$service,$router,$runtime] as $value)self::assertIsString($value);
        self::assertStringContainsString('mg_task_agent_shortlist_for_model',$service);
        self::assertStringContainsString('array_slice($items,0,8)',$service);
        ...
        self::assertStringNotContainsString('recipient_context',$service, 'Model shortlist projection must not include recipient contact identifiers.');
    }
}
