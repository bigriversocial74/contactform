...
final class ProductionCheckoutCaptureBehaviorTest extends TestCase
{
    public function testCheckoutCaptureIssuanceAndFulfillmentAgainstRealDatabase(): void
    {
        ...
        $required=[
            'checkout_created','checkout_replay','checkout_conflict_rejected',
            'session_created','session_intent_linked','session_replay','session_conflict_rejected','expired_session_recovery',
            'capture_completed','capture_replay','capture_conflict_rejected','ledger_balanced','merchant_balance_reconciled',
            'pppm_issued','pppm_issuer_authoritative','entitlements_granted','asset_access_gated',
            'microgifts_issued','microgifts_linked_to_pppm','action_center_projected','buyer_inbox_only',
            'issuance_summary_complete','fulfillment_replay_safe','lifecycle_ready','receipt_consistent',
            'audit_consistent','notifications_once','failed_capture_no_fulfillment',
    ...
    public function testEndpointsUseCanonicalCheckoutAndCaptureAuthorities(): void
    {
        ...
        $checkout=file_get_contents($root.'/api/commerce/_checkout.php');
    ...
    public function testComposerAndFocusedWorkflowRunPaidCheckoutFulfillment(): void
    {
        ...
        $workflow=file_get_contents($root.'/.github/workflows/paid-checkout-fulfillment-validation.yml');
        self::assertIsString($composer);self::assertIsString($workflow);
