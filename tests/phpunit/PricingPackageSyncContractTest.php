...
final class PricingPackageSyncContractTest extends TestCase
{
    ...
    public function testPublicPricingPageReadsSharedPackageSource(): void
    {
        ...
        self::assertIsString($source);
        foreach(["require_once __DIR__ . '/includes/pricing-packages.php" ,'$plans = mg_public_pricing_packages();','$summary = mg_pricing_package_summary();','data-package-id','Simple pricing for local growth','foreach ($plans as $plan)'] as $needle){ self::assertStringContainsString($needle,$source); }
