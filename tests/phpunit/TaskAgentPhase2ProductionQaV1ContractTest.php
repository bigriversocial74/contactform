...
final class TaskAgentPhase2ProductionQaV1ContractTest extends TestCase
{
    ...
    private function source(string $path): string { $source=file_get_contents($this->root.'/'.$path);self::assertIsString($source,'Unable to read '.$path);return $source; }
    ...
    {
        $runtime=$this->source('includes/multi-agent-runtime.php');$router=$this->source('includes/task-agent-intent-router.php');$context=$this->source('includes/task-agent-context.php');$memory=$this->source('includes/task-agent-memory.php');
        $routePosition=strpos($runtime,'mg_task_agent_shortlist_route');$providerPosition=strpos($runtime,'mg_anthropic_messages');self::assertNotFalse($routePosition);self::assertNotFalse($providerPosition);self::assertLessThan($providerPosition,$routePosition);
        ...
        $compact=$this->compact($runtime);self::assertStringContainsString("if(!$result&&$aiReason!=='')",$compact);self::assertStringContainsString("mg_multi_agent_runtime_messages($pdo,$userId,(int)$agent['id'],(int)$thread['id'],8)",$compact);self::assertStringContainsString('max(350,min(900',$compact);
    ...
    {
        $runtime=$this->source('includes/multi-agent-runtime.php');$router=$this->source('includes/task-agent-intent-router.php');$api=$this->source('api/agents/runtime.php');$memory=$this->source('includes/task-agent-memory.php');
        foreach(['Never reveal secrets','All write actions are handled by deterministic server controls after user approval.','Do not purchase, publish, message, schedule, charge, claim, redeem, transfer'] as $marker)self::assertStringContainsString($marker,$runtime);
        ...
        self::assertStringContainsString("'used_ai'=>false",$this->compact($api));self::assertStringContainsString("'response_source'=>'system_action'",$this->compact($api));self::assertStringContainsString("'ai_reason'=>$aiReason",$runtime);self::assertStringContainsString("'ai_tokens_total'=>$tokens['total']",$runtime);
    }
