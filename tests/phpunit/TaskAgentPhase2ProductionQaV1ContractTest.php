...
final class TaskAgentPhase2ProductionQaV1ContractTest extends TestCase
{
    ...
    {
        $context = $this->source('includes/task-agent-context.php');
        $router = $this->source('includes/task-agent-intent-router.php');
        $runtime = $this->source('includes/multi-agent-runtime.php');
        $api = $this->source('api/agents/runtime.php');
        $script = $this->source('assets/js/multi-agent-runtime.js');

        foreach (['mg_personal_agent_contacts','mg_personal_agent_upcoming_dates','mg_personal_agent_plans','mg_personal_agent_reminders'] as $marker) {
            self::assertStringContainsString($marker, $context);
        }
        foreach (['mg_task_agent_contextual_response','mg_task_agent_memory_preview_response','mg_task_agent_plan_payload','mg_task_agent_reminder_payload','/discover.php?'] as $marker) {
            self::assertStringContainsString($marker, $router);
        }
        foreach (['mg_task_agent_route','mg_multi_agent_runtime_store','mg_multi_agent_runtime_fallback'] as $marker) {
            self::assertStringContainsString($marker, $runtime);
        }
        foreach (['mg_agent_require_owned','mg_require_csrf_for_write','mg_personal_agent_create_plan','mg_personal_agent_create_reminder','mg_task_agent_memory_save','mg_personal_agent_update_reminder_status'] as $marker) {
            self::assertStringContainsString($marker, $api);
        }
        foreach (["action:'save_draft'","action:'create_reminder'","action:'update_reminder_status'","action:'save_memory'","action:'archive_memory'",'data-agent-open-link'] as $marker) {
    ...
    public function testDeterministicRoutingPrecedesAndUsuallyAvoidsPaidAi(): void
    {
        ...
        $router = $this->source('includes/task-agent-intent-router.php');
        $context = $this->source('includes/task-agent-context.php');
        $memory = $this->source('includes/task-agent-memory.php');

        $routePosition = strpos($runtime, 'mg_task_agent_route($message,$context,$template)');
    ...
    public function testAiContextAndCardsCannotPerformAutonomousWrites(): void
    {
        ...
        foreach (['Never reveal secrets','Do not purchase, publish, message, schedule, charge, claim, redeem, transfer, or save data.','All write actions are handled by deterministic server controls after user approval.'] as $marker) {
            self::assertStringContainsString($marker, $runtime);
        }
        foreach (['mg_task_agent_model_context','mg_task_agent_sanitize_model_cards',"['none','seed_prompt']",'no_purchase_or_send','approval_required'] as $marker) {
            self::assertStringContainsString($marker, $router);
        }
        foreach (['password','api[_ -]?key','claim[_ -]?code','card[_ -]?number','private[_ -]?key','email address','phone number'] as $marker) {
    ...
    {
        $workspace = $this->source('includes/personal-agent/multi-agent-workspace.php');
        $script = $this->source('assets/js/multi-agent-runtime.js');
        $page = $this->source('agent.php');

        foreach (['data-agent-runtime-messages','data-agent-runtime-composer','data-task-agent-context','data-agent-memory-list','data-agent-manage-open'] as $marker) {
        ...
        foreach (['data-save-agent-draft','data-save-agent-reminder','data-reminder-status','data-save-agent-memory','data-archive-agent-memory','data-agent-open-link'] as $marker) {
            self::assertStringContainsString($marker, $script);
        }
    ...
    {
        foreach ([
            'tests/phpunit/TaskAgentContextUpcomingEventsV1ContractTest.php',
            'tests/phpunit/TaskAgentGiftPlanningDraftsV1ContractTest.php',
            'tests/phpunit/TaskAgentRemindersV1ContractTest.php',
            'tests/phpunit/TaskAgentMemoryPermissionSafeRetrievalV1ContractTest.php',
            'tests/phpunit/TaskAgentContextualChatActionCardsV1ContractTest.php',
            'tests/phpunit/MultiAgentRuntimeMemoryV1ContractTest.php',
            'tests/phpunit/UnifiedAgentChatCanvasV2ContractTest.php',
        ] as $path) {
