...
$customerEmail = 'v1-browser@example.test';
$pdo->prepare("INSERT INTO users
    (email,password_hash,full_name,display_name,status,email_verified_at,created_at,updated_at)
    VALUES (?,?,'V1 Browser Customer','V1 Browser Customer','active',NOW(),NOW(),NOW())
    ON DUPLICATE KEY UPDATE full_name=VALUES(full_name),display_name=VALUES(display_name),status='active',email_verified_at=COALESCE(email_verified_at,NOW()),updated_at=NOW()")
    ->execute([$customerEmail, password_hash('BrowserFixturePassword123!', PASSWORD_DEFAULT)]);
$customerStmt = $pdo->prepare('SELECT id FROM users WHERE email=? LIMIT 1');
$customerStmt->execute([$customerEmail]);
$customerId = (int) $customerStmt->fetchColumn();
if ($customerId < 1) throw new RuntimeException('Unable to seed browser customer.');

$roleStmt = $pdo->prepare("SELECT id FROM roles WHERE slug='customer' LIMIT 1");
$roleStmt->execute();
$customerRoleId = (int) $roleStmt->fetchColumn();
if ($customerRoleId < 1) throw new RuntimeException('Unable to resolve browser customer role.');
$pdo->prepare('INSERT IGNORE INTO user_roles (user_id,role_id,created_at) VALUES (?,?,NOW())')
    ->execute([$customerId, $customerRoleId]);

$customer = mg_load_user_auth($customerId);
if (!is_array($customer)) throw new RuntimeException('Unable to load browser customer identity.');
mg_set_session_user($customer, 'password');
session_write_close();
