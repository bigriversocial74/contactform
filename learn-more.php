...
.lm-agent-intro::before,.lm-question::before{
  content:"";
  position:absolute;
  inset:0;
  pointer-events:none;
...
.lm-agent-pin{position:relative;z-index:2;display:block;overflow:visible;padding:0;}
.lm-agent-grid{
  width:min(1180px,92%);
  margin:auto;
  display:grid;
...
.lm-copy h1,.lm-copy h2{margin:0;font-size:clamp(38px,5.2vw,68px);line-height:.96;letter-spacing:-.07em;}
.lm-copy p{margin:20px 0 0;max-width:620px;color:var(--lm-muted);font-size:18px;line-height:1.58;}
.lm-progress{width:min(360px,100%);height:8px;margin-top:20px;border-radius:999px;background:rgba(15,23,42,.09);overflow:hidden;}
.lm-progress span{display:block;height:100%;width:var(--progress,0%);border-radius:999px;background:linear-gradient(90deg,var(--lm-purple),#20bfd2);}
.lm-agent-card{width:100%;margin:0;padding:34px;border:1px solid var(--lm-border);border-radius:28px;background:rgba(255,255,255,.96);box-shadow:0 26px 72px rgba(15,23,42,.11);}
.lm-agent-card h3{margin:0;font-size:30px;line-height:1;letter-spacing:-.05em;}
.lm-agent-card p{margin:12px 0 0;color:var(--lm-muted);line-height:1.5;}
.lm-field{display:grid;gap:10px;margin-top:22px;}
.lm-field label{font-size:12px;font-weight:950;text-transform:uppercase;letter-spacing:.08em;color:#475569;}
.lm-field input,.lm-field select,.lm-field textarea{width:100%;min-height:58px;padding:14px 16px;border:1px solid #cbd5e1;border-radius:15px;background:#fff;color:#0f172a;font:inherit;outline:none;}
.lm-field textarea{min-height:150px;resize:vertical;}
.lm-field input:focus,.lm-field select:focus,.lm-field textarea:focus{border-color:#7c3aed;box-shadow:0 0 0 4px rgba(124,58,237,.1);}
.lm-actions{display:flex;flex-wrap:wrap;gap:12px;margin-top:22px;}
.lm-btn{display:inline-flex;align-items:center;justify-content:center;min-height:48px;padding:0 19px;border-radius:999px;font-weight:950;cursor:pointer;text-decoration:none;}
.lm-btn-primary,.lm-btn-primary:visited,.lm-btn-primary *{border:0;background:#071225!important;color:#fff!important;}
.lm-btn-primary:hover{background:#0f1f36!important;color:#fff!important;}
.lm-btn-secondary{border:1px solid #cbd5e1;background:#fff;color:#071225;}
.lm-btn-link{border:0;background:transparent;color:#64748b;}
.lm-review{display:grid;gap:10px;margin-top:22px;}
.lm-review-row{display:grid;grid-template-columns:150px 1fr;gap:15px;padding:14px 0;border-bottom:1px solid #e2e8f0;}
...
.lm-complete{display:none;margin-top:18px;padding:18px;border-radius:18px;background:#ecfdf5;color:#166534;font-weight:850;}
.lm-complete.is-visible{display:block;}
@media(max-width:900px){
  .lm-agent-intro,.lm-question{padding:88px 0;}
...
@media(max-width:680px){
  .lm-agent-intro,.lm-question{padding:64px 0;}
...
<div class="lm-agent-page" data-learn-more-agent>
  <section class="lm-agent-intro is-active" data-lm-stage="intro">
    <div class="lm-agent-pin"><div class="lm-agent-grid">
      <div class="lm-copy"><span class="mg-badge"><span class="mg-pulse"></span> Agent-guided discovery</span><h1>Tell us what you want to build.</h1><p>Complete the form below to share your business, goals, and the type of Microgifter experience you want to create.</p><div class="lm-progress" style="--progress:8%"><span></span></div></div>
      <article class="lm-agent-card"><h3>Start with the basics.</h3><p>Scroll through the questions, complete the fields that apply, and submit everything at the end.</p><div class="lm-actions"><a class="lm-btn lm-btn-primary" href="#learn-more-form" data-lm-next>Start questionnaire</a><a class="lm-btn lm-btn-secondary" href="/signup.php">Create account</a></div></article>
    </div></div>
  </section>
