...
CREATE TABLE IF NOT EXISTS multi_agent_distribution_program_links (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  public_id CHAR(36) NOT NULL,
  agent_id BIGINT UNSIGNED NOT NULL,
  owner_user_id BIGINT UNSIGNED NOT NULL,
  distribution_program_id BIGINT UNSIGNED NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_multi_agent_distribution_link_public_id (public_id),
  UNIQUE KEY uq_multi_agent_distribution_link_owner_program (owner_user_id,distribution_program_id),
  KEY idx_multi_agent_distribution_link_agent (agent_id,updated_at),
  CONSTRAINT fk_multi_agent_distribution_link_agent FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE CASCADE,
  CONSTRAINT fk_multi_agent_distribution_link_owner FOREIGN KEY (owner_user_id) REFERENCES users(id) ON DELETE CASCADE,
  CONSTRAINT fk_multi_agent_distribution_link_program FOREIGN KEY (distribution_program_id) REFERENCES distribution_programs(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

INSERT INTO schema_migrations (migration_key,description,checksum,applied_at)
VALUES ('20260720_task_agent_phase4_v1','Consolidated Task Agent Phase 4 recurring, group-gifting, distribution-program, and approval integration schema.',NULL,NOW())
