...
SET @mg_sql := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
   WHERE TABLE_SCHEMA=@mg_schema
     ...
     AND COLUMN_NAME='signed_verification_source') = 0,
...
-- Backfill provenance only when an approved maker/checker decision exists.
...
INNER JOIN investment_financial_verification_requests vr
 ...
 AND vr.verification_type IN ('signed_amount','signed_reversal')
...
VALUES (
  ...
  'Adds explicit investor-portal consent visibility, maker-checker signed/funded provenance, relations publish separation, and investor module 10/10 audit hardening support.',
  NOW()
);
